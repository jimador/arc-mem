...
class ImportanceSignalTest {
    ...
    void setUp() {
        ...
        signal = new ImportanceSignal(arcMemEngine, properties);
    }
    ...
    class EmptyContext {
        ...
        @Test
        @DisplayName("returns 0.5 neutral importance when no active memory units exist")
        void noUnitsReturnsNeutral() {
    ...
    class ImportanceScoring {
        ...
        void fullOverlapYieldsHighImportance() {
            when(arcMemEngine.inject(CONTEXT_ID)).thenReturn(List.of(
                    unit("a-1", "dragon guards eastern gate"),
                    unit("a-2", "guardian stands watch")));
        ...
        void noOverlapYieldsZeroImportance() {
            when(arcMemEngine.inject(CONTEXT_ID)).thenReturn(List.of(
                    unit("a-1", "dungeon traps surround"),
                    unit("a-2", "corridors twist underground")));
            // "weather" and "capital" don't appear in units
        ...
        void scoreIsClampedToRange() {
            when(arcMemEngine.inject(CONTEXT_ID)).thenReturn(List.of(
                    unit("a-1", "dragon guardian gate tower castle")));
