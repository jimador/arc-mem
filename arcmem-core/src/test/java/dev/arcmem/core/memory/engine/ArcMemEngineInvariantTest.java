...
@DisplayName("ArcMemEngine — invariant enforcement")
class ArcMemEngineInvariantTest {
    ...
    class ArchiveBlockedByMust {
        ...
        @Test
        @DisplayName("archive is blocked and repository.archiveMemoryUnit is NOT called")
        void archiveBlockedByMustInvariant() {
            ...
            when(repository.findActiveUnits(CONTEXT_ID)).thenReturn(List.of(node));
            ...
            verify(eventPublisher).publishEvent(any(MemoryUnitLifecycleEvent.InvariantViolation.class));
        }
    }
    ...
    class ArchiveWarnedByShould {
        ...
        @Test
        @DisplayName("archive proceeds with warning and repository.archiveMemoryUnit IS called")
        void archiveWarnedByShouldInvariant() {
            var node = unitNode("a2", Authority.RELIABLE);
    ...
    class DemotionBlockedByAuthorityFloor {
        ...
        void demoteBlockedByAuthorityFloor() {
            var node = unitNode("a3", Authority.RELIABLE);
    ...
    class MinAuthorityCountBlocks {
        ...
        void archiveBlockedByMinAuthorityCount() {
            var node = unitNode("a4", Authority.RELIABLE);
    ...
    class EvictionSkipsImmune {
        ...
        @Test
        @DisplayName("invariant-protected memory units are not evicted during budget enforcement")
        void invariantProtectedUnitsNotEvicted() {
            var promotedNode = unitNode("new-1", Authority.PROVISIONAL);
            ...
            var protectedNode = unitNode("protected-1", Authority.RELIABLE);
            protectedNode.setRank(100);
            var normalNode = unitNode("normal-1", Authority.PROVISIONAL);
            normalNode.setRank(150);

            when(repository.findActiveUnits(CONTEXT_ID))
            ...
            when(invariantEvaluator.evaluate(eq(CONTEXT_ID), eq(ProposedAction.EVICT), any(), any()))
                    .thenAnswer(inv -> {
                        MemoryUnit target = inv.getArgument(3);
            ...
            var engineSmallBudget = ArcMemEngineTestFactory.create(
                    repository,
            ...
            verify(repository, never()).archiveUnit(eq("protected-1"));
            verify(repository).archiveUnit("normal-1");
        }
    }
    ...
    class ReinforceHookOrder {
        ...
        void reinforceBlockedByInvariantUpdatesRankButSkipsAuthority() {
            var node = unitNode("a5", Authority.UNRELIABLE);
            node.setRank(500);
        ...
        void reinforceWithoutBlockingInvariantUpgradesAuthority() {
            var node = unitNode("a6", Authority.UNRELIABLE);
            node.setRank(500);
            node.setReinforcementCount(7);
        ...
        void reinforceRespectsPersistedAuthorityCeiling() {
            var node = unitNode("a7", Authority.UNRELIABLE);
            node.setRank(500);
            node.setReinforcementCount(7);
    ...
    class PromotionPersistsAuthorityCeiling {
        ...
        void promoteStoresAuthorityCeiling() {
            ...
            verify(repository).promoteToUnit(
                    "p7",
                    500,
    ...
    private static ArcMemProperties propertiesWithBudget(int budget) {
