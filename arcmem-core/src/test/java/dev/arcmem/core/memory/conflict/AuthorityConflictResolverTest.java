...
class AuthorityConflictResolverTest {
    ...
    @Test
    @DisplayName("keeps existing when memory unit has RELIABLE authority (legacy resolver)")
    void keepsExistingReliable() {
        var conflict = new ConflictDetector.Conflict(
                MemoryUnit.withoutTrust("1", "King is dead", 700, Authority.RELIABLE, false, 0.9, 0),
    ...
    @Test
    @DisplayName("keeps existing when memory unit has CANON authority (legacy resolver)")
    void keepsExistingCanon() {
        var conflict = new ConflictDetector.Conflict(
                MemoryUnit.withoutTrust("1", "King is dead", 850, Authority.CANON, true, 0.99, 5),
    ...
    @Test
    @DisplayName("replaces PROVISIONAL memory unit with high-confidence incoming (legacy resolver)")
    void replacesProvisionalHighConfidence() {
        var conflict = new ConflictDetector.Conflict(
                MemoryUnit.withoutTrust("1", "Something uncertain", 300, Authority.PROVISIONAL, false, 0.5, 0),
    ...
    @Test
    @DisplayName("coexists for PROVISIONAL memory unit with moderate confidence (legacy resolver)")
    void coexistsProvisionalModerateConfidence() {
        var conflict = new ConflictDetector.Conflict(
                MemoryUnit.withoutTrust("1", "Maybe this", 400, Authority.PROVISIONAL, false, 0.6, 0),
    ...
    class CanonAlwaysKeep {
        ...
        void canonAlwaysKeepsExisting() {
            var conflict = new ConflictDetector.Conflict(
                    MemoryUnit.withoutTrust("1", "Canon fact", 850, Authority.CANON, true, 0.99, 5),
    ...
    class ReliableMatrix {
        ...
        void reliableHighConfidenceReturnsReplace() {
            var conflict = new ConflictDetector.Conflict(
                    MemoryUnit.withoutTrust("1", "Reliable fact", 700, Authority.RELIABLE, false, 0.9, 3),
    ...
    class UnreliableMatrix {
        ...
        void unreliableAdequateConfidenceReturnsReplace() {
            var conflict = new ConflictDetector.Conflict(
                    MemoryUnit.withoutTrust("1", "Unreliable fact", 350, Authority.UNRELIABLE, false, 0.6, 1),
    ...
    class ProvisionalAlwaysReplace {
        ...
        void provisionalAlwaysReplaces() {
            var conflict = new ConflictDetector.Conflict(
                    MemoryUnit.withoutTrust("1", "Provisional fact", 300, Authority.PROVISIONAL, false, 0.5, 0),
    ...
    class DegradedDetectionQuality {
        ...
        @Test
        @DisplayName("DEGRADED conflict with RELIABLE memory unit still returns KEEP_EXISTING")
        void degradedReliableUnitStillKeepsExisting() {
            var conflict = new ConflictDetector.Conflict(
                    MemoryUnit.withoutTrust("1", "Reliable fact", 700, Authority.RELIABLE, false, 0.9, 3),
