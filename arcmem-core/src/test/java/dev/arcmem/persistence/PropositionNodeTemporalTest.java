...
class PropositionNodeTemporalTest {
    ...
    class JsonRoundTrip {
        ...
        void temporalFieldsSurviveJsonRoundTrip() throws Exception {
            ...
            var node = new PropositionNode(UUID.randomUUID().toString(), "test-context", "test text", 0.9, 0.0, null, List.of(),
        ...
        void nonTemporalFieldsPreservedAfterRoundTrip() throws Exception {
            var node = new PropositionNode(UUID.randomUUID().toString(), "test-context", "round-trip text", 0.85, 0.0, null, List.of(),
                    Instant.now(), Instant.now(), PropositionStatus.ACTIVE, null, List.of());
            node.setRank(500);
    ...
    class ConvenienceConstructorDefaults {
        ...
        void twoArgConstructorSetsTemporalFieldsToNull() {
            var node = new PropositionNode(UUID.randomUUID().toString(), "test-context", "simple text", 0.5, 0.0, null, List.of(),
                    Instant.now(), Instant.now(), PropositionStatus.ACTIVE, null, List.of());
