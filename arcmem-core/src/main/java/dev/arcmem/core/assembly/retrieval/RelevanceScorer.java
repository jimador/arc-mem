...
/**
 * Computes relevance scores for units using heuristic signals (authority, tier, confidence)
...
public class RelevanceScorer {
    ...
    public double computeHeuristicScore(MemoryUnit unit, @Nullable ScoringConfig scoring) {
        var authorityVal = authorityValue(unit.authority());
        var tierVal = tierValue(unit.memoryTier());
        var confidence = unit.confidence();
        if (scoring == null) {
    ...
    /**
     * Scores units by relevance to a query, blending LLM semantic scores with heuristic scores.
    ...
    public List<ScoredMemoryUnit> scoreByRelevance(String query, List<MemoryUnit> units,
        ...
        if (chatModel == null || llmCallService == null) {
            ...
            return scoreAndRank(units, scoring);
        }

        try {
            var llmScores = callLlmForScores(query, units);
            return blendScores(units, llmScores, scoring);
        } catch (Exception e) {
