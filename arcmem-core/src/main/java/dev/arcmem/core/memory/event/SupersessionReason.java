 ...
 * near-total overlap. {@code ArchiveReason} describes <em>why an unit was deactivated</em>
 * (which may or may not involve supersession). {@code SupersessionReason} describes
 * <em>why one unit replaced another</em> (which always implies archival of the predecessor).
 ...
 *       {@code SUPERSEDES} relationship in practice — decayed units typically fade
...
public enum SupersessionReason {
     ...
     * lifecycle trigger (unit decayed below viability threshold) but use different
