...
import dev.dunnam.arcmem.core.memory.event.ArchiveReason;
import dev.dunnam.arcmem.core.persistence.MemoryUnitRepository;
import dev.dunnam.arcmem.core.persistence.TieredMemoryUnitRepository;
...
public class SimulationTurnExecutor {
    ...
    private ComplianceSnapshot enforceCompliance(String dmResponse, List<MemoryUnit> units, boolean injectionEnabled) {
        ...
        try {
            ...
            return new ComplianceSnapshot(
                    ...
                    result.suggestedAction() == dev.dunnam.arcmem.core.assembly.compliance.ComplianceAction.RETRY
                    || result.suggestedAction() == dev.dunnam.arcmem.core.assembly.compliance.ComplianceAction.REJECT,
     ...
     */
    static List<SimulationTurn.UnitEvent> diffUnitState(
            Map<String, MemoryUnit> previous,
            List<MemoryUnit> current,
            int turnNumber,
            Set<String> archivedUnitIds) {

        var events = new ArrayList<SimulationTurn.UnitEvent>();
        ...
        for (var unit : current) {
            ...
            if (prev == null) {
                events.add(new SimulationTurn.UnitEvent(
    ...
    private static dev.dunnam.arcmem.core.assembly.compaction.CompactionConfig toAssemblyConfig(
            SimulationScenario.CompactionConfig scenarioConfig) {
        return new dev.dunnam.arcmem.core.assembly.compaction.CompactionConfig(
