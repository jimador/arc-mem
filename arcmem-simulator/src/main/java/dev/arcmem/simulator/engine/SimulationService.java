...
import dev.arcmem.simulator.adversary.AdaptiveAttackPrompter;
import dev.arcmem.simulator.adversary.AttackHistory;
import dev.arcmem.simulator.adversary.AttackOutcome;
import dev.arcmem.simulator.adversary.AttackPlan;
import dev.arcmem.simulator.adversary.TieredEscalationStrategy;
...
public class SimulationService {
        ...
        try {
            ...
            if (scenario.setting() != null && !scenario.setting().isBlank()
                ...
                var sceneUnitEvents = sceneTurn.unitEvents() != null
                        ? sceneTurn.unitEvents() : List.<SimulationTurn.MemoryUnitEvent> of();
            ...
            for (int i = 0; i < maxTurns && !ctx.isCancelRequested(); i++) {
                ...
                var unitEvents = turn.unitEvents() != null ? turn.unitEvents() : List.<SimulationTurn.MemoryUnitEvent> of();
    ...
    private String generateWarmUpMessage(SimulationScenario scenario) {
        if (scenario.persona() != null) {
            return PromptTemplates.render(SimulationPromptPaths.WARMUP_PLAYER_MESSAGE, Map.of(
    ...
    private String generateAdversarialMessage(SimulationScenario scenario, List<String> history) {
        if (!scenario.adversarial() || scenario.groundTruth() == null || scenario.groundTruth().isEmpty()) {
            return PromptTemplates.load(SimulationPromptPaths.DEFAULT_PLAYER_MESSAGE).trim();
        }
        ...
        var prompt = PromptTemplates.render(SimulationPromptPaths.ADVERSARIAL_REQUEST, Map.of(
        ...
        try {
            ...
            return PromptTemplates.load(SimulationPromptPaths.ADVERSARIAL_FALLBACK_MESSAGE).trim();
        }
    }
