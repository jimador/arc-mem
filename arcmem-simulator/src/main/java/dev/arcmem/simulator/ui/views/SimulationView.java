...
import dev.arcmem.simulator.engine.SimControlState;
...
public class SimulationView extends VerticalLayout {
    ...
    private final MemoryUnitManipulationPanel manipulationPanel;
    private final MemoryUnitTimelinePanel timelinePanel;
    ...
    private int unitCountBeforePause;
    ...
    public SimulationView(
        ...
        var title = new H2("ARC-Mem Drift Simulator");
        ...
        injectionToggle = new Checkbox("ARC-Mem Injection", true);
        ...
        timelinePanel = new MemoryUnitTimelinePanel();
        manipulationPanel = new MemoryUnitManipulationPanel(contextUnitRepository, arcMemEngine, mutationStrategy);
        knowledgeBrowserPanel = new KnowledgeBrowserPanel(arcMemEngine, contextUnitRepository);
        ...
        inspectorPanel.setBrowseCallback(unitText -> {
            knowledgeBrowserPanel.filterByUnitText(unitText);
            rightTabSheet.setSelectedTab(knowledgeBrowserTab);
        });
        inspectorPanel.setBrowseGraphCallback(unitText -> {
            knowledgeBrowserPanel.focusGraphForUnitText(unitText);
            rightTabSheet.setSelectedTab(knowledgeBrowserTab);
        });
    ...
    private void transitionTo(SimControlState target) {
        ...
        switch (target) {
            ...
            case PAUSED -> {
                ...
                if (contextId != null) {
                    unitCountBeforePause = arcMemEngine.activeCount(contextId);
                    manipulationPanel.loadUnits(contextId);
                }
    ...
    private void wireButtons() {
        ...
        resumeButton.addClickListener(e -> {
            ...
            var contextId = simulationService.getCurrentContextId();
            if (contextId != null) {
                int currentCount = arcMemEngine.activeCount(contextId);
                int delta = currentCount - unitCountBeforePause;
    ...
    private void startSimulation(SimulationScenario scenario) {
        ...
        var seedCount = scenario.seedUnits() != null ? scenario.seedUnits().size() : 0;
        driftSummaryPanel.setSeedUnitCount(seedCount);
    ...
    private void initTheme() {
        UI.getCurrent().getPage().executeJs("""
                                                    const t = localStorage.getItem('unit-theme');
