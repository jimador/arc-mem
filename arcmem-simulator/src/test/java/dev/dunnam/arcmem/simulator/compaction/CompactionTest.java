...
import dev.dunnam.arcmem.core.assembly.compaction.CompactionDriftEvaluator;
...
import dev.dunnam.arcmem.core.assembly.protection.PropositionContentProtector;
import dev.dunnam.arcmem.core.assembly.protection.UnitContentProtector;
import dev.dunnam.arcmem.core.memory.engine.ArcMemEngine;
import dev.dunnam.arcmem.core.memory.model.Authority;
import dev.dunnam.arcmem.core.memory.model.MemoryUnit;
...
class CompactionTest {
    ...
    class CompactedContextProviderTests {
        ...
        @BeforeEach
        void setUp() {
            provider = new CompactedContextProvider(List.of(), summaryGenerator, arcMemEngine, event -> {});
        }
        ...
        class Compact {
            ...
            void compactCollectsProtectedContentIds() {
                ...
                when(protector.getProtectedContent(CONTEXT_ID)).thenReturn(List.of(
                        new ProtectedContent("unit-1", "The sword is cursed", 700, "unit"),
                        new ProtectedContent("unit-2", "Elf king is hostile", 500, "unit")
                ));
                when(summaryGenerator.generateSummary(any(), anyString(), any(), anyInt(), any()))
                        .thenReturn(new SummaryResult("Summary.", 0, false));

                var providerWithProtectors = new CompactedContextProvider(
                        List.of(protector), summaryGenerator, arcMemEngine, event -> {});
                providerWithProtectors.addMessage(CONTEXT_ID, "msg");
                var config = new CompactionConfig(true, 0, 1, List.of(), 0.5, 2, 1000L, true);

                var result = providerWithProtectors.compact(CONTEXT_ID, config, "test");

                assertThat(result.protectedContentIds()).containsExactly("unit-1", "unit-2");
            }
        }
    ...
    @Nested
    @DisplayName("UnitContentProtector")
    class UnitContentProtectorTests {

        @Mock
        private ArcMemEngine arcMemEngine;

        private UnitContentProtector protector;

        @BeforeEach
        void setUp() {
            protector = new UnitContentProtector(arcMemEngine);
        }

        @Test
        @DisplayName("priority equals context unit activation score")
        void getProtectedContentPriorityEqualsRank() {
            var unit = MemoryUnit.withoutTrust(
                    "a-1", "The dragon is sleeping", 750, Authority.RELIABLE, false, 0.9, 3);
            when(arcMemEngine.inject(CONTEXT_ID)).thenReturn(List.of(unit));
        ...
        @Test
        @DisplayName("returns id and text from context unit")
        ...
        @Test
        @DisplayName("returns multiple protected items for multiple context units")
        void getProtectedContentMultipleUnitsReturnsMultipleItems() {
            var units = List.of(
                    MemoryUnit.withoutTrust("a-1", "Fact one", 800, Authority.CANON, true, 1.0, 10),
                    MemoryUnit.withoutTrust("a-2", "Fact two", 300, Authority.PROVISIONAL, false, 0.6, 0)
            );
            when(arcMemEngine.inject(CONTEXT_ID)).thenReturn(units);
        ...
        @Test
        @DisplayName("returns empty list when no context units exist")
        void getProtectedContentNoUnitsReturnsEmpty() {
    ...
    class CompactionDriftEvaluatorTests {
        ...
        void evaluateLossTextIncludesContentId() {
            var losses = evaluator.evaluate(Set.of("unit-42"), Set.of());

            assertThat(losses.getFirst().lostText()).contains("unit-42");
        }
