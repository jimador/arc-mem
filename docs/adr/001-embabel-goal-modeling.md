...
Primary question: should `SemanticUnitPromoter` move from the current imperative pipeline to Embabel goal-directed action chaining (`@Action` + `@AchievesGoal` + GOAP planner)?

Recommendation: not yet.

## 1) Current pipeline (what exists now)

`SemanticUnitPromoter` uses a strict five-gate sequence:
...
Location: `arcmem-core/src/main/java/dev/arcmem/core/extraction/SemanticUnitPromoter.java`
...
```java
var conflicts = arcMemEngine.detectConflicts(contextId, prop.getText());
...
```java
if (trustScore != null) {
    arcMemEngine.promote(prop.getId(), initialRank, trustScore.authorityCeiling());
} else {
    arcMemEngine.promote(prop.getId(), initialRank);
}
promoted++;
```
...
```java
record DemoteCommand(String unitId, DemotionReason reason) {}
...
agentProcess.addObject(arcMemEngine);
agentProcess.addObject(memoryUnitRepository);
```
...
Current recommendation: **defer goal-directed refactor for `SemanticUnitPromoter`**.
...
- `SemanticUnitPromoter`: `arcmem-core/src/main/java/dev/arcmem/core/extraction/SemanticUnitPromoter.java`
- `ChatActions`: `arcmem-simulator/src/main/java/dev/arcmem/simulator/chat/ChatActions.java`
- `ChatConfiguration`: `arcmem-simulator/src/main/java/dev/arcmem/simulator/chat/ChatConfiguration.java`
- OpenSpec artifacts:
