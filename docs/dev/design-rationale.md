...
LLMs degrade in multi-turn conversations. Laban et al. (2025) measured a 39% average performance drop from single-turn to multi-turn settings across 15 models, with degradation starting at 2+ turns. The root cause is not context capacity but the absence of any mechanism to distinguish **load-bearing facts** from ambient context.
...
| `PropositionRepository` tier-aware queries | Filter by memory tier (`T0_INVARIANT`, `T1_WORKING`, `T2_EPISODIC`) | Optional tier parameter on existing `findByContext` methods | Current repo adds tier semantics at the application layer, not in shared persistence contracts |
