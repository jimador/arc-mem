# Anchor Working Memory Research Roadmap
...
4. Changes to the core anchor engine (`anchor/` package) — this roadmap evaluates the engine, not modifies it.
...
4. The paper MUST position dice-anchors explicitly against MemGPT/Letta, Zep/Graphiti, ShardMemo, MemOS, and A-MemGuard. It MUST NOT claim generalization beyond what is tested.
...
Multi-agent anchor governance — extending the trust/authority model to agent-to-agent communication with asymmetric context budgets. Captured as future work in the paper's discussion section.
...
| F05        | multi-agent-anchor-governance     | —               | Future work; scaffold when ready                |
...
1. **No cross-domain evaluation**: This roadmap evaluates anchors in a D&D/collaborative fiction domain only. Cross-domain evaluation (medical, legal, customer support) is deferred.
2. **Single-model evaluation**: Initial experiments will use one LLM (likely GPT-4o or Claude). Cross-model evaluation (comparing anchor effectiveness across different LLM backends) is a natural follow-up but not in scope.
