...
Introduce a spreading-activation-inspired retrieval mechanism that, with configurable probability, fetches graph-distant nodes alongside directly referenced Propositions. The goal is to inject controlled serendipity into anchor-governed context assembly, enabling the LLM to surface creative connections it would not otherwise make. This is the GENERATIVE complement to the DEFENSIVE anchor resilience work in Track A.
...
3. Conflict guard: serendipitous nodes MUST NOT contradict existing anchors (checked via `NegationConflictDetector` or equivalent).
...
1. Modifications to the core anchor engine (`anchor/` package) -- rank, authority, and budget semantics are unchanged.
...
2. **`assembly`**: `AnchorsLlmReference` or equivalent context assembly component extended to format and inject serendipitous nodes with clear provenance markers.
...
3. Serendipitous nodes MUST NOT contradict existing active anchors. Conflict detection MUST use the same mechanism as anchor conflict resolution (`NegationConflictDetector` or equivalent).
4. Serendipitous nodes MUST NOT count against the anchor budget (max 20). They are injected as supplementary context, not promoted to anchors.
...
3. **Constraints/non-goals**: No modification to anchor semantics (rank, authority, budget). No embedding-based similarity. No real-time graph learning. The mechanism is structural (hop-based), not semantic.
...
2. **Requirement**: Serendipitous nodes SHALL be conflict-checked against active anchors before injection.
