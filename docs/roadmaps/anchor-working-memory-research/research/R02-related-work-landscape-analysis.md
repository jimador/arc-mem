...
How does dice-anchors position against MemGPT/Letta, Zep/Graphiti, ShardMemo, MemOS, and A-MemGuard, and what is the differentiation argument for the tech report?

## Why This Matters

The tech report MUST position dice-anchors within the existing landscape of LLM memory systems to establish novelty and contribution. Without a rigorous comparative analysis, the paper risks either (a) claiming uniqueness that does not hold under scrutiny, or (b) failing to articulate what specifically is new. The adversarial memory space is active: MemGPT/Letta, Zep/Graphiti, ShardMemo, and MemOS all address aspects of LLM memory, and A-MemGuard directly addresses memory security. The paper MUST demonstrate that dice-anchors occupies a distinct position and that its design choices produce measurably different properties.

## Scope

### In Scope

1. Systematic comparison of dice-anchors against six systems: MemGPT/Letta, Zep/Graphiti, ShardMemo, MemOS, A-MemGuard, and ASB (as a benchmark framework).
2. Analysis along four dimensions: core problem addressed, adversarial resistance mechanism, trust/authority model, and working memory semantics.
3. Identification of the specific design space gap that dice-anchors fills.
4. Attack landscape analysis: MINJA, InjecMEM, MemoryGraft as motivation for why adversarial resistance matters.
...
3. **Gap identification:** Map the design space and identify where dice-anchors sits relative to the others.
4. **Attack landscape review:** Survey recent memory attack papers to establish the threat model that motivates dice-anchors.
5. **Taxonomy alignment:** Map dice-anchors to the memory taxonomy from arXiv:2512.13564 to use shared vocabulary.
...
**Positioning vs. dice-anchors:** MemGPT's design goal is capacity (virtual context extension); authority and adversarial resistance are out of scope. dice-anchors assumes the context window is large enough (modern 128K+ context windows reduce the capacity pressure) and focuses on what gets injected and with what priority.
...
**Positioning vs. dice-anchors:** Zep provides temporal accuracy for long-term storage. Its conflict resolution is recency-based, not authority-based. It is a complementary system (dice-anchors could theoretically use Zep as a persistence backend) but does not address the working memory governance problem.
...
**Positioning vs. dice-anchors:** ShardMemo solves multi-agent retrieval scalability. Its Tier A per-agent working state is an unstructured blob with no governance semantics. dice-anchors' working memory model (ranked, authority-governed, budget-enforced) could theoretically serve as a structured replacement for Tier A.
...
**Trust model:** Provenance is descriptive (audit trail) not operative (access control). This is the key distinction: MemOS tells you where a memory came from; dice-anchors tells you how much to trust it and what to do when it conflicts with a higher-authority memory.

**Positioning vs. dice-anchors:** MemOS provenance is descriptive; dice-anchors authority levels are operative -- they drive injection resistance (higher authority = harder to contradict), eviction priority (lower rank evicted first under budget pressure), and promotion gating (CANON never auto-assigned).
...
**Positioning vs. dice-anchors:** A-MemGuard uses binary access control; dice-anchors uses graduated authority with promotion semantics and budget enforcement. Specifically:
- A-MemGuard: memory is allowed or blocked. dice-anchors: memory has rank [100-900] and authority (PROVISIONAL -> UNRELIABLE -> RELIABLE -> CANON), which determines injection priority, eviction order, and conflict resolution outcome.
- A-MemGuard: defense is reactive (detect and block). dice-anchors: resistance is structural (the authority hierarchy inherently privileges established facts over new injections).
- A-MemGuard: no budget management. dice-anchors: max 20 active anchors with rank-based eviction under budget pressure.
- A-MemGuard: binary outcome (allow/block). dice-anchors: graduated outcome (low-authority propositions coexist with high-authority anchors but are outranked in context assembly).
...
| InjecMEM (E9) | Retriever-agnostic anchor + command injection | High (retriever-independent) | Retrieved content is trusted |
| MemoryGraft (E10) | Poisoned episodic memory retrieval | Persistent across sessions | Long-term memory is trustworthy |

All three attacks exploit the same fundamental assumption: **content retrieved from memory is treated as trustworthy.** This is the assumption that dice-anchors challenges. By assigning authority levels to memory entries and enforcing upgrade-only semantics (an anchor's authority can only increase, never decrease; CANON is never auto-assigned), the system creates structural resistance to injected content.

### Memory Taxonomy Alignment (E11)

Per the taxonomy from arXiv:2512.13564:

| Dimension | dice-anchors Classification |
|-----------|---------------------------|
...
| **Formation** | Explicit extraction (DICE pipeline) + explicit promotion (AnchorPromoter) |
...
dice-anchors is the only system that combines:
1. **Working memory as the target domain** (not long-term storage or retrieval).
2. **Structural adversarial resistance** (not reactive detection, not absent).
3. **Graduated authority** (not binary, not implicit).
4. **Budget enforcement** (fixed active anchor limit with rank-based eviction).
...
> Existing LLM memory systems address capacity (MemGPT/Letta), temporal accuracy (Zep/Graphiti), retrieval scalability (ShardMemo), and lifecycle management (MemOS). These systems treat all memory content with equal trust, leaving them vulnerable to adversarial injection attacks that achieve >95% success rates (MINJA) and persist across sessions (MemoryGraft). A-MemGuard introduces reactive defense via binary trust scoring, but its allow/block model lacks the graduated semantics needed for nuanced working memory governance. dice-anchors introduces trust-governed working memory: a graduated authority hierarchy (PROVISIONAL through CANON) with upgrade-only semantics, rank-based eviction under budget constraints, and structural resistance to adversarial drift. Where existing systems ask "what should the model remember?", dice-anchors asks "what should the model trust, and how much?"
...
2. The differentiation argument SHOULD be presented as a two-part claim: (a) no existing system combines trust governance with working memory management, and (b) the authority hierarchy produces measurably different behavior under adversarial pressure (demonstrated by the ablation study).
3. The paper MUST NOT claim that dice-anchors replaces or supersedes these systems. They solve different problems. The claim is complementarity and novelty in the specific intersection of trust governance and working memory.
4. The attack landscape (MINJA, InjecMEM, MemoryGraft) SHOULD be cited in the introduction to motivate why adversarial resistance matters. ASB SHOULD be cited to establish the evaluation gap.
5. The memory taxonomy from arXiv:2512.13564 SHOULD be used to position dice-anchors using shared vocabulary (working memory function, token-level form, authority-driven evolution dynamics).

### For Future Work

1. The paper's discussion section SHOULD note that dice-anchors' authority model could complement existing systems: e.g., authority-governed MemCubes in MemOS, authority-aware edges in Graphiti, trust-graduated rather than binary gating in A-MemGuard.
2. Cross-system integration experiments (e.g., using Zep as a persistence backend with dice-anchors providing authority governance at the working memory layer) MAY be proposed as future work.
...
| **F01 (Experiment Framework)** | The ablation study design is informed by this analysis: the framework MUST support anchors-enabled vs. anchors-disabled conditions to demonstrate the behavioral difference that justifies the differentiation claim. |
| **Paper positioning** | The "trust-governed working memory" framing distinguishes dice-anchors from all surveyed systems without overclaiming. The paper does not need to demonstrate superiority on existing benchmarks (LoCoMo, etc.) because those benchmarks measure different properties (retrieval accuracy, temporal consistency). |
...
1. **No head-to-head benchmark exists.** Because dice-anchors and the compared systems solve different problems, there is no shared benchmark for direct comparison. The paper MUST frame this as a feature (complementary systems in different parts of the design space), not a weakness.
