...
My current proof of concept for this is [**ARC-Mem**](https://github.com/jimador/dice-anchors).

DICE handles proposition extraction and persistence. Embabel handles orchestration. ARC-Mem is the current proposition-based prototype implementation of ARC.

In this prototype, propositions are the semantic units. A memory unit is a DICE proposition that has been promoted and given additional structure. The key fields are activation score, authority, and membership in a bounded working set. In practice, that means the system maintains a small active pool of facts rather than feeding everything it has ever seen back into the prompt. Operationally, that activation score is standing in for the activation idea: higher-scored units earn retention in the active context.
...
At runtime, memory units are injected into prompts as a ranked established-facts block. Retrieval-based memory is opportunistic: the system may surface the right fact, or it may not. ARC, by contrast, is about deliberately keeping certain state in play. In the current prototype, ARC-Mem makes that policy visible by explicitly marking and budgeting which units remain active in prompt assembly itself.

The more interesting part is how that active set is managed.

Conflict checks evaluate contradictory candidate updates before they are allowed to quietly replace established state. Trust scoring helps determine whether a newly extracted proposition should be promoted at all. Reinforcement and decay adjust importance over time so repeatedly confirmed facts gain staying power while stale facts lose it. Budget enforcement keeps the working set bounded by evicting low-value memory units instead of allowing context to sprawl indefinitely.
