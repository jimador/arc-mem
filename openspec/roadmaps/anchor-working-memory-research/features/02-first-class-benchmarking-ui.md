...
2. **Vaadin 24.6.4**: The existing UI framework. The benchmarking view MUST use the same theme (`frontend/themes/anchor-retro/styles.css`) and component patterns as SimulationView and ChatView.
...
3. **`frontend/themes/anchor-retro/styles.css`**: New CSS classes for comparison views, delta badges, heatmap cells, and effect size indicators.
...
1. **Experiment Configuration Panel**:
   - Condition selection MUST use checkboxes for the four standard conditions (FULL_ANCHORS, NO_ANCHORS, FLAT_AUTHORITY, NO_RANK_DIFFERENTIATION). At least two conditions MUST be selected to run an experiment.
...
3. **Condition Comparison View**:
   ...
   - MUST display delta badges between condition pairs (e.g., "+12.3% survival rate" between FULL_ANCHORS and NO_ANCHORS).
...
4. **Fact-Level Drill-Down**:
   ...
   - Per-fact display MUST show: fact text, survival count per condition (e.g., "5/5 in FULL_ANCHORS, 2/5 in NO_ANCHORS"), and first-drift turn per condition.
...
6. Per-fact drill-down MUST show per-fact survival counts across conditions (e.g., "Fact X survived in FULL_ANCHORS 5/5, drifted in NO_ANCHORS 2/5").
...
9. The UI MUST use the existing `anchor-retro` CSS theme for visual consistency.
...
| **CSS theme consistency** | Low | Low | New CSS classes MUST follow the naming conventions in `anchor-retro/styles.css`. Delta badges and heatmap cells SHOULD be reviewed against the existing theme before finalization. |
...
3. **Constraints/non-goals**: No modifications to SimulationView's existing BenchmarkPanel. No external charting libraries unless Vaadin's built-in components are insufficient. No data export (candidate extension). Use existing `anchor-retro` theme.
