# Research Task: TITANS-Inspired Integration Hypotheses for dice-anchors
...
Given the validated overlap, what concrete and testable changes should be run in dice-anchors first?
...
2. Anchors remains trust-governed working memory.
...
Code touchpoints:
1. `src/main/java/dev/dunnam/diceanchors/extract/AnchorPromoter.java`
2. `src/main/java/dev/dunnam/diceanchors/anchor/TrustPipeline.java`
3. `src/main/java/dev/dunnam/diceanchors/anchor/TrustScore.java`

Metrics:
1. Fact Survival Rate
2. Anchor-fact contradiction/hallucination incidence
...
Proposed implementation:
1. Track a short-lived reinforcement accumulator per anchor.
2. Decay accumulator each turn; consume into rank updates.

Code touchpoints:
1. `src/main/java/dev/dunnam/diceanchors/anchor/ReinforcementPolicy.java`
2. `src/main/java/dev/dunnam/diceanchors/anchor/DecayPolicy.java`
3. `src/main/java/dev/dunnam/diceanchors/persistence/PropositionNode.java`
...
2. Lower rank volatility for high-value anchors.
...
Code touchpoints:
1. `src/main/java/dev/dunnam/diceanchors/anchor/AnchorEngine.java`
2. `src/main/java/dev/dunnam/diceanchors/assembly/PromptBudgetEnforcer.java`
3. `src/main/java/dev/dunnam/diceanchors/assembly/CompactionValidator.java`

Metrics:
1. Anchor retention quality under token stress
...
2. `src/main/java/dev/dunnam/diceanchors/sim/engine/SimulationTurnExecutor.java`
3. `src/main/java/dev/dunnam/diceanchors/sim/report/MarkdownReportRenderer.java`

Metrics:
1. Fact survival by compaction setting
2. Inconsistency severity distribution by setting
3. Anchor churn (created/evicted per turn)
...
2. Momentum carryover may over-entrench early wrong anchors.
...
Proceed with a benchmark-first integration path (H4 -> H1 -> H2 -> H3). This sequence extracts TITANS-style memory dynamics where they are likely to help, while preserving the explicit governance model that defines dice-anchors.
