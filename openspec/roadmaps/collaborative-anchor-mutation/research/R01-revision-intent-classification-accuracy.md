...
Statement B (existing anchor): {{ statement_b }}
...
2. **Authority gating reduces effective risk**: Even if an adversarial input is classified as REVISION, it only succeeds in triggering supersession if the target anchor's authority level permits it (R03). RELIABLE and CANON anchors are protected by independent gates. The practical false-positive risk applies only to PROVISIONAL and UNRELIABLE anchors — where false positives are less damaging because those anchors are already expendable.

3. **The CANON exemption (F01 requirement) eliminates the highest-stakes false-positive scenario**: CANON anchors MUST NOT be revision-eligible regardless of classification. This removes the catastrophic failure mode.
...
**Recommended target**: < 5% false-positive rate for PROVISIONAL/UNRELIABLE anchors; 0% for RELIABLE anchors by default (via authority gating). The < 5% target matches the accuracy achievable from few-shot prompted GPT-4.1/Claude Sonnet at time of writing.
...
2. **Existing anchor authority level** (medium signal, available from codebase): Passing `anchor.authority()` as context to the classification prompt allows the LLM to calibrate: "this is a PROVISIONAL anchor, which means it was never confirmed as established fact." This aligns with R03's finding that PROVISIONAL/UNRELIABLE anchors are revision-eligible. The prompt variable `{{ anchor_authority }}` is straightforward to add — `LlmConflictDetector.evaluatePair()` already has the `anchor` object available (E03).
...
**Remaining attack surface**: An adversary who crafts a statement that genuinely looks like a revision (uses correction markers, targets a PROVISIONAL anchor, expresses plausible correction intent) while actually intending to corrupt a fact cannot be reliably distinguished by LLM classification alone. This is the irreducible false-positive risk. The authority gating (RELIABLE anchors not revision-eligible by default) and CANON exemption are the backstops for this case.
...
- The existing `evaluatePair()` method already has all necessary context: `incomingText`, `anchor.text()`, and `anchor.authority()` (E03).
...
- "A separate call is cleaner" — clean separation at the cost of doubled LLM calls per detected conflict; conflicts are already rare (most anchors don't conflict with most inputs), so the batch efficiency case for a separate call is weak.
...
5. **Create `RevisionAwareConflictResolver`** that dispatches on `ConflictType`:
   - `REVISION` → route to `AnchorEngine.supersede()` (new behavior, gated by authority eligibility from R03).
...
3. **Pass `anchor.authority()` as a template variable** — the instruction "this anchor has PROVISIONAL authority, which means it was not yet confirmed" shifts the classification prior appropriately.
...
6. Evaluate manually on the R00 failure scenario: "Actually, I want Anakin Skywalker to be a bard" should classify as REVISION against the PROVISIONAL "Anakin Skywalker is a wizard" anchor.
