...
The system SHALL provide an `UnitMutationStrategy` interface that gates all context unit text mutation requests. Every context unit mutation — regardless of origin (UI, LLM tool, conflict resolver) — MUST route through this SPI before execution. The interface is unsealed to allow test doubles; the `HitlOnlyMutationStrategy` implementation is `final`.
...
The active `UnitMutationStrategy` SHALL be configurable via `context units.context unit.mutation.strategy` property. The default value SHALL be `hitl-only`.

#### Scenario: Default configuration

- **WHEN** no `context unit.mutation.strategy` property is set
- **THEN** `HitlOnlyMutationStrategy` SHALL be the active bean

#### Scenario: Explicit hitl-only configuration

- **WHEN** `context unit.mutation.strategy = hitl-only` is configured
- **THEN** `HitlOnlyMutationStrategy` SHALL be the active bean
...
- **THEN** the context unit SHALL be revised (predecessor archived, successor created)

#### Scenario: ChatView revision denied by strategy

- **WHEN** user clicks "Revise" in ChatView AND the strategy returns `Deny`
- **THEN** the context unit SHALL NOT be revised AND an error notification SHALL display the denial reason

### Requirement: Sim panel revision UI

The `UnitManipulationPanel` SHALL include an inline text field and "Revise" button for each context unit in the edit card. Revision requests SHALL route through `UnitMutationStrategy` with `source = UI`.

#### Scenario: Sim panel revision

- **WHEN** operator enters revised text and clicks "Revise" in the sim context unit edit card
- **THEN** the mutation request SHALL be evaluated by `UnitMutationStrategy`
- **AND** if allowed, the context unit SHALL be revised and an intervention event SHALL be logged

## Invariants

- **CAM1**: Every context unit text mutation MUST route through `UnitMutationStrategy.evaluate()` — no bypass paths
