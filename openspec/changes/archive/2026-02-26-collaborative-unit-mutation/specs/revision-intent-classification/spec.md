...
- `WORLD_PROGRESSION` — return `COEXIST` (both facts are valid at different points in time; KEEP_EXISTING would reject the incoming proposition, causing context unit loss when combined with REPLACE outcomes in the same conflict batch)
...
`RevisionAwareConflictResolver` SHALL be wired as the `@Primary` `ConflictResolver` bean when `context unit.revision.enabled = true`. When revision is disabled, `AuthorityConflictResolver` SHALL be the primary bean.

#### Scenario: Revision of PROVISIONAL context unit accepted (permissive strategy)

- **GIVEN** the active mutation strategy allows `CONFLICT_RESOLVER` source
- **AND** a conflict with `conflictType = REVISION` against a PROVISIONAL context unit
- **WHEN** `RevisionAwareConflictResolver.resolve()` is called
- **THEN** the resolution SHALL be `REPLACE`

#### Scenario: Revision of CANON context unit rejected

- **GIVEN** a conflict with `conflictType = REVISION` against a CANON context unit
- **WHEN** `RevisionAwareConflictResolver.resolve()` is called
- **THEN** the resolution SHALL be `KEEP_EXISTING`

#### Scenario: Revision of RELIABLE context unit rejected by default

- **GIVEN** `context unit.revision.reliable-revisable = false` (default)
- **AND** a conflict with `conflictType = REVISION` against a RELIABLE context unit
- **WHEN** `RevisionAwareConflictResolver.resolve()` is called
- **THEN** the resolution SHALL delegate to `AuthorityConflictResolver`

#### Scenario: Revision of RELIABLE context unit accepted when configured (permissive strategy)

- **GIVEN** the active mutation strategy allows `CONFLICT_RESOLVER` source
- **AND** `context unit.revision.reliable-revisable = true`
- **AND** a conflict with `conflictType = REVISION` against a RELIABLE context unit with confidence >= `replaceThreshold`
- **WHEN** `RevisionAwareConflictResolver.resolve()` is called
- **THEN** the resolution SHALL be `REPLACE`

#### Scenario: Revision below confidence threshold delegates to contradiction path

- **GIVEN** a conflict with `conflictType = REVISION` against an UNRELIABLE context unit
