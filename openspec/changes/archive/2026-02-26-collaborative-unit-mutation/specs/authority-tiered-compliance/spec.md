...
**Reason**: Revision carveout removed — context unit mutation is now HITL-only via UI. The prompt template SHALL NOT contain `[revisable]` annotations, `reviseFact` tool instructions, or revision exception blocks.
**Migration**: Remove all revision-related conditional blocks from `context units.jinja`. Stop passing `revision_enabled` and `reliable_revisable` template variables.

#### Scenario: No revisable annotations in prompt

- **GIVEN** any configuration
- **WHEN** the prompt template is rendered
- **THEN** the output SHALL NOT contain `[revisable]` annotations on any context unit entry
