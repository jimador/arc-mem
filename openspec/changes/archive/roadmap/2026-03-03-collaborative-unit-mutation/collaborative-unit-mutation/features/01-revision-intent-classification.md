...
1. Cascade logic for dependent context units (F03).
...
5. CANON context unit revision (CANON context units MUST remain exempt from automated revision classification).
...
1. Packages/components: `context unit/` (Conflict record, ConflictResolver, CompositeConflictDetector), `extract/` (UnitPromoter conflict gate)
...
1. User-facing surface: Context Unit panel MAY annotate recently-revised context units differently from newly-created ones.
2. What is shown: Conflict type (REVISION/CONTRADICTION) in context unit lifecycle events.
3. Success signal: When a revision is detected, the context unit panel shows the new context unit replacing the old one.

### Observability Visibility

1. Logs/events/metrics: `ContextUnitLifecycleEvent` MUST include conflict type when supersession is triggered by revision. Logger MUST emit conflict type at INFO level.
...
3. A `RevisionAwareConflictResolver` MUST route REVISION-typed conflicts to `ArcMemEngine.supersede()` when confidence exceeds a configurable threshold.
...
5. CANON context units MUST NOT be revision-eligible regardless of classification.
6. Classification false-positive rate (adversarial input classified as REVISION) SHOULD be below a target defined in R01.
7. The resolver MUST be a drop-in replacement configurable via Spring @Bean — no changes to `UnitPromoter` interface required.
...
1. Problem statement: The conflict detection pipeline treats all contradictions uniformly. When a collaborator says "actually, make that a bard instead of a wizard," the system classifies this identically to an adversarial injection attempt. The DM refuses the change, and the refusal itself reinforces the disputed context unit (rank +50, potential authority upgrade), creating a feedback loop where legitimate revisions become progressively harder. This blocks the most basic collaborative workflow: an actor revising a fact they introduced.
2. Why now: This is the foundational capability for the Collaborative Context Unit Mutation roadmap. F02 and F03 depend on the classification signal.
3. Constraints and non-goals: CANON context units exempt; no ownership/source gating; additive changes only (no breaking API changes); classification accuracy validated before deployment.
4. User-visible and/or observability-visible outcomes: Conflict type in lifecycle events; context unit panel reflects successful revisions; logger emits classification decisions.
...
2. Scenario: When a player sends "Actually, I want Anakin Skywalker to be a bard" and an context unit "Anakin Skywalker is a wizard" exists, the conflict SHALL be classified as REVISION rather than CONTRADICTION.

## Research Findings

| Task ID | Key Finding | Evidence Source | Confidence | Impact on Scope |
|---------|-------------|-----------------|------------|-----------------|
| R00 | Observed: DM refuses player class change; wizard context unit reinforced to UNRELIABLE/650/x3 after 3 turns; refusal creates reinforcement feedback loop. | `openspec/roadmaps/collaborative-unit-mutation/research/R00-chat-mutation-failure-analysis.md` | High | Confirms classification gap is the blocking issue. |
...
3. Integration test: End-to-end chat flow where player revises character class → context unit superseded, new context unit created.
