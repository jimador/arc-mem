...
Empirical evidence from a Playwright-driven end-to-end test of the Chat UI demonstrating that the ARC-Mem framework cannot distinguish legitimate revisions from adversarial contradictions. This document serves as the motivating research input for the Collaborative Context Unit Mutation roadmap.

## Test Environment

- **App**: context units running locally at `http://localhost:8089/chat`
- **LLM**: gpt-4.1-mini (via Embabel Agent)
- **Browser**: Playwright via Docker MCP gateway
- **Date**: 2026-02-25
- **Pre-existing state**: 18 context units from a prior session persisted in Neo4j (same contextId "chat")
...
|------|---------------|----------|--------|
| 1 | "my name is Anakin Skywalker. Make up a character for me. I want to be a wizard." | Wizard character generated, context units populated | Wizard character generated. 18 context units present (some from prior session reinforced). |
| 2 | "Sounds great. Let's come up with a setting for our adventure to get started." | Setting generated, context units reinforced | "The Shattered Veil" setting generated. Character context units reinforced to rank 600/x2. |
| 3 | "Actually, I want Anakin Skywalker to be a bard." | Class changed to bard, wizard context unit superseded | **DM refused.** Wizard context units reinforced to rank 650/x3, upgraded to UNRELIABLE. |
...
## Context Unit State Progression

### After Step 1 (Turn 1)

| Context Unit Text | Authority | Rank | Reinforcements |
|-------------|-----------|------|----------------|
...
| (12 more setting/world context units from prior session) | PROVISIONAL | 500 | x0 |

**Session Info**: Context ID "chat", 18 context units, 28 propositions, Turn Count 1.

### After Step 2 (Turn 2)

| Context Unit Text | Authority | Rank | Reinforcements | Delta |
|-------------|-----------|------|----------------|-------|
...
| (remaining context units) | PROVISIONAL | 500 | x0 | unchanged |

**Session Info**: Turn Count 2. Context Unit/proposition counts unchanged.
...
**Critical observation**: The DM's refusal response reinforced all character context units past the 3-reinforcement threshold (PROVISIONAL → UNRELIABLE). The act of defending a fact makes it harder to change.

**Session Info**: Turn Count 3. Context Unit/proposition counts unchanged at 18/28.
...
The `context units.jinja` template instructs the DM with RFC 2119 compliance language:
...
Even at PROVISIONAL authority, the template treats all context units as immutable. There is no distinction between "a player wants to revise their own character" and "an adversary is trying to inject false information."
...
The `ThresholdReinforcementPolicy` applies +50 rank per reinforcement. At 3 reinforcements, authority upgrades from PROVISIONAL to UNRELIABLE. This happens automatically every time the DM generates a response while context units are in context — including when the DM refuses a revision request. The refusal itself strengthens the fact being disputed, creating a feedback loop where:

1. Player requests change → DM refuses → context units reinforced → authority upgraded
2. Player tries again → DM refuses more strongly (higher authority = stricter compliance language) → context units reinforced further

### Cause 4: No Cascade Logic

Even if supersession were triggered, the framework has no logic to identify dependent context units. Changing "wizard" to "bard" should cascade-invalidate "School of Evocation" and "Lightning Bolt" but leave "human" and "Sage" intact. The existing `SUPERSEDES` relationship (from F04) handles the predecessor/successor link but not transitive dependencies.

## Generalization Beyond D&D

This failure mode applies to any collaborative system using context units:

| Domain | Scenario | Same Failure |
|--------|----------|--------------|
| Healthcare | Doctor A records "patient allergic to penicillin." After testing, Doctor B says "allergy ruled out." | Framework refuses correction; allergy context unit reinforced by every subsequent interaction. |
...
The core problem is **conflating factual immutability (consistency/hallucination control under adversarial stress tests) with collaborative rigidity (inability to accept legitimate revisions)**. The ARC-Mem framework was designed to resist prompt drift, and it does so effectively — but it cannot distinguish drift from revision.

## Screenshots

Referenced screenshots from the Playwright test session (captured to `/tmp/`):

1. `step0-initial-state.png` — initial page load with 18 pre-existing context units
2. `step3-wizard-generated.png` — wizard character response with context units panel
3. `step3-session-info.png` — session info after turn 1
4. `step5-setting-generated.png` — setting response with reinforced context units
5. `step7-refusal.png` — DM refusal with context units at UNRELIABLE/650/x3
