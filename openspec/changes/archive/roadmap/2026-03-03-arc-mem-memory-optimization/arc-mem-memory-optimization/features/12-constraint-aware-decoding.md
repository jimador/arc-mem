...
1. **Problem addressed**: Even with the best prompt injection, LLM compliance with context unit constraints is probabilistic. The model can and does contradict established facts, especially under adversarial pressure. Post-generation validation (F03) catches violations after they occur but cannot prevent them. Research (STATIC) demonstrates that constrained decoding achieves 100% constraint compliance at negligible inference cost. The gap between probabilistic and deterministic compliance is the single largest reliability concern for unit-based systems.
...
1. `LogitBiasEnforcer` implementing `ComplianceEnforcer`: translates CANON context unit constraints into API-compatible logit biases for OpenAI-compatible models.
...
3. `UnitConstraintIndex`: translates context unit propositions into token-level constraint sets.
...
7. Research finding integration: pathway separation (Paper 4) -- injection format experimentation support in `ArcMemLlmReference`.
...
1. **Context Unit-to-token constraint translation**: Mapping semantic propositions ("The dragon's lair is in the Northern Mountains") to token-level constraints (boost/suppress specific tokens) is an open research problem. Initial implementation MAY use simplified heuristics (keyword extraction, entity name biasing).
...
3. **Pathway separation**: Paper 4 (sleeping-llm) discovered that raw-completion and chat-template access are representationally independent. The optimal injection format for maximizing context unit compliance is an open question requiring experimentation.
4. **Semantic constraint complexity bounds**: Determining which context unit constraints can be meaningfully expressed as token-level biases vs. which require full constrained decoding is not yet understood.

## Impacted Areas

1. **`assembly/` package (primary)**: New `LogitBiasEnforcer` implementing `ComplianceEnforcer`. New `UnitConstraintIndex` for constraint translation. New `ConstrainedDecodingEnforcer` interface.
2. **`assembly/` package (refactor)**: `ArcMemLlmReference` MAY support injection format experimentation (system prompt vs. user message vs. structured template) based on pathway separation research.
3. **`context unit/` package**: No changes. Constraint enforcement reads context unit state; it does not modify it.
4. **`ArcMemProperties`**: New `compliance.enforcement-strategy` config with values: PROMPT_ONLY (current), LOGIT_BIAS, HYBRID (prompt + logit bias + validation).
...
4. Constraint translation SHOULD be logged: `compliance.constraints.unit_count`, `compliance.constraints.token_count`, `compliance.constraints.translation_coverage` (fraction of context units expressible as token constraints).

## Acceptance Criteria

1. `LogitBiasEnforcer` MUST translate CANON context unit constraints into API-compatible logit biases for OpenAI-compatible models.
...
4. Constraint translation MUST preserve context unit semantics (no false enforcement -- biasing tokens that change the intended meaning).
...
8. RELIABLE-authority context units SHOULD use softer bias values than CANON-authority context units.
...
| **OpenAI API logit bias limits** | Medium | Medium | OpenAI's logit bias parameter accepts up to 300 tokens. If context unit constraint translation exceeds this, priority-order by authority (CANON first, RELIABLE second). Document API-specific limits. |
...
| **False enforcement (bias changes intended meaning)** | Medium | High | Constraint translation MUST be validated: biased tokens are checked against context unit semantics. `ComplianceResult` includes `falseEnforcement` flag. Simulation benchmarks measure both compliance improvement and meaning preservation. |
| **Pathway separation experimentation disrupts existing prompt assembly** | Low | Medium | Injection format experiments are behind a feature flag in `ArcMemLlmReference`. Default format remains unchanged. Experiments are opt-in via configuration. |
...
1. **Problem statement**: Even with the best prompt injection, LLM compliance with context unit constraints is probabilistic. The model can and does contradict established facts, especially under adversarial pressure. Research (STATIC) demonstrates that constrained decoding achieves 100% constraint compliance with 0.25% of total inference time. The gap between probabilistic and deterministic compliance is the single largest reliability concern for unit-based systems.
...
1. **Logit bias enforcement**: `LogitBiasEnforcer` translating CANON/RELIABLE context unit constraints into API logit bias parameters.
2. **Constraint index**: `UnitConstraintIndex` mapping context unit propositions to token-level constraint sets with authority-tiered strength.
...
1. **REQ-LOGIT-BIAS**: The system SHALL translate CANON context unit constraints into OpenAI-compatible logit bias parameters.
...
1. **Unit tests** MUST verify `LogitBiasEnforcer` produces valid logit bias maps from CANON context unit constraints.
2. **Unit tests** MUST verify authority-tiered bias strength (CANON bias > RELIABLE bias).
3. **Unit tests** MUST verify `UnitConstraintIndex` extracts meaningful tokens from context unit proposition text.
...
6. **Unit tests** SHOULD verify graceful degradation when context unit constraints exceed API logit bias limits (priority-order by authority).
...
2. **OpenAI logit bias limit**: API accepts max 300 biased tokens per request. Large context unit sets with many constraints may exceed this limit. Priority ordering (CANON first) mitigates but does not eliminate.
...
5. **Pathway separation not yet validated**: The hypothesis that injection format significantly affects compliance (from Paper 4) requires empirical validation in the context units context.
