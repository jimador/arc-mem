...
Add novelty and importance scoring axes to proposition evaluation, inspired by sleeping-llm's curator (Amygdala) fact scoring. These axes complement the existing `TrustPipeline` signals to filter low-value propositions before they enter the promotion pipeline, improving budget efficiency and signal-to-noise ratio of the active context unit set.
...
1. Problem addressed: context units' `DuplicateDetector` is binary (duplicate or not), and `TrustPipeline` signals focus on reliability rather than value. A proposition can be non-duplicate, high-confidence, and well-sourced but still low-value -- a trivial elaboration of existing knowledge. As context unit budgets fill with organic propositions (via scene-setting and warm-up turns), distinguishing valuable from trivial propositions becomes critical for budget efficiency.
...
3. Why now: Wave 3 -- no hard dependencies. As context unit sets grow and mature through Waves 1-2 improvements, the volume of organic propositions increases, making quality-based filtering increasingly important.

## Scope

### In Scope

1. Define a novelty score [0.0, 1.0] per incoming proposition: how much new information does this proposition add relative to existing context units?
...
7. **Prolog relationship inference (scoring mechanism option)**: A `PrologRelationshipScorer` MAY derive proposition relationships (entailment, logical support, subsumption) via DICE's tuProlog projection. Prolog rules can detect logical support/contradiction patterns that pure embedding similarity misses — e.g., "X is a dragon" + "X breathes fire" entails support via domain rules, which text similarity would not capture. This contributes to importance scoring by identifying propositions that logically extend or support existing context units. tuProlog (2p-kt) is already on classpath via DICE 0.1.0-SNAPSHOT; zero new dependencies REQUIRED.
...
1. Utility scoring (sleeping-llm's third axis -- procedural knowledge detection). This is less relevant to context units' narrative domain.
...
4. Retroactive scoring of already-promoted context units (proactive maintenance cycle F07 handles post-promotion quality assessment).

## Dependencies

1. Feature dependencies: None.
2. Technical prerequisites: `TrustPipeline`, `TrustSignal` interface, `DomainProfile`, `UnitPromoter`, `DuplicateDetector`.
3. Priority: SHOULD.
...
1. Packages/components: `context unit/` (new `TrustSignal` implementations or pre-trust filter), `extract/` (`UnitPromoter` integration), `ArcMemProperties` (scoring configuration, thresholds).
...
4. DICE Prolog integration (optional): A `PrologRelationshipScorer` MAY contribute to importance scoring by projecting propositions and context units as Prolog facts and querying for logical relationships (entailment, support, subsumption) via `PrologEngine`. Lives in `context unit/` alongside other scoring components.
...
3. Success signal: Promotion pipeline produces a higher-quality context unit set with fewer trivial propositions.
...
3. Risk: Scoring adds latency to the promotion pipeline.
   Mitigation: Heuristic scoring is O(N * M) where N = candidates, M = active context units (text comparison). This is negligible compared to existing LLM-based conflict detection.
...
1. Problem statement: context units' duplicate detector is binary (duplicate or not), and TrustPipeline signals focus on reliability rather than value. A proposition can be non-duplicate, high-confidence, and well-sourced but still low-value (trivial elaboration of existing knowledge). Research (sleeping-llm curator) shows that fact-level quality scoring (novelty, importance, utility) significantly improves the signal-to-noise ratio of injected knowledge.
2. Why now: As context unit budgets fill with organic propositions (via scene-setting and warm-up turns), distinguishing valuable from trivial propositions becomes critical for budget efficiency.
...
1. Novelty scoring (information gain relative to existing context unit set).
...
2. Scenario: When a proposition "The tavern has wooden furniture" is evaluated against existing context units including "The tavern is a cozy establishment with oak tables and chairs," the novelty score SHALL be low (< 0.3) and the proposition SHOULD be filtered.
3. Scenario: When a proposition "The dragon's name is Aldrathar" is evaluated with no existing context units mentioning dragons, the novelty score SHALL be high (> 0.7) and the proposition SHALL proceed to trust evaluation.

## Research Findings

| Task ID | Key Finding | Evidence Source | Confidence | Impact on Scope |
|---------|-------------|-----------------|------------|-----------------|
| Rec B | Sleeping-llm curator scores facts by novelty (message length, question marks, technical markers) and importance (correction markers, emphasis). Combined score = arithmetic mean; threshold filters low-quality exchanges. | `openspec/research/llm-optimization-external-research.md` sec 3.1 | High | Validates two-axis scoring design. context units adaptation uses semantic similarity rather than surface heuristics. |
...
1. Unit tests: Novelty scoring with known context unit sets and predictable proposition overlaps. Importance scoring with known conversation context. Threshold filtering with edge cases (scores at boundary).
...
4. Quality scoring is computed at promotion time. Already-promoted context units are not retroactively scored (use F07 proactive maintenance for post-promotion quality assessment).
