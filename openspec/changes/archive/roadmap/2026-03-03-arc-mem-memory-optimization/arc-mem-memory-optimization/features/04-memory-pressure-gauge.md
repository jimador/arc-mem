...
1. **Problem addressed**: context units has no quantitative measure of context unit system health. Budget enforcement is binary (over/under), decay is reactive, and there is no early warning for degraded states. A maintenance sweep triggered too late finds irrecoverable damage; one triggered too early wastes resources.
...
3. Configurable weights per dimension (via `ArcMemProperties`).
...
6. Event-driven updates: pressure recalculated on `ContextUnitLifecycleEvent` emissions.
...
|-----------|----------|-------|
| **AttentionWindow** | Existing `context unit/` package | Sliding-window event rate computation with burst factor. SHOULD be reused for conflict pressure and decay pressure dimensions. |
...
1. **`context unit/` package (primary)**: New types -- `MemoryPressureGauge` (service), `PressureScore` (record with total + per-dimension breakdown), `PressureThreshold` (record with light/full sweep levels), `PressureDimension` (enum: BUDGET, CONFLICT, DECAY, COMPACTION).
2. **`context unit/event/` package**: New event types -- `PressureThresholdBreached` extending `ContextUnitLifecycleEvent`.
3. **`ArcMemProperties`**: New `pressure` config section with weights, thresholds, and exponents.
...
4. Weights and thresholds MUST be configurable via `ArcMemProperties`.
...
| **Performance at scale** | Low | Low | Pressure computation is O(1) per dimension (counts and ratios). No concern at current unit budget sizes (20). |
...
1. **Problem statement**: context units has no quantitative measure of context unit system health. Budget enforcement is binary (over/under), decay is reactive, and there is no early warning for degraded states. Research (sleeping-llm) shows that a composite drowsiness signal effectively triggers maintenance before quality degrades.
...
3. **REQ-CONFIG**: Pressure weights, thresholds, and exponents SHALL be configurable via `ArcMemProperties`.
...
1. **Unit tests** MUST verify pressure computation against known inputs (e.g., 15 context units with budget 20 at weight 0.4 = known budget pressure contribution).
...
1. **Weight calibration is empirical.** Default weights are informed by sleeping-llm ratios but not validated for the context unit domain. Simulation-based calibration is expected post-implementation.
