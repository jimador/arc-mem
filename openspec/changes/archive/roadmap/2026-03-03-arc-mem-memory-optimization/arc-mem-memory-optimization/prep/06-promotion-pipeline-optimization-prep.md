...
1. **Pre-extraction conflict check**: New gate inserted between confidence and dedup in the `UnitPromoter` pipeline. Uses `ConflictIndex.getConflicts()` (O(1)) to filter propositions conflicting with RELIABLE+ context units before spending LLM calls on dedup and trust evaluation.
...
| 4 | **Pre-check authority threshold** | (a) Filter against RELIABLE+ context units only (RELIABLE and CANON). (b) Filter against all active context units. (c) Configurable threshold. | (a) RELIABLE+ only. PROVISIONAL and UNRELIABLE context units are not stable enough to gate new propositions. Filtering against low-authority context units would be overly conservative. | Design phase. |
...
|------|-------------|---------|
| Pipeline throughput improved | Integration test: same scenario before/after optimization. Measure LLM call count and total promotion duration. After MUST have fewer LLM calls. | `./mvnw test -pl . -Dtest=ContextUnitPromoterOptimizationTest` |
| No change to promotion outcomes | Unit test: same input propositions + same context unit state -> same promoted set with optimized and unoptimized pipelines. | `./mvnw test -pl . -Dtest=UnitPromoterTest` |
| Fixed-size batches work with padding | Unit test: 3 real candidates + 7 padding in a batch of 10 -> result contains exactly 3 real outcomes. | `./mvnw test -pl . -Dtest=UnitPromoterTest` |
| Graceful fallback on empty index | Unit test: ConflictIndex is empty -> pre-check gate skipped, pipeline operates in original order. | `./mvnw test -pl . -Dtest=UnitPromoterTest` |
...
- **Scope boundary**: `extract/` package primarily; `context unit/` for ConflictIndex consumption

## Task Sketch

| # | Scope | Files | Gate |
|---|-------|-------|------|
| T1 | Pre-check gate in `UnitPromoter` using `ConflictIndex` + gate reordering | `UnitPromoter.java` (refactor), `UnitPromoterTest.java` (update) | Pre-check filters conflicting propositions; pipeline outcome unchanged for non-conflicting |
| T2 | Fixed-size batch padding with `NoOpProposition` sentinel | `BatchPaddingUtil.java`, `UnitPromoter.java` (update), `BatchPaddingUtilTest.java` | Batches are fixed-size; padding filtered from results |
| T3 | Observability: per-gate timing + early-rejection logging + batch utilization | `UnitPromoter.java` (update), observability test | Structured logs emitted with timing and rejection metrics |

## Risks Requiring Design Attention

1. **UnitPromoter refactor scope**: Reordering gates in `UnitPromoter` touches the core promotion logic. The refactor MUST be tested against all existing promotion test cases to ensure no behavioral change.
...
3. **ConflictIndex dependency injection**: `UnitPromoter` gains a new dependency on `ConflictIndex`. If the index bean is not available (e.g., INDEXED strategy not configured), the pre-check gate MUST be skipped gracefully. Use `Optional<ConflictIndex>` injection.
