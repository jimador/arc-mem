...
1. **Neo4j-backed storage**: Conflict adjacency stored as `CONFLICTS_WITH` relationships between `PropositionNode` entities in Neo4j. Properties: `confidence`, `reason`, `detectedAt`. Leverages existing Drivine ORM and `ContextUnitRepository` patterns. No new dependency needed.
2. **Incremental maintenance**: Index updates on individual `ContextUnitLifecycleEvent` emissions. No full rebuild. On Promoted: compute conflicts between new context unit and all existing context units, create edges. On Archived/Evicted: delete all edges involving the context unit.
3. **Fast lookup**: `getConflicts(String unitId)` queries `MATCH (a)-[:CONFLICTS_WITH]->(b) WHERE a.id = $unitId`. No LLM call at lookup time.
4. **Lifecycle-event-driven updates**: `ConflictIndex` subscribes to `ContextUnitLifecycleEvent` (Promoted, Archived, AuthorityChanged). Updates are synchronous within the turn pipeline, ensuring the index is current by the next turn's promotion cycle.
5. **ConflictEntry stacked layout**: Each entry bundles context unit ID, text, authority, conflict type, confidence — all fields needed for resolution in a single object. No secondary lookups required (recommendation I).
...
| 2 | **Cold-start population strategy** | (a) Eager: on first promotion in a new context, batch-detect all existing context unit pairs. (b) Lazy: populate incrementally as promotions occur (new context unit vs. all existing). (c) Background: populate asynchronously after context creation. | (b) Lazy. Each promotion adds edges for the new context unit only (N-1 LLM calls for N existing context units). Amortized over promotions. Eager (a) is O(N^2) upfront; background (c) adds async complexity. | Design phase. Profile cold-start cost in simulation. |
...
|---------|------|------|--------|
| Structured logs | `conflict.index.update` | Each lifecycle event that modifies the index | INFO: event type, context units affected, edges added/removed |
...
| Index updates on lifecycle events | Unit test: promote context unit -> index gains edges. Archive context unit -> edges removed. | `./mvnw test -pl . -Dtest=InMemoryConflictIndexTest` |
...
| ConflictEntry has all resolution fields | Unit test: entry contains context unit ID, text, authority, conflict type, confidence. | `./mvnw test -pl . -Dtest=InMemoryConflictIndexTest` |
...
- **New Neo4j relationship type**: `CONFLICTS_WITH` between `PropositionNode` entities (new Drivine queries in `ContextUnitRepository`)
- **Scope boundary**: `context unit/` package for index types; `extract/` integration is a separate task
...
| T2 | `ContextUnitRepository` conflict edge queries (create, delete, lookup) | `ContextUnitRepository.java` (update), test | CONFLICTS_WITH relationships CRUD via Cypher |
...
1. **LlmConflictDetector batch API**: Cold-start population needs to call `LlmConflictDetector.batchDetect()` for new-unit-vs-all-existing pairs. Verify the batch API supports this usage pattern (currently used by `UnitPromoter` for candidate-vs-existing).
...
3. **Drivine relationship mapping**: Verify that Drivine supports creating/querying relationships with properties (`confidence`, `reason`, `detectedAt`) between existing `PropositionNode` entities. The existing `CANONIZATION_REQUEST_FOR` relationship pattern in `ContextUnitRepository` is a good reference.
