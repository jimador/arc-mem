...
2. **PostGenerationValidator as first implementation**: Validates agent response text against active context units (authority-filtered) using an LLM call. Returns violations with context unit ID, description, and suggested action.
3. **Authority-based strictness**: CANON context units MUST be enforced. RELIABLE context units SHOULD be enforced (configurable). PROVISIONAL and UNRELIABLE context units are NOT enforced by default.
4. **PromptInjectionEnforcer preserves current behavior**: Wraps the existing `ArcMemLlmReference` prompt injection. Always returns `ComplianceResult(compliant=true, violations=[], action=ACCEPT)`. This is the default strategy -- zero behavioral change.
5. **ComplianceResult is actionable**: Includes `compliant` boolean, `violations` list (each with context unit ID + description), and `suggestedAction` enum (`ACCEPT`, `RETRY`, `REJECT`). Callers decide how to act.
...
| Structured logs | `compliance.check` | Each enforcement call | INFO: strategy, context unit count, result, duration |
| Structured logs | `compliance.violation` | Each detected violation | WARN: context unit ID, authority, violation description, suggested action |
...
|------|-------------|---------|
| PostGenerationValidator detects CANON contradictions | Unit test: response text "The guardian is a wizard" against CANON context unit "The guardian is a warrior" -> violation detected. | `./mvnw test -pl . -Dtest=PostGenerationValidatorTest` |
...
| Authority-based strictness configurable | Unit test: with `compliance.strictness.reliable=false`, RELIABLE context units are excluded from validation. | `./mvnw test -pl . -Dtest=PostGenerationValidatorTest` |
...
- **No changes to ArcMemLlmReference**: Prompt assembly logic is untouched
...
| T3 | `PostGenerationValidator` with authority-filtered context unit validation | `PostGenerationValidator.java`, `PostGenerationValidatorTest.java` | Detects CANON contradictions |
| T4 | `ArcMemProperties` compliance config + Spring wiring | `ArcMemProperties.java` (update), compliance config section | Strategy selectable, strictness configurable |
...
- Authority hierarchy constraints (e.g., CANON context units MUST NOT be contradicted by PROVISIONAL propositions)
...
- Budget limit enforcement (active context unit count MUST NOT exceed configured maximum)
...
2. **LLM call cost**: Every enforced response adds one LLM call. For simulation runs with 20 turns, this is 20 additional calls. Ensure the validation prompt is efficient (single call, not per-context unit). A future `PrologInvariantEnforcer` (Wave 2-3) would eliminate LLM cost for rule-expressible invariants.
