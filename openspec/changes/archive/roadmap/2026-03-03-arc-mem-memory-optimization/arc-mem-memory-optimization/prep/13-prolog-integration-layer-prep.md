...
Feature doc: `openspec/roadmaps/unit-memory-optimization/features/13-prolog-integration-layer.md`
Research: `openspec/roadmaps/unit-memory-optimization-roadmap.md` (cross-cutting Prolog strategy table), `openspec/research/llm-optimization-external-research.md` (rec A)
...
3. **Proposition-to-Prolog-fact projection as shared foundation**: `ContextUnitPrologProjector` is the single entry point for all unit-to-Prolog conversions. All three Prolog implementations consume this projector -- no independent projection logic.
...
5. **Sub-second query latency**: All Prolog queries MUST complete in < 100ms for context unit sets up to budget max (default 20). This is a hard performance gate, not a guideline.
...
### Q1: Prolog Fact Schema for Context Units

**Question**: What Prolog fact schema best represents context units for contradiction detection, invariant checking, and audit pre-filtering?

**Decision space**:
- (a) Flat fact per context unit: `context unit(Id, Text, Authority, Rank, Pinned)`. Simple but text is opaque to Prolog reasoning.
- (b) Entity-triple decomposition: `context unit(Id, Authority, Rank, Pinned)` + `entity(Id, Subject, Predicate, Object)` extracted from context unit text. Enables relational reasoning but requires text decomposition.
- (c) Hybrid: flat context unit facts for metadata queries + entity triples for semantic queries. Separate fact families for different query types.
- (d) Token-level: `unit_token(Id, Token, Position)`. Maximum granularity but high fact count and limited reasoning value.

**Recommendation**: Option (c) -- hybrid schema. Flat `context unit/5` facts carry metadata (authority, rank, pinned). Entity triples `claim(Id, Subject, Predicate, Object)` carry semantics. Contradiction rules operate on `claim/4`; invariant rules operate on `context unit/5`. This separates concerns cleanly.
...
**Question**: What Prolog rule patterns are most effective for detecting contradictions among projected context unit facts?
...
**Question**: How to handle Prolog projection for context units with complex, multi-clause text?

**Decision space**:
- (a) First-sentence heuristic: extract entity triple from the first clause only. Fast, lossy.
- (b) Multi-claim extraction: decompose multi-clause text into multiple `claim/4` facts per context unit. More complete but higher fact count.
...
1. **Task 1: ContextUnitPrologProjector foundation** (3 files)
   - Create `ContextUnitPrologProjector` in `context unit/` package.
   - Implement hybrid fact schema: `context unit/5` for metadata, `claim/4` for entity triples.
   - Heuristic text decomposition into subject-predicate-object triples.
   - Wire as Spring `@Service`.
   - Files: `ContextUnitPrologProjector.java`, `PrologFactSchema.java` (constants/helpers), test.
...
2. **Task 2: Prolog rule definitions** (3 files)
   ...
   - Package rules so they are loadable by `ContextUnitPrologProjector`.
   - Files: Prolog rule resource files, `PrologRuleLoader.java`, test.

3. **Task 3: LOGICAL conflict detection strategy** (4 files)
   - Implement `PrologConflictDetector` that uses `ContextUnitPrologProjector` to project context units and query for contradictions.
   ...
   - Files: `PrologConflictDetector.java`, `CompositeConflictDetector.java` (modify LOGICAL branch), `ArcMemConfiguration.java` (wire detector), test.

4. **Task 4: Prolog audit pre-filter** (4 files)
   - Implement `PrologAuditPreFilter` that projects active context units and queries for contradiction chains and unsupported claims.
   - Integrate with `ProactiveMaintenanceStrategy` audit step -- context units flagged by Prolog get score 0.0.
   - Toggleable via configuration property.
   - Files: `PrologAuditPreFilter.java`, `ProactiveMaintenanceStrategy.java` (pre-filter integration), `ArcMemProperties.java` (toggle property), test.

5. **Task 5: Prolog invariant enforcer** (4 files)
   - Implement `PrologInvariantEnforcer` as a `ComplianceEnforcer`.
   - Project active context units to Prolog facts, load invariant rules, query for violations.
   - Return `ComplianceResult` with violation details.
   - Files: `PrologInvariantEnforcer.java`, `ArcMemConfiguration.java` or assembly config (wire as bean), `ArcMemProperties.java` (enforcer selection), test.
...
6. **Task 6: A/B testability wiring** (5 files)
   ...
   - Files: scenario YAML schema update, `ScenarioLoader.java`, `SimulationRunContext.java`, `ArcMemConfiguration.java` or strategy factory, test.

7. **Task 7: Tests and verification** (4 files)
   - A/B comparison tests: same context unit set processed by Prolog vs. non-Prolog strategies.
   - Performance tests: query latency < 100ms for 20-context unit sets.
   - Integration test: full simulation run with LOGICAL strategy enabled.
   - Files: `PrologConflictDetectorTest.java`, `PrologAuditPreFilterTest.java`, `PrologInvariantEnforcerTest.java`, `ContextUnitPrologProjectorTest.java`.
...
3. **Performance**: All Prolog queries MUST complete in < 100ms for 20-context unit sets.
4. **Projection works**: `ContextUnitPrologProjector` MUST successfully project any valid context unit set to Prolog facts and return results via `PrologEngine` queries.
...
- `ContextUnitPrologProjector` (new, shared projection foundation)
...
- `ArcMemProperties` (existing, for configuration)
