# Prep: Tiered Context Unit Storage

**Feature**: F11 (`tiered-unit-storage`)
**Wave**: 4
**Priority**: MAY
...
HOT-tier context units MUST be served from an in-memory cache with O(1) lookup by context unit ID. The cache is populated on context initialization and maintained via lifecycle event invalidation. There is no database round-trip for HOT context unit reads during prompt assembly.

### L2: WARM = Standard Neo4j Queries

WARM-tier context units MUST be served from standard `ContextUnitRepository` Neo4j queries. No caching, no special treatment. This is the current behavior for all context units, preserved for the WARM tier.

### L3: COLD = Lazy-Loaded, Excluded from Prompt Assembly

COLD-tier context units MUST NOT be included in standard prompt assembly. They are loaded only during maintenance sweeps (F07), audit operations, or explicit retrieval requests. Standard `ArcMemLlmReference` assembly skips COLD context units entirely.

### L4: Event-Driven Cache Invalidation

HOT cache MUST be invalidated by `ContextUnitLifecycleEvent`. The following events MUST trigger cache invalidation for the affected context unit:
...
**Context**: HOT tier contains context units above the hot-threshold rank. Typical context has 15-20 active context units; HOT tier is roughly the top 5-10 by rank. Each cached context unit is lightweight (ID, text, rank, authority, metadata -- roughly 1-2 KB).

**Candidates**:
- **Unbounded within tier**: Cache all HOT context units. Size is naturally bounded by unit budget (max ~20-50 per context). With 100 concurrent contexts, that is ~5000 entries, ~10 MB. Minimal memory concern.
...
**Recommendation**: Unbounded within tier is RECOMMENDED for the initial implementation. The natural bound (unit budget * context count) is small enough that explicit size limits add complexity without meaningful benefit.

### O3: COLD Compression Format

**Question**: Should COLD context units be stored in a compressed representation?

**Context**: Initial implementation excludes COLD context units from loading. Actual compression (storing a summarized or reduced representation) is a future optimization.

**Decision**: COLD compression is OUT OF SCOPE for initial implementation. COLD context units exist in Neo4j with full fidelity; they are simply not loaded during standard prompt assembly. Compression MAY be added as a follow-up optimization.
...
- **Lazy migration**: On next prompt assembly for an existing context, populate HOT cache from current active context units. Seamless but adds one-time latency.
...
### Task 1: ContextUnitCache Component

**Files** (3):
1. `src/main/java/dev/dunnam/arcmem/persistence/ContextUnitCache.java` -- new class
2. `src/main/java/dev/dunnam/arcmem/ArcMemProperties.java` -- add `tiered-storage` config section
3. `src/test/java/dev/dunnam/arcmem/persistence/ContextUnitCacheTest.java` -- new test

**Work**:
1. Implement `ContextUnitCache` with `get(unitId) -> Optional<Context Unit>`, `put(Context Unit)`, `invalidate(unitId)`, `invalidateAll(contextId)`, `isPresent(unitId) -> boolean`.
...
### Task 2: TieredContextUnitRepository Decorator

**Files** (4):
1. `src/main/java/dev/dunnam/arcmem/persistence/TieredContextUnitRepository.java` -- new decorator
2. `src/main/java/dev/dunnam/arcmem/persistence/ContextUnitCache.java` -- minor updates if needed
3. `src/main/java/dev/dunnam/arcmem/context unit/MemoryTier.java` -- verify tier classification (read-only)
4. `src/test/java/dev/dunnam/arcmem/persistence/TieredContextUnitRepositoryTest.java` -- new test

**Work**:
1. Implement `TieredContextUnitRepository` wrapping `ContextUnitRepository`.
2. Route `findActiveUnitsByContext()` to: HOT from cache, WARM from Neo4j, COLD excluded.
...
**Files** (4):
1. `src/main/java/dev/dunnam/arcmem/persistence/ContextUnitCacheInvalidator.java` -- new event listener
2. `src/main/java/dev/dunnam/arcmem/persistence/ContextUnitCache.java` -- invalidation method enhancements
3. `src/main/java/dev/dunnam/arcmem/persistence/TieredContextUnitRepository.java` -- tier transition handling
4. `src/test/java/dev/dunnam/arcmem/persistence/ContextUnitCacheInvalidatorTest.java` -- new test

**Work**:
1. Create `ContextUnitCacheInvalidator` as a Spring `@EventListener` for `ContextUnitLifecycleEvent`.
2. On rank change: check if tier changed; if context unit moved from WARM to HOT, add to cache; if HOT to WARM, remove from cache.
...
**Files** (4):
1. `src/main/java/dev/dunnam/arcmem/assembly/ArcMemLlmReference.java` -- tier-aware query
2. `src/main/java/dev/dunnam/arcmem/assembly/PromptBudgetEnforcer.java` -- tier-aware budget
3. `src/main/java/dev/dunnam/arcmem/assembly/CompactedContextProvider.java` -- tier-aware compaction (COLD excluded)
4. `src/test/java/dev/dunnam/arcmem/assembly/ArcMemLlmReferenceTest.java` -- update tests

**Work**:
1. Modify `ArcMemLlmReference` to query `TieredContextUnitRepository` when tiered storage is enabled; fall back to `ContextUnitRepository` when disabled.
2. HOT context units served from cache (fast path). WARM context units from Neo4j. COLD excluded.
...
**Files** (4):
1. `src/main/java/dev/dunnam/arcmem/persistence/TieredContextUnitRepository.java` -- add metrics logging
2. `src/main/java/dev/dunnam/arcmem/persistence/ContextUnitCache.java` -- add hit/miss counters
3. `src/main/resources/application.yml` -- add tiered-storage defaults
4. `src/test/java/dev/dunnam/arcmem/persistence/TieredContextUnitRepositoryTest.java` -- fallback regression test

**Work**:
1. Add structured logging: cache hit/miss rates, tier distribution, load latency per tier.
2. Add cache invalidation logging: reason, context unit ID.
...
- `ContextUnitCache` compiles and passes unit tests.
...
- `TieredContextUnitRepository` routes queries by tier.
- COLD context units are NOT returned by standard `findActiveUnitsByContext()` when tiered storage is enabled.
- COLD context units ARE returned by `findAllTiersForContext()` for maintenance access.
- `ArcMemLlmReference` does not include COLD context units in prompt assembly.

### Gate 3: Cache Coherence on Events (after Task 3)

- Every `ContextUnitLifecycleEvent` type correctly updates or invalidates cached context units.
