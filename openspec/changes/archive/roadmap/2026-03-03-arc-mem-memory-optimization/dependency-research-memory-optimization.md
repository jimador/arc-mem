# Dependency Research: Context Unit Memory Optimization Roadmap

**Date**: 2026-03-03
**Context**: Evaluating 3rd-party libraries needed for the 11 active features in the unit-memory-optimization roadmap.
...
**Decision: Use Neo4j relationships for conflict adjacency.** The project already models all context unit state as Neo4j graph data via Drivine. Conflict relationships are a natural graph edge:
...
- Leverages existing Drivine ORM and `ContextUnitRepository` patterns
- Queryable via Cypher: `MATCH (a)-[:CONFLICTS_WITH]->(b) WHERE a.id = $unitId RETURN b`
...
**Neo4j vector indexes** are already configured and used in `ContextUnitRepository`:
...
For **novelty scoring**: Query semantic search. High similarity to existing context units = low novelty. The inverse of max similarity score is a novelty signal. No new dependency.
...
**Dependencies needed: none.** Template changes in `ArcMemLlmReference` and `PromptBudgetEnforcer`. Pure domain logic.
...
Provides `DBSCANClusterer` for density-based clustering of context unit embeddings:
```java
var clusterer = new DBSCANClusterer<>(epsilon, minPts);
List<Cluster<DoublePoint>> clusters = clusterer.cluster(points);
```

DBSCAN is ideal for interference density because it finds dense regions without requiring a predefined cluster count, handles noise (isolated context units) naturally.
...
### F11: Tiered Context Unit Storage
...
- O(1) lookup by key (`Cache<String, Context Unit>`)
...
- Spring Cache abstraction (`@Cacheable`) — F11 needs structural caching (a `TieredContextUnitRepository` decorator), not method-level annotation caching. Caffeine is used programmatically.
...
| Current usage in context units | **None** — zero Prolog usage today |

**Why this matters for the roadmap:** Prolog's backward chaining and logical inference are a natural fit for several context unit operations that currently require LLM calls or hand-coded Java logic:

| Feature | Prolog Application | Current Approach | Prolog Advantage |
|---------|--------------------|------------------|-----------------|
| F03 (Compliance) | Express context unit invariants as Prolog rules; query for violations deterministically | `InvariantEvaluator` with hand-coded Java checks | Declarative rules, trivially extensible, zero LLM cost |
...
| F07 (Proactive Maintenance) | Audit context unit consistency via Prolog queries against the full proposition set | LLM-batched relevance audit | Fast, deterministic consistency checks as a pre-filter before LLM audit |
...
**Approach:**
1. Project context unit propositions to Prolog facts (text, authority, rank, conflict relationships).
...
**Risk:** tuProlog 2p-kt is experimental in DICE. If the projection API changes, rules need updating. Mitigation: wrap Prolog access behind a context units interface (`PrologReasoner` or similar) so the projection mechanism is swappable.

### Timefold Solver

**Status: Not recommended for current scope. Premature for ~20 context unit problem size.**
...
Timefold is a constraint satisfaction/optimization solver (fork of OptaPlanner). It excels at multi-objective optimization with complex constraints — e.g., "maximize context unit coverage while minimizing conflict density and staying within token budget."

**Why not now:** The current unit budget (~20 active context units) is small enough that greedy algorithms (sort by rank, evict lowest) are near-optimal. Timefold's value emerges when:
- Budget optimization becomes multi-dimensional (coverage + conflict density + token cost + diversity)
- Context Unit counts grow beyond where greedy heuristics are sufficient
...
```xml
<!-- F11: HOT tier context unit cache (version managed by Spring Boot 3.5.10 = 3.2.3) -->
<dependency>
    <groupId>com.github.ben-manes.caffeine</groupId>
    <artifactId>caffeine</artifactId>
</dependency>
...
| Timefold Solver | Premature — greedy algorithms near-optimal for ~20 context units | F09, F10 |
