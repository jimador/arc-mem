...
Feature doc: `openspec/roadmaps/anchor-memory-optimization/features/07-proactive-maintenance-cycle.md`
...
2. **Transaction-wrapped**: The entire cycle MUST run inside an anchor transaction (F01). Validation failure in step 5 triggers automatic rollback of all changes from steps 2-4.
...
6. **CANON/pinned immunity**: CANON anchors and pinned anchors MUST NOT be pruned during any cycle step (invariants A3b, A4).
...
**Question**: What prompt structure yields reliable relevance scoring for a batch of anchors against recent conversation context in a single LLM call?

**Decision space**:
- (a) Single-call batch: pass all active anchors + recent N turns of conversation; ask for per-anchor relevance score [0.0, 1.0].
- (b) Two-phase: lightweight heuristic pre-filter (entity overlap, recency) followed by LLM call for borderline anchors only.
...
- (d) Prolog pre-filter + LLM: project active anchors to Prolog facts via DICE's `PrologEngine`, run deterministic queries to identify contradiction chains and unsupported claims. Anchors flagged by Prolog are marked as logically inconsistent (scored 0.0) without LLM evaluation. Remaining anchors proceed to a batched LLM relevance call.

**Recommendation**: Option (d) for full sweeps (pressure >= 0.8) -- Prolog pre-filter reduces the LLM batch size by eliminating logically inconsistent anchors deterministically. Option (b) for light sweeps (pressure >= 0.4), augmented with the Prolog pre-filter where available. Pure heuristic (c) as fallback when both LLM and Prolog are unavailable.

**Constraints**: Audit MUST complete in a single batched call where LLM is used. The prompt MUST include anchor text, authority, and rank alongside conversation context. Prolog pre-filter SHOULD run before the LLM call to reduce batch size. Prolog projection uses DICE 0.1.0-SNAPSHOT's tuProlog (2p-kt) -- already on the classpath with zero new dependencies.

### Q2: Consolidation Candidacy Criteria

**Question**: What criteria identify RELIABLE anchors ready for CANON nomination?
...
**Constraints**: Candidacy criteria MUST be configurable via `DiceAnchorsProperties`. CANON assignment still requires `CanonizationGate` approval.
...
- (b) Relative threshold (e.g., bottom 20% of anchors by audit score).
...
**Recommendation**: Option (d) -- two-tier prevents death spiral by only aggressively pruning anchors that are genuinely irrelevant (hard floor), while the soft floor responds to budget pressure. The death spiral risk from sleeping-llm Paper 5 occurs when pruning itself triggers more degradation; the hard floor being very low (0.1) mitigates this.

**Constraints**: Thresholds MUST be configurable. Pruning MUST NOT remove anchors above the hard floor when budget pressure is below 0.4. Pruning MUST respect CANON and pinned immunity.

### Q4: Prolog Pre-Filter Query Design

**Question**: What Prolog query patterns best identify logically inconsistent anchors among projected proposition facts?
...
**Constraints**: Prolog projection MUST use DICE's existing `PrologEngine.query()` / `queryAll()` / `findAll()` API. Query execution time MUST be sub-second for anchor sets up to budget maximum (default 20). Prolog pre-filter is a cost-reduction optimization -- anchors NOT flagged by Prolog still require LLM evaluation.
...
2. **Task 2: Audit step -- Prolog pre-filter** (4 files)
   - Project active anchors to Prolog facts via DICE `PrologEngine`.
   - Implement Prolog queries for contradiction chain detection and unsupported claim identification.
   - Anchors flagged by the pre-filter are scored 0.0 (logically inconsistent) and excluded from the LLM batch.
   - Files: `AnchorPrologPreFilter.java`, Prolog query definitions, `ProactiveMaintenanceStrategy.java` (pre-filter integration), test.

3. **Task 3: Audit step -- LLM relevance evaluation** (4 files)
   - Implement batched LLM relevance call for anchors passing the Prolog pre-filter.
   - Design audit prompt template for batched LLM call.
   - Merge Prolog pre-filter results with LLM scores into unified per-anchor audit scores.
...
4. **Task 4: Refresh + consolidate steps** (4 files)
   - Refresh: re-rank anchors based on audit scores, trigger trust re-evaluation for borderline.
   - Consolidate: identify RELIABLE candidacy anchors, route to `CanonizationGate`.
   - Files: `ProactiveMaintenanceStrategy.java` (refresh + consolidate methods), `DiceAnchorsProperties` (candidacy config), `CanonizationGate` integration, test.

5. **Task 5: Prune + validate steps** (5 files)
   - Prune: archive anchors below threshold with two-tier logic.
   ...
   - Files: `ProactiveMaintenanceStrategy.java` (prune + validate methods), `DiceAnchorsProperties` (prune thresholds), Prolog validation queries, `CompactionValidator`/`InvariantEvaluator` integration, test.
...
6. **Task 6: Simulation integration** (4 files)
   ...
   - Files: `SimulationTurnExecutor.java`, `DiceAnchorsProperties` (frequency config), `MemoryPressureGauge` integration, test.
...
7. **Task 7: Convergence metrics + observability** (4 files)
   ...
   - Emit structured log events at INFO for cycle summary, DEBUG for per-anchor details.
...
2. **Rollback works**: A test scenario where validation fails (e.g., protected fact pruned) MUST trigger automatic rollback, restoring all anchors to pre-cycle state.
3. **Measurable recall improvement**: Running proactive maintenance on a degraded anchor set in simulation MUST produce measurable improvement in anchor quality metrics (average audit score, budget utilization, or fact survival rate) compared to the same scenario with reactive-only maintenance.
...
```
F01 (anchor-transaction-framework) ──┐
...
- `AnchorTransaction` rollback from F01
