...
Feature doc: `openspec/roadmaps/anchor-memory-optimization/features/08-proposition-quality-scoring.md`
...
- **Option B (Pre-trust filter)**: Implement a `QualityFilter` that runs before `TrustPipeline` in the `AnchorPromoter` pipeline. Propositions below quality threshold are rejected before trust evaluation.
...
**Decision space**:
- **(a) Heuristic**: Novelty via text similarity (Jaccard, cosine on TF-IDF) between proposition and existing anchor texts. Importance via entity overlap with recent conversation turns.
...
- **(b) Embedding similarity**: Novelty via cosine distance between proposition embedding and nearest anchor embedding. Importance via cosine similarity to conversation topic embedding.
  - Pro: Semantic-aware; moderate cost (embedding calls are cheap relative to full LLM calls).
  - Con: Requires embedding model integration (not currently in dice-anchors); adds a dependency.
- **(c) Batched LLM call**: Score all candidate propositions in a single LLM call with the existing anchor set as context.
...
1. **Task 1: Novelty scoring implementation** (4 files)
   - Implement novelty computation: compare proposition text against active anchor texts using text similarity.
   ...
   - Files: `NoveltySignal.java` (or `NoveltyScorer.java`), `DomainProfile` (add weight config), `DiceAnchorsProperties` (threshold config), test.
...
3. **Task 3: Pipeline integration + filtering** (4 files)
   - Wire scoring into `AnchorPromoter` pipeline (via TrustPipeline signal registration or pre-trust filter).
   - Implement threshold-based filtering for low-scoring propositions.
   - Add opt-in toggle to configuration.
   - Files: `TrustPipeline` or `AnchorPromoter` (integration), `DiceAnchorsProperties` (toggle + thresholds), Spring config bean, test.
...
5. **Task 5 (OPTIONAL): Prolog relationship scorer** (4 files)
   - Implement `PrologRelationshipScorer` that projects anchors and candidate propositions as Prolog facts, expresses domain relationship rules as Prolog clauses, and queries for logical support/entailment/subsumption via DICE's `PrologEngine`.
   - Contributes to importance scoring: propositions that logically support or extend existing anchors receive a higher importance score.
   ...
   - **Prerequisite**: Familiarity with DICE Prolog projection API (`PrologEngine.query()`, `queryAll()`, `findAll()`). Currently zero Prolog usage in dice-anchors — this would be the first integration point.
...
Integrates with (existing):
  ...
  - AnchorPromoter (pipeline integration)
