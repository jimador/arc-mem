...
- `computeEffectiveBudget(List<Anchor> activeAnchors) -> int`
- `selectForEviction(List<Anchor>, int excess) -> List<Anchor>`

All budget enforcement in `AnchorEngine` MUST delegate to the active `BudgetStrategy`.

### L2: Count-Based as Default

`CountBasedBudgetStrategy` MUST be the default strategy. It MUST reproduce current `AnchorEngine` budget enforcement behavior exactly. No behavioral change without explicit opt-in.

### L3: Density Reduces Effective Budget

`InterferenceDensityBudgetStrategy` MUST compute a reduced effective budget when anchor semantic overlap is high. The effective budget is always less than or equal to the raw budget (never increases it).

### L4: Cluster-Aware Eviction

When the density strategy triggers eviction, it SHOULD prefer evicting the lowest-ranked anchor in the largest interference cluster, rather than the globally lowest-ranked anchor. Pinned anchors and CANON-authority anchors remain immune to eviction regardless of strategy.
...
Budget strategy MUST be selectable per-context. Simulation scenarios MUST be able to specify budget strategy in scenario YAML. Global default is configured via `DiceAnchorsProperties`.
...
**Candidates**:
- **Conflict index edge density (F05)**: Count edges in the precomputed conflict adjacency graph per anchor. Anchors with many conflict/near-conflict edges are in dense regions. Pros: leverages existing F05 infrastructure, no additional data. Cons: conflict edges may not capture all forms of semantic overlap (two anchors can be related without conflicting).
- **Embedding-based clustering**: Compute cosine similarity between anchor text embeddings. Cluster anchors above a similarity threshold. Pros: captures semantic overlap beyond conflict. Cons: requires embedding infrastructure (model calls or pre-computed embeddings).
...
**Approach**: Thresholds MUST be configurable. Initial values SHOULD be calibrated empirically by running simulation benchmarks with varying anchor counts and density levels. Suggested starting points:
- Warning threshold: density score > 0.6 (60% of anchor pairs have conflict-index edges)
...
**Question**: How are anchors grouped into interference clusters?
...
- **Simple threshold grouping**: any pair with conflict-index edge weight above threshold forms a cluster. Transitive closure for multi-anchor clusters.
...
**Files** (3):
1. `src/main/java/dev/dunnam/diceanchors/anchor/BudgetStrategy.java` -- new interface
2. `src/main/java/dev/dunnam/diceanchors/anchor/CountBasedBudgetStrategy.java` -- new class extracting current budget logic
3. `src/test/java/dev/dunnam/diceanchors/anchor/CountBasedBudgetStrategyTest.java` -- new test

**Work**:
1. Define `BudgetStrategy` interface with `computeEffectiveBudget` and `selectForEviction` methods.
2. Extract current budget enforcement logic from `AnchorEngine` into `CountBasedBudgetStrategy`.
...
### Task 2: Wire BudgetStrategy into AnchorEngine

**Files** (4):
1. `src/main/java/dev/dunnam/diceanchors/anchor/AnchorEngine.java` -- refactor to delegate to `BudgetStrategy`
2. `src/main/java/dev/dunnam/diceanchors/DiceAnchorsProperties.java` -- add budget strategy config
3. `src/main/resources/application.yml` -- add default budget strategy property
4. `src/test/java/dev/dunnam/diceanchors/anchor/AnchorEngineTest.java` -- update tests for delegation

**Work**:
1. Replace inline budget logic in `AnchorEngine` with delegation to injected `BudgetStrategy`.
2. Add `budget.strategy` property to `DiceAnchorsProperties` (default: COUNT).
3. Spring configuration wires the appropriate strategy bean based on property.
4. All existing `AnchorEngine` tests MUST pass without modification (behavioral equivalence).
...
**Files** (3):
1. `src/main/java/dev/dunnam/diceanchors/anchor/InterferenceDensityCalculator.java` -- new class
2. `src/main/java/dev/dunnam/diceanchors/anchor/AnchorCluster.java` -- new record
3. `src/test/java/dev/dunnam/diceanchors/anchor/InterferenceDensityCalculatorTest.java` -- new test
...
2. Define `AnchorCluster` record: `(Set<String> anchorIds, double internalDensity)`.
...
4. Expose `computeDensity(List<Anchor>) -> double` and `identifyClusters(List<Anchor>) -> List<AnchorCluster>`.
...
**Files** (4):
1. `src/main/java/dev/dunnam/diceanchors/anchor/InterferenceDensityBudgetStrategy.java` -- new class
2. `src/test/java/dev/dunnam/diceanchors/anchor/InterferenceDensityBudgetStrategyTest.java` -- new test
3. `src/main/java/dev/dunnam/diceanchors/DiceAnchorsProperties.java` -- add density config parameters
...
2. Implement cluster-aware eviction: select lowest-ranked anchor from largest cluster.
...
**Files** (4):
1. `src/main/java/dev/dunnam/diceanchors/sim/engine/ScenarioLoader.java` -- read `budgetStrategy` from YAML
2. `src/main/java/dev/dunnam/diceanchors/sim/engine/SimulationTurnExecutor.java` -- apply per-scenario strategy
3. `src/main/resources/simulations/` -- update one scenario as example
4. `src/test/java/dev/dunnam/diceanchors/sim/engine/ScenarioLoaderTest.java` -- test YAML parsing
...
- `AnchorEngine` delegates to `BudgetStrategy`.
- ALL existing `AnchorEngine` tests pass without modification.
- Property-based strategy selection works (default: COUNT).

### Gate 3: Density Strategy Reduces Budget Under Overlap (after Task 4)

- `InterferenceDensityBudgetStrategy` computes reduced effective budget for high-overlap anchor sets.
- Cluster-aware eviction selects from largest cluster.
- Pinned and CANON anchors are immune.
