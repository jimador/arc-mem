...
2. **Four dimensions**: Budget pressure (anchor count vs. cap), conflict pressure (recent conflict rate), decay pressure (recent demotion rate), compaction pressure (compaction frequency).
3. **Event-driven updates**: Pressure recalculated on `AnchorLifecycleEvent` emissions (Promoted, Archived, RankDecayed, AuthorityChanged). No polling.
...
|---|----------|---------|---------|-------------------|
| 1 | **Exact weight defaults** | (a) Budget 0.4, conflict 0.3, decay 0.2, compaction 0.1 (emphasis on count and conflicts). (b) Budget 0.6, conflict 0.3, decay 0.1, compaction 0.0 (sleeping-llm edit-pressure-heavy). (c) Equal weights 0.25 each (unbiased). | (a) Budget 0.4, conflict 0.3, decay 0.2, compaction 0.1. Budget and conflict are the strongest degradation signals. Sleeping-llm's 0.6 edit-pressure weight is too heavy for anchors where conflict rate matters more than raw count. | Calibration via simulation runs post-implementation. Defaults are overridable. |
...
|------|-------------|---------|
| Pressure computable per-context | Unit test: given 15 anchors with budget 20, known conflict rate, known decay count -> deterministic pressure score. | `./mvnw test -pl . -Dtest=MemoryPressureGaugeTest` |
...
- **Scope boundary**: `anchor/` package for gauge and records; `sim/` integration is a separate task
...
| T3 | `DiceAnchorsProperties` pressure config + `PressureThresholdBreached` event + history tracking | `DiceAnchorsProperties.java` (update), `PressureThresholdBreached.java` | Threshold breach emits event; history snapshots accumulate per turn |

## Risks Requiring Design Attention

1. **Event subscription ordering**: `MemoryPressureGauge` subscribes to `AnchorLifecycleEvent`. If the gauge update fires before other event handlers (e.g., maintenance strategy), the pressure score may be stale for one turn. Ensure event ordering is documented or gauge is recalculated lazily on demand.
