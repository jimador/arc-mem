...
`LogitBiasEnforcer` is the primary deliverable. It MUST translate CANON anchor constraints into logit bias parameters compatible with the OpenAI API. This is implementable now with existing API infrastructure and provides immediate compliance improvement.
...
### O1: Anchor-to-Token Constraint Translation Approach

**Question**: How are semantic anchor propositions translated into token-level constraints?

**Context**: An anchor like "The dragon's lair is in the Northern Mountains" needs to become a set of tokens to boost/suppress. This is the hardest design problem in F12.

**Candidates**:
- **Keyword extraction**: Extract key entities and terms from anchor text ("dragon", "lair", "Northern", "Mountains"). Boost these tokens. Suppress their antonyms if identifiable. Pros: simple, deterministic. Cons: loses relational meaning; "Northern" boosted doesn't prevent "Southern Mountains".
- **Entity name biasing**: Focus only on proper nouns and named entities. Boost entity names that appear in anchors. Pros: high precision, low risk of false enforcement. Cons: low coverage -- misses adjectival/relational constraints entirely.
- **Negation-aware extraction**: Extract anchor terms AND their negations. Boost anchor terms, suppress negation patterns ("not Northern", "isn't in the Mountains"). Pros: better coverage than keyword-only. Cons: requires negation detection, still limited to surface-level tokens.
- **LLM-assisted constraint generation**: Use a small/fast LLM call to generate constraint token sets from anchor text. Pros: semantic understanding. Cons: per-anchor LLM call adds latency and cost; may itself hallucinate constraints.
...
**Question**: Which anchor constraints can be meaningfully expressed as token-level biases vs. which require full constrained decoding?
...
- **Coverage estimate**: Logit bias can meaningfully address ~30-40% of typical anchor constraints (primarily entity names and simple attributes). The remaining 60-70% rely on prompt injection.

**Decision**: `AnchorConstraintIndex` MUST track `translationCoverage` -- the fraction of active anchors whose constraints were expressible as token-level biases. This metric informs whether logit bias alone provides sufficient improvement or whether constrained decoding is needed.
...
### Task 1: AnchorConstraintIndex

**Files** (3):
1. `src/main/java/dev/dunnam/diceanchors/assembly/AnchorConstraintIndex.java` -- new class
2. `src/main/java/dev/dunnam/diceanchors/assembly/AnchorConstraint.java` -- new record
3. `src/test/java/dev/dunnam/diceanchors/assembly/AnchorConstraintIndexTest.java` -- new test

**Work**:
1. Define `AnchorConstraint` record: `(String anchorId, Authority authority, Set<String> boostTokens, Set<String> suppressTokens, double translationCoverage)`.
2. Implement `AnchorConstraintIndex.build(List<Anchor>) -> AnchorConstraintIndex`.
3. Implement entity name extraction from anchor text (initial approach per O1).
4. Compute per-anchor translation coverage (fraction of anchor meaning captured by token constraints).
5. Expose `getConstraints() -> List<AnchorConstraint>`, `getTotalCoverage() -> double`.
6. Unit tests: constraint extraction from sample anchor texts, coverage calculation, authority-based constraint strength.
...
**Files** (5):
1. `src/main/java/dev/dunnam/diceanchors/assembly/LogitBiasEnforcer.java` -- new class implementing `ComplianceEnforcer`
2. `src/main/java/dev/dunnam/diceanchors/assembly/LogitBiasMap.java` -- new record for bias representation
3. `src/main/java/dev/dunnam/diceanchors/assembly/AnchorConstraintIndex.java` -- consume constraints
4. `src/main/java/dev/dunnam/diceanchors/DiceAnchorsProperties.java` -- add enforcement strategy config
5. `src/test/java/dev/dunnam/diceanchors/assembly/LogitBiasEnforcerTest.java` -- new test

**Work**:
1. Implement `LogitBiasEnforcer` consuming `AnchorConstraintIndex` to produce logit bias maps.
...
6. Add `compliance.enforcement-strategy` property to `DiceAnchorsProperties` (default: PROMPT_ONLY).
...
2. `src/main/java/dev/dunnam/diceanchors/assembly/ConstraintMask.java` -- new record for constraint representation
3. `src/test/java/dev/dunnam/diceanchors/assembly/ConstrainedDecodingEnforcerTest.java` -- contract test (mock implementation)

**Work**:
1. Define `ConstrainedDecodingEnforcer` interface extending `ComplianceEnforcer`.
2. Add method: `computeConstraintMask(AnchorConstraintIndex, int vocabSize) -> ConstraintMask`.
...
**Files** (5):
1. `src/main/java/dev/dunnam/diceanchors/assembly/HybridComplianceEnforcer.java` -- new class composing enforcement layers
2. `src/main/java/dev/dunnam/diceanchors/assembly/ModelCapabilityDetector.java` -- new class for API feature detection
3. `src/main/java/dev/dunnam/diceanchors/assembly/LogitBiasEnforcer.java` -- integrate capability detection
4. `src/main/resources/application.yml` -- add enforcement strategy defaults
5. `src/test/java/dev/dunnam/diceanchors/assembly/HybridComplianceEnforcerTest.java` -- new test
...
- `AnchorConstraintIndex` extracts meaningful tokens from anchor text.
- `LogitBiasEnforcer` produces valid logit bias maps from anchor constraints.
