...
1. **Pre-extraction conflict check**: New gate between confidence and dedup that uses `ConflictIndex` (F05) for O(1) conflict lookup against existing RELIABLE+ anchors.
...
4. **Configurable batch size**: Via `DiceAnchorsProperties` (default: 10, reusing existing `sim.batchMaxSize`).
...
1. **`extract/` package (primary)**: `AnchorPromoter` gate reordering, new `ConflictPreCheck` gate, batch sizing logic.
2. **`anchor/` package (integration)**: `ConflictIndex` consumed for pre-check lookups.
3. **`DiceAnchorsProperties`**: Batch size configuration (MAY reuse existing `sim.batchMaxSize` or add dedicated `promotion.batchSize`).
...
2. Early-rejection counts MUST be logged: `promotion.precheck.rejected` with count and reason (conflict with existing anchor).
...
2. Propositions conflicting with existing RELIABLE+ anchors MUST be filtered before the dedup gate.
...
9. Pre-check rejection MUST be logged with the conflicting anchor ID and text for auditability.
...
1. **REQ-PRECHECK**: The system SHALL check incoming propositions against the `ConflictIndex` before the dedup gate, filtering candidates that conflict with RELIABLE+ anchors.
...
1. **Unit tests** MUST verify that pre-check filters propositions conflicting with RELIABLE+ anchors.
