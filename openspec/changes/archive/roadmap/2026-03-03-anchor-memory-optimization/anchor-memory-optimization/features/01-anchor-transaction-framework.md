# Feature: Anchor Transaction Framework
...
Introduce a declarative transaction/rollback framework for anchor operations, modeled after Spring's `@Transactional`. Snapshot anchor state before operations, execute, validate invariants, commit or rollback. Primary use case is bulk operations (maintenance sweeps, batch promotions, decay cycles) but the interface MUST support individual operations too.
...
1. **Problem addressed**: All anchor lifecycle operations are currently forward-only. Maintenance sweeps, batch promotions, and decay cycles can leave the system in a degraded state if interrupted or if results violate invariants. There is no way to atomically undo a set of mutations. When a decay cycle reduces ranks below thresholds triggering cascading authority demotions, the only recovery path is manual intervention.
...
1. `AnchorTransactionTemplate` programmatic API (analogous to Spring's `TransactionTemplate`).
2. `@AnchorTransactional` annotation for declarative transaction demarcation on service methods.
3. `AnchorSnapshot` value type capturing anchor state (IDs, ranks, authorities, trust scores, pinned status) at a point in time.
4. `AnchorTransactionManager` managing snapshot lifecycle, invariant validation, commit/rollback orchestration.
5. Validation gates: configurable invariant checks that determine commit/rollback (reusing `InvariantEvaluator`).
6. Rollback implementation: restore snapshot state via `AnchorRepository` bulk update.
7. Transaction lifecycle events (`TransactionBegun`, `TransactionCommitted`, `TransactionRolledBack`).
8. Integration with `AnchorEngine` bulk operations (batch promote, decay cycle, maintenance sweep).
...
3. Neo4j-level transaction management (this operates at the anchor domain level, above the persistence layer).
...
2. Priority: MUST.
3. OpenSpec change slug: `anchor-transaction-framework`.
...
1. **`anchor/` package (primary)**: New types -- `AnchorSnapshot` (record), `AnchorTransactionManager` (service), `AnchorTransactionTemplate` (programmatic API), `@AnchorTransactional` (annotation), `AnchorTransactionContext` (thread-local state).
2. **`anchor/event/` package**: New lifecycle events -- `TransactionBegun`, `TransactionCommitted`, `TransactionRolledBack` extending `AnchorLifecycleEvent`.
...
2. Rollback events SHOULD display the reason (which invariant was violated) and the count of anchors restored.

### Observability Visibility

1. Transaction lifecycle MUST emit structured log events at INFO level: `transaction.begin` (context ID, scope), `transaction.commit` (anchor count, duration), `transaction.rollback` (reason, anchor count restored).
2. Transaction events SHOULD include OpenTelemetry span attributes: `anchor.transaction.id`, `anchor.transaction.scope`, `anchor.transaction.outcome`.
3. Rollback events MUST log the specific invariant violations that triggered the rollback.

## Acceptance Criteria

1. Bulk anchor operations (batch promote, decay cycle, maintenance sweep) MUST be wrappable in a transaction via `AnchorTransactionTemplate`.
...
4. Rollback MUST restore all anchor states (rank, authority, trust score, pinned status) to pre-transaction values.
5. Individual operations SHOULD be wrappable (opt-in, not mandatory for single-anchor mutations).
6. Existing non-transactional code MUST continue working unchanged -- the framework is opt-in.
7. `@AnchorTransactional` annotation MUST support declarative usage on Spring-managed service methods.
...
|------|-----------|--------|------------|
| **Snapshot overhead for large contexts** | Medium | Medium | Snapshot only affected anchors (scoped by context ID), not all anchors globally. Lazy snapshot: capture on first mutation, not on transaction begin. |
| **Race conditions with concurrent transactions** | Low | High | Anchor operations are already context-isolated (`contextId`). Transactions inherit this isolation. Document that concurrent transactions on the same context ID are undefined behavior. |
| **Rollback inconsistency with Neo4j state** | Medium | High | Rollback uses `AnchorRepository` bulk update (same persistence layer as forward operations). Test rollback round-trip: snapshot -> mutate -> rollback -> verify state matches snapshot. |
...
`anchor-transaction-framework`

### Proposal Starter Inputs

1. **Problem statement**: All anchor lifecycle operations are forward-only. Maintenance sweeps, batch promotions, and decay cycles can leave the system in a degraded state if interrupted or if results violate invariants. There is no way to atomically undo a set of mutations.
...
1. **Snapshot management**: Capture and restore anchor state (rank, authority, trust score, pinned).
2. **Transaction lifecycle**: Begin, validate, commit/rollback orchestration with invariant gates.
3. **Annotation support**: `@AnchorTransactional` for declarative transaction demarcation.
4. **Event integration**: Lifecycle events for transaction begin/commit/rollback.

### Candidate Requirement Blocks

1. **REQ-SNAPSHOT**: The system SHALL capture anchor state (rank, authority, trust score, pinned status) as a snapshot before transactional operations begin.
2. **REQ-VALIDATE**: The system SHALL validate invariants (via `InvariantEvaluator`) before committing transactional changes.
3. **REQ-ROLLBACK**: The system SHALL restore all anchors to their snapshot state when validation fails.
...
1. **Unit tests** MUST verify snapshot capture preserves all anchor fields (rank, authority, trust score, pinned).
...
1. **Snapshot granularity**: Snapshots capture anchor-level state only. Relationship changes (edges in Neo4j) between anchors during a transaction are not captured or rolled back. If transaction operations modify inter-anchor relationships, rollback may be incomplete.
...
```
/opsx:new anchor-transaction-framework
```
