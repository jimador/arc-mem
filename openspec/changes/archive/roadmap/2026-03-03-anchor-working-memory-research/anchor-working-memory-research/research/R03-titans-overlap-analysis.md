# Research Task: TITANS Overlap Analysis for Anchor + DICE Architecture
...
Does the TITANS neuroscience-memory framing meaningfully overlap with dice-anchors' anchor concept and DICE usage, and if yes, what should we change?
...
3. Useful overlap for improving anchor promotion/retention policy without abandoning DICE + explicit governed-context control.
4. Adversarial inputs remain important in this project as a stress harness to force hallucination/contradiction and test whether relevant anchors stay stable.
...
3. Mapping to current dice-anchors design and DICE integration.
...
2. Primary evidence should be anchored to TITANS + TITANS Revisited + BABILong papers.
3. Repository truth for "what we are doing" is grounded in [README.md](/Users/jamesdunnam/playground/dice-anchors/README.md).

## Direct Quote Evidence (With Hard Links)

| Source | Direct quote | Why it matters here |
|---|---|---|
| Freedium article | "multiple memory systems operating at different timescales" ([link](https://freedium-mirror.cfd/https://ai.gopubby.com/google-titans-neuroscience-ai-memory-explained-69b319f1f516)) | Matches our separation between broad DICE memory and bounded anchor working set. |
...
| BABILong | "effectively utilize only 10-20% of the context" ([link](https://arxiv.org/abs/2406.10149)) | Reinforces why explicit salience control (anchors) is needed despite long context windows. |
| dice-anchors README | "Anchors add a trust-governed working memory layer" ([link](/Users/jamesdunnam/playground/dice-anchors/README.md)) | Our system-level intent is explicit governed working memory, not raw storage. |
| dice-anchors README | "DICE owns extraction and revision. Anchors owns promotion" ([link](/Users/jamesdunnam/playground/dice-anchors/README.md)) | Clarifies architectural boundary for any TITANS-inspired change. |

## What We Are Doing Today (Anchor + DICE)

Current design (already implemented):

1. DICE extracts/revises propositions and preserves broad memory coverage.
2. Anchors promotes a subset into a governed working set (rank + authority + trust + budget).
3. Prompt assembly guarantees anchor injection (not best-effort retrieval).
...
- dice-anchors centers these controls.
...
- dice-anchors mutates explicit external state with auditable lifecycle events.
- Assessment: different execution model and risk profile.

3. **DICE integration**
- TITANS is architecture-level.
- dice-anchors is orchestration/governance over DICE extraction.
- Assessment: overlap is conceptual, not plug-and-play.

## High-Value Improvements for dice-anchors
...
Where to integrate:
1. `extract/AnchorPromoter`
2. `anchor/TrustPipeline`
3. `anchor/ReinforcementPolicy`
...
Where to integrate:
1. `anchor/ReinforcementPolicy`
2. `anchor/DecayPolicy`
...
2. Use turn-level indicators: inconsistency rate, compaction loss risk, anchor churn.

Where to integrate:
1. `anchor/AnchorEngine`
...
2. Removing these controls would erase the core purpose of anchors.

## Conclusion

There is meaningful overlap, so stopping would leave value on the table. The overlap is not "replace anchors with TITANS." The useful path is to import TITANS-style retention dynamics (surprise, carryover, adaptive forgetting pressure) into our existing DICE + anchor governance stack.
...
5. Local architecture/source of truth: [README.md](/Users/jamesdunnam/playground/dice-anchors/README.md)
