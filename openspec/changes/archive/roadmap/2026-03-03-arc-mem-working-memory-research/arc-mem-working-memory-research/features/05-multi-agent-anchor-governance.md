# Feature: Multi-Agent Context Unit Governance
...
Extend the context unit trust model from single-agent (one LLM, one context window) to multi-agent (multiple LLMs with heterogeneous capabilities, communicating via A2A or similar protocols). The core insight is that trust becomes two-dimensional: fact-level authority (existing PROVISIONAL to CANON hierarchy) and agent-level provenance (who asserted this fact, and how much do we trust that agent). Combined with per-agent budget partitioning based on context capacity, this makes context units the governance layer for heterogeneous multi-agent memory -- a capability no existing system provides.
...
1. **Problem addressed**: The current context unit model assumes a single agent consuming a single context window. In multi-agent systems (e.g., a large orchestrator coordinating small task agents), there is no mechanism to track which agent asserted a fact, no way to partition the unit budget by agent capability, and no cross-agent authority promotion when multiple agents corroborate a fact.
2. **Value delivered**: Small-context-window agents (Haiku-class, 8K tokens, budget 5) receive the 5 highest-authority facts that fit their window -- not a random subset. Large orchestrators (GPT-4o-class, 128K tokens, budget 50) receive the full authority hierarchy. Context Unit budget enforcement becomes the mechanism that makes small models useful in multi-agent coordination.
...
4. **Why now**: This feature is Wave 3 (future work) because it requires the single-agent context unit model to be thoroughly evaluated first (Track A). It is documented now so the paper's "Future Work" section can describe a concrete architecture extension, and so the A2A protocol alignment can be tracked as it matures.

## Scope

### In Scope

1. Agent-level trust provenance on context units: each context unit carries a `sourceAgent` field identifying which agent asserted it.
2. Per-agent budget configuration: each agent's unit budget is derived from its context capacity (e.g., 8K window gets budget 5, 128K window gets budget 50).
...
5. Per-agent context assembly: each agent receives a rank-ordered, budget-constrained subset of context units tailored to its context capacity.
6. Evaluation methodology for asymmetric agent scenarios (large orchestrator + small task agents).

### Out of Scope

1. Implementation of a full A2A protocol stack -- this feature defines the context unit governance layer, not the transport/communication protocol.
...
3. Priority: MAY.
4. OpenSpec change slug: `multi-agent-unit-governance` (scaffold when ready).
...
| How does the A2A protocol handle shared state and context? | Google A2A spec, A2A GitHub repo, implementation examples | 3h | Protocol alignment documented; gaps between A2A context model and context unit governance identified |

## Impacted Areas

1. **`context unit/`**: `Context Unit` model extended with `sourceAgent` provenance field. `ArcMemEngine` extended with per-agent budget enforcement and cross-agent corroboration logic.
2. **`assembly/`**: `ArcMemLlmReference` and `PromptBudgetEnforcer` extended to support per-agent context assembly with agent-specific budget constraints.
...
1. The context inspector panel SHOULD display agent provenance for each context unit -- which agent asserted it and at what trust level.
...
1. Each context unit assertion MUST be logged with: context unit ID, source agent ID, agent trust level, claimed authority, effective authority (after trust adjustment), and target agent's budget state.
...
3. Budget eviction events in multi-agent context MUST include: which agent's budget overflowed, what was evicted, and whether the evicted context unit was from a different source agent.
...
1. Each context unit MUST carry agent-level trust provenance identifying the source agent.
...
3. The authority upgrade-only invariant MUST be preserved across agents. No agent MAY downgrade another agent's context unit authority.
...
7. Per-agent context assembly MUST produce a rank-ordered, budget-constrained context unit set tailored to each agent's configured budget.
8. The feature SHOULD demonstrate measurable benefit in asymmetric scenarios: a small task agent (budget 5) with context unit governance SHOULD outperform the same agent without governance on factual consistency metrics.
...
1. **Risk**: A compromised or hallucinating agent floods the shared knowledge base with false propositions. **Mitigation**: Trust-degraded injection (AC #6) ensures low-trust agents' facts arrive as PROVISIONAL. Budget enforcement prevents any single agent from monopolizing context unit slots. Rate limiting on per-agent assertions MAY be added.
...
3. **Risk**: Budget partitioning strategy is wrong -- small agents get too few or too many context units. **Mitigation**: Budget is configurable per agent. Research task on optimal partitioning. Empirical evaluation with at least 3 model size tiers before committing to defaults.
4. **Risk**: A2A protocol is immature and may change significantly. **Mitigation**: Governance layer is protocol-agnostic (AC #9). A2A integration is a MAY, not a MUST. The context unit trust model has value independent of any specific protocol.
5. **Risk**: Complexity explosion -- multi-agent governance adds significant system complexity. **Mitigation**: Single-agent mode regression guarantee (AC #10). Multi-agent governance is additive, not a rewrite. The existing `ArcMemEngine` is extended, not replaced.
...
`multi-agent-unit-governance`

### Proposal Starter Inputs

1. **Problem statement**: Multi-agent LLM systems have no graduated trust model for shared memory. Existing approaches use either no governance (ShardMemo), no trust model (MemGPT/Letta), or binary trust (A-MemGuard). Context Units already implement graduated trust (PROVISIONAL to CANON) for single-agent contexts. Extending this to multi-agent requires two additions: agent-level provenance and per-agent budget partitioning.
2. **Why now**: The A2A protocol is emerging as a standard for multi-agent communication. Establishing context unit governance as the trust layer early positions context units as the reference implementation for governed multi-agent memory.
...
4. **Visible outcomes**: Operators MUST be able to see which agent asserted each context unit, what trust level was applied, and how per-agent budgets are utilized. Researchers MUST be able to compare single-agent vs. multi-agent governance in controlled experiments.
...
1. **Requirement**: The system SHALL track agent-level provenance on all context units, recording the source agent and its trust level at assertion time.
...
3. **Scenario**: A large orchestrator (budget 50) and a small task agent (budget 5) share a knowledge base of 30 context units. The orchestrator receives all 30 (within budget). The task agent receives the top 5 by authority-then-rank ordering. The task agent's outputs are factually consistent because it received the highest-authority facts.
...
2. **Asymmetric agent experiment**: Run a scenario with one large orchestrator (budget 50) and two small task agents (budget 5 each). Compare task agent factual consistency with and without context unit governance. Hypothesis: governed agents are significantly more consistent.
...
5. **Single-domain evaluation**: As with all features in this roadmap, evaluation is in the D&D/collaborative fiction domain. Whether multi-agent context unit governance generalizes to other domains (e.g., multi-agent code generation, multi-agent customer support) is an open question.

## Suggested Command

`openspec new change multi-agent-unit-governance`
