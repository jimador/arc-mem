...
1. **Ablation condition model**: Declarative definitions for at least four experimental conditions that configure context unit subsystem behavior per-run.
...
3. Modifications to the core ARC-Mem engine (`context unit/` package) -- conditions configure existing engine behavior, not extend it.
...
2. **`sim.engine` package (minor)**: SimulationService MAY need a condition-application hook to override context unit authority/rank seeding per condition. The existing `injectionStateSupplier` handles the NO_UNITS condition; other conditions require per-run configuration of context unit seeding behavior.
...
1. The framework MUST support at least four ablation conditions:
   - `FULL_UNITS` -- injection ON, full authority hierarchy (PROVISIONAL through CANON), rank differentiation, budget enforcement.
   - `NO_UNITS` -- injection OFF (formalizes the existing UI toggle).
   - `FLAT_AUTHORITY` -- injection ON, all context units seeded at RELIABLE authority, no promotion occurs during the run.
   - `NO_RANK_DIFFERENTIATION` -- injection ON, all context units seeded at rank 500, no rank changes during the run.
...
4. The framework MUST compute Cohen's d effect sizes between every pair of conditions for each metric (factSurvivalRate, driftAbsorptionRate, contradictionCount, majorContradictionCount, meanTurnsToFirstDrift, unitAttributionCount).
5. The framework MUST compute 95% confidence intervals for each metric in each condition cell.
6. The framework MUST compute per-strategy effectiveness comparison across conditions, enabling statements like "SUBTLE_REFRAME succeeded 60% against NO_UNITS but only 15% against FULL_UNITS."
...
12. The framework MUST NOT modify the core ARC-Mem engine (`context unit/` package). Condition application MUST work through configuration of the simulation service layer.
...
| **Condition application requires SimulationService changes** | Medium | Medium | Prefer a condition-applicator strategy pattern that configures the existing `injectionStateSupplier` and seed-context unit behavior rather than modifying SimulationService internals. If SimulationService changes are unavoidable, they MUST be backward-compatible. |
| **FLAT_AUTHORITY and NO_RANK conditions alter seed context units** | Low | Low | Conditions that override authority or rank MUST do so at the seed-context unit level before the simulation loop starts, not by mutating live context unit state mid-run. This preserves the existing context unit lifecycle invariants. |
...
1. **Problem statement**: The existing benchmark infrastructure supports single-condition multi-run execution. Evaluating the marginal contribution of individual context unit subsystems (authority hierarchy, rank differentiation) requires controlled ablation experiments that the current infrastructure cannot express or execute.
...
3. **Constraints/non-goals**: No changes to the core ARC-Mem engine. No formal hypothesis testing (descriptive stats and effect sizes only). No UI (deferred to F02). No new scenarios (runs against existing corpus).
...
1. **REQ-COND**: The system SHALL define ablation conditions as declarative configuration objects that control context injection, authority seeding, rank seeding, and rank mutation behavior per simulation run.
