...
- [x] 1.1 Add `AttentionConfig` nested record to `ArcMemProperties` with all properties (enabled, windowDuration, pressureThreshold, minConflictsForPressure, heatPeakThreshold, heatDropThreshold, clusterDriftMinUnits, maxExpectedEventsPerWindow)
- [x] 1.2 Add `context units.attention.*` defaults to `application.yml`
- [x] 1.3 Test: config binding with defaults, config binding with overrides

## 2. Core Value Objects

- [x] 2.1 Create `AttentionKey` record in `context unit/attention/` — `(unitId, contextId)`
...
- [x] 4.2 Implement `@EventListener` method with context unit ID extraction via pattern matching switch
...
- [x] 4.9 Test: ConflictDetected updates windows for all conflicting context unit IDs
...
- [x] 5.1 Implement `getWindow(unitId, contextId)` returning `Optional<AttentionWindow>`
- [x] 5.2 Implement `getHottestUnits(contextId, limit)` returning sorted list
...
- [x] 5.6 Test: `getHottestUnits` returns correct order and limit
...
- [x] 6.3 Document design decisions and trade-offs: why `context unit/attention/` package placement, why ApplicationEvent (not sealed hierarchy), why ConcurrentHashMap over synchronized, heatScore static normalization (with note to revisit adaptive normalization), CLUSTER_DRIFT per-event evaluation (with note to revisit periodic scheduled evaluation)
