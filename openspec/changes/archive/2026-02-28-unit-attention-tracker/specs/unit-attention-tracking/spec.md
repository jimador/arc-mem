...
### Requirement: AttentionWindow per-context unit accumulator

The system SHALL maintain an `AttentionWindow` record per (unitId, contextId) pair that accumulates lifecycle event counts within a sliding time window. `AttentionWindow` SHALL be an immutable record in `dev.dunnam.arcmem.context unit.attention`.

`AttentionWindow` SHALL contain the following fields:

| Field | Type | Description |
|-------|------|-------------|
| `unitId` | String | The context unit being tracked |
| `contextId` | String | The context (chat session or sim run) |
| `conflictCount` | int | `ConflictDetected` events targeting this context unit within the window |
| `reinforcementCount` | int | `Reinforced` events for this context unit within the window |
| `totalEventCount` | int | All lifecycle events for this context unit within the window |
...
- **GIVEN** an `AttentionWindow` for context unit "A1" with `conflictCount = 2` and `totalEventCount = 5`
...
The system SHALL define an `AttentionSignalType` enum in `dev.dunnam.arcmem.context unit.attention` with the following values:

| Value | Description | Trigger condition |
|-------|-------------|-------------------|
| `PRESSURE_SPIKE` | Rapid accumulation of conflict events targeting a single context unit | `pressureScore() >= config.pressureThreshold` AND `conflictCount >= config.minConflictsForPressure` |
| `HEAT_PEAK` | High overall activity for an context unit | `heatScore() >= config.heatPeakThreshold` |
...
| `CLUSTER_DRIFT` | Multiple context units in the same context experiencing concurrent heat drops | Count of context units with `HEAT_DROP` in the same evaluation cycle `>= config.clusterDriftMinUnits` |
...
- **AND** an context unit with `pressureScore() = 0.7` and `conflictCount = 4`
...
- **GIVEN** `config.pressureThreshold = 0.6` and `config.minConflictsForPressure = 3`
- **AND** an context unit with `pressureScore() = 0.8` but `conflictCount = 2`
- **WHEN** the tracker evaluates attention signals
...
The system SHALL define an `AttentionSignal` class extending `ApplicationEvent` in `dev.dunnam.arcmem.context unit.attention`. `AttentionSignal` SHALL be published via Spring's `ApplicationEventPublisher` when attention thresholds are crossed.

`AttentionSignal` SHALL contain:

| Field | Type | Description |
|-------|------|-------------|
| `unitId` | String | The context unit that triggered the signal (null for `CLUSTER_DRIFT`) |
...
`AttentionSignal` SHALL NOT be part of the `ContextUnitLifecycleEvent` sealed hierarchy. It is a separate event type — lifecycle events are inputs to the tracker; attention signals are outputs.

#### Scenario: Signal carries frozen snapshot

- **GIVEN** an context unit with `heatScore = 0.8`, `pressureScore = 0.3`, `burstFactor = 2.1`
...
- **GIVEN** a cluster drift condition involving context units "A1", "A2", "A3"
...
The system SHALL define an `AttentionSnapshot` record in `dev.dunnam.arcmem.context unit.attention` that captures a frozen point-in-time view of an context unit's attention metrics.
...
| `unitIds` | `List<String>` | Context Unit IDs included (single context unit for per-context unit signals, multiple for cluster signals) |
...
- **THEN** the snapshot SHALL contain matching values and `unitIds` containing the single context unit ID

#### Scenario: Snapshot from cluster

- **GIVEN** three `AttentionWindow`s for context units "A1", "A2", "A3" with heat scores 0.2, 0.1, 0.15
...
The system SHALL provide an `AttentionTracker` Spring bean in `dev.dunnam.arcmem.context unit.attention` that listens for `ContextUnitLifecycleEvent` subtypes and maintains `AttentionWindow` state per (unitId, contextId).

`AttentionTracker` SHALL:

1. Listen for all `ContextUnitLifecycleEvent` subtypes via `@EventListener`
2. Extract the context unit ID from each event (using the event-type-specific accessor — `getUnitId()`, `getPropositionId()`, etc.)
3. For `ConflictDetected` events, update windows for ALL context unit IDs in `getConflictingUnitIds()`
...
- **GIVEN** an active `AttentionTracker` with no prior events for context unit "A1" in context "ctx-1"
...
- **AND** context unit "A1" has `conflictCount = 2`, `totalEventCount = 3` (pressure = 0.67, below min count)
...
- **GIVEN** a `PRESSURE_SPIKE` was already emitted for context unit "A1"
...
- **GIVEN** a `PRESSURE_SPIKE` was emitted for context unit "A1"
...
- **GIVEN** an `AttentionWindow` exists for context unit "A1" in context "ctx-1"
- **WHEN** an `Archived` or `Evicted` event for "A1" is received
- **THEN** the window SHALL be removed from the tracker's state (the context unit is no longer active)

---

### Requirement: AttentionConfig configuration properties

The system SHALL define `AttentionConfig` as a `@ConfigurationProperties` record bound to `context units.attention`.
...
| `clusterDriftMinUnits` | int | `3` | Minimum context units experiencing `HEAT_DROP` to trigger `CLUSTER_DRIFT` |
...
- **GIVEN** `context units.attention.enabled = false`
- **WHEN** any `ContextUnitLifecycleEvent` is published
- **THEN** the `AttentionTracker` SHALL not update any windows or publish any signals

#### Scenario: Custom window duration

- **GIVEN** `context units.attention.window-duration = PT10M`
- **WHEN** events are processed
- **THEN** the sliding window SHALL span the last 10 minutes

#### Scenario: Custom pressure thresholds

- **GIVEN** `context units.attention.pressure-threshold = 0.8` and `context units.attention.min-conflicts-for-pressure = 5`
- **WHEN** an context unit has `pressureScore() = 0.7` and `conflictCount = 4`
...
|--------|-------------|-------------|
| `getWindow(String unitId, String contextId)` | `Optional<AttentionWindow>` | Current window for a specific context unit |
| `getHottestUnits(String contextId, int limit)` | `List<AttentionWindow>` | Top N context units by `heatScore()` descending |
| `getAllWindows(String contextId)` | `List<AttentionWindow>` | All active windows for a context |
| `snapshot(String contextId)` | `Map<String, AttentionSnapshot>` | Frozen snapshots of all windows, keyed by context unit ID |
...
#### Scenario: Query hottest context units

- **GIVEN** context "ctx-1" has 10 context units with varying heat scores
...
- **GIVEN** context "ctx-1" has active windows for context units "A1" and "A2"
...
2. Automatic cleanup when all context units in a context have been archived or evicted (detected when the last `AttentionWindow` for a context is removed)
...
#### Scenario: Automatic cleanup on last context unit removal

- **GIVEN** context "ctx-1" has exactly one remaining attention window for context unit "A1"
...
- **ATT3**: `AttentionSignal` SHALL NOT be part of the `ContextUnitLifecycleEvent` sealed hierarchy. Lifecycle events are inputs; attention signals are outputs.
- **ATT4**: The `AttentionTracker` SHALL NOT modify context unit state (rank, authority, pinned status). It is a passive observer that produces signals for downstream consumers to act on.
