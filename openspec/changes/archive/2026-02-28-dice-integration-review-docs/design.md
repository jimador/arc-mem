...
dice-anchors integrates with DICE 0.1.0-SNAPSHOT for proposition extraction, revision, persistence, and incremental analysis. The integration spans two distinct extraction paths:
...
Both paths share the same pipeline configuration (`PropositionConfiguration`) and persistence layer (`AnchorRepository` implementing DICE's `PropositionRepository` SPI).
...
- Establish clear responsibility boundaries between dice-anchors and DICE
...
|  |- DiceAnchorsChunkHistoryStore wrapper (DiceAnchorsChunkHistoryStore.java) [MEDIUM risk]
...
|  |- dice-anchors implements: anchor lifecycle, trust pipeline, conflict detection, context assembly
