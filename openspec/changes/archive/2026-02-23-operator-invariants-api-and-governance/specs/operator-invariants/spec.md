...
The system SHALL define an `OperatorInvariant` record representing a declarative governance rule that constrains anchor lifecycle operations. Each invariant SHALL contain the following fields:
...
| `targetAnchorId` | String (nullable) | Specific anchor ID the rule targets; null for type-wide rules |
...
#### Scenario: Create a MUST-strength anchor protection invariant

- **GIVEN** an operator wants to protect anchor "A1" from eviction
- **WHEN** an `OperatorInvariant` is created with `type = ANCHOR_PROTECTED`, `targetAnchorId = "A1"`, `strength = MUST`
...
- **GIVEN** an operator wants at least 3 RELIABLE anchors in all contexts
...
| Value | Description |
|-------|-------------|
| `ANCHOR_PROTECTED` | The specified anchor MUST NOT be archived, evicted, or demoted. Requires `targetAnchorId`. |
| `AUTHORITY_FLOOR` | The specified anchor's authority MUST NOT drop below a configured level. Requires `targetAnchorId` and a minimum authority parameter. |
| `MINIMUM_COUNT` | The context MUST maintain at least N active anchors at or above a specified authority level. |
| `CONTEXT_FROZEN` | No anchors in the specified context MAY be archived, evicted, or demoted. The context is read-only for destructive operations. |
...
#### Scenario: ANCHOR_PROTECTED constrains archive, evict, and demote

- **GIVEN** an invariant of type `ANCHOR_PROTECTED`
...
- **GIVEN** a global `MINIMUM_COUNT` invariant requiring at least 2 RELIABLE anchors
...
- **GIVEN** an `ANCHOR_PROTECTED` invariant with `strength = MUST` targeting anchor "A1"
...
- **GIVEN** an `ANCHOR_PROTECTED` invariant with `strength = SHOULD` targeting anchor "A1"
- **WHEN** budget enforcement attempts to evict anchor "A1"
...
- `evaluate(LifecycleAction action, String anchorId, String contextId)` -- evaluates all applicable invariants for the given action and returns an `InvariantEvaluation` containing the list of violations (if any) and whether the action is blocked.
...
- **GIVEN** a MUST-strength `ANCHOR_PROTECTED` invariant for anchor "A1" and a SHOULD-strength `MINIMUM_COUNT` invariant
...
- **GIVEN** an `ANCHOR_PROTECTED` invariant with `enabled = false`
- **WHEN** `evaluate(ARCHIVE, "A1", "ctx-1")` is called
...
The system SHALL support defining operator invariants via YAML configuration under the `dice-anchors.invariants` key. Each invariant definition SHALL map to an `OperatorInvariant` record.
  ...
  invariants:
    rules:
      - rule-id: protect-cursed-blade
        type: ANCHOR_PROTECTED
        scope: GLOBAL
        target-anchor-id: "anchor-uuid-123"
        strength: MUST
        description: "The cursed blade anchor must never be removed"
        enabled: true
      - rule-id: min-reliable-count
        ...
        strength: SHOULD
        description: "Maintain at least 3 RELIABLE anchors"
        enabled: true
        parameters:
          min-count: 3
...
The configuration SHALL be bound via `@ConfigurationProperties` on `DiceAnchorsProperties` and validated at startup. Invalid invariant definitions (e.g., `ANCHOR_PROTECTED` without `targetAnchorId`) SHALL cause a startup failure with a descriptive error.
...
- **GIVEN** an `ANCHOR_PROTECTED` invariant in YAML with no `target-anchor-id`
- **WHEN** the application starts
- **THEN** startup SHALL fail with `IllegalStateException` containing "ANCHOR_PROTECTED requires targetAnchorId"
...
1. **Before archive** -- evaluated when `AnchorEngine.archive()` is called
2. **Before eviction** -- evaluated during budget enforcement in `AnchorEngine.promote()` before each candidate anchor is evicted
3. **Before demotion** -- evaluated when `AnchorEngine.demote()` is called
4. **Before authority change** -- evaluated when any authority transition is about to occur (both promotion and demotion paths)

Evaluation SHALL occur before the state change is committed. If a MUST-strength invariant is violated, the state change SHALL NOT be committed and the lifecycle operation SHALL skip the violating action (e.g., skip evicting a protected anchor and try the next candidate).

#### Scenario: Protected anchor skipped during eviction

- **GIVEN** a MUST-strength `ANCHOR_PROTECTED` invariant for anchor "A1" and the budget is full
- **WHEN** "A1" is the lowest-ranked non-pinned anchor and a new anchor is being promoted
- **THEN** "A1" SHALL be skipped and the next-lowest-ranked non-pinned, non-protected anchor SHALL be evicted instead

#### Scenario: All eviction candidates protected

- **GIVEN** MUST-strength `ANCHOR_PROTECTED` invariants for all non-pinned anchors and the budget is full
- **WHEN** a new anchor is promoted
- **THEN** the budget SHALL temporarily exceed the limit, a WARN-level log SHALL be emitted indicating invariant protection prevented eviction, and the anchor SHALL still be promoted

#### Scenario: Invariant blocks demotion

- **GIVEN** an `AUTHORITY_FLOOR` invariant with `strength = MUST` requiring anchor "A1" to remain at RELIABLE or above
- **WHEN** `demote("A1", DemotionReason.RANK_DECAY)` is called
- **THEN** the demotion SHALL be blocked, the anchor SHALL remain at RELIABLE, and an `InvariantViolation` event SHALL be published
