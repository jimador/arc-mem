...
The repository SHALL include a canonical DICE integration surface document at `docs/dev/dice-integration.md` that explains how context units composes with DICE 0.1.0-SNAPSHOT for proposition extraction, revision, persistence, and incremental analysis.
...
- **THEN** it SHALL describe the event-driven path: `ConversationAnalysisRequestEvent` -> `ConversationPropositionExtraction` -> `PropositionIncrementalAnalyzer` -> `PropositionPipeline` -> `ContextUnitRepository` -> `UnitPromoter`
...
- **THEN** it SHALL describe the synchronous path: `SimulationExtractionService.extract()` -> `PropositionPipeline.process()` -> `PropositionResults.persist()` with fallback -> `UnitPromoter`
...
The document SHALL describe each DICE component used by context units with method signatures and configuration.
...
The document SHALL catalog all known fragile coupling points between context units and DICE 0.1.0-SNAPSHOT with risk assessment and mitigation strategy.
...
- **WHEN** the fragile coupling section is reviewed
- **THEN** it SHALL document the `ArcMemChunkHistoryStore` interface and `InMemoryArcMemChunkHistoryStore` implementation
...
- **AND** SHALL reference `ArcMemChunkHistoryStore.java:19-42` for the interface
- **AND** SHALL reference `InMemoryArcMemChunkHistoryStore.java:38-44` for the clearing limitation
...
- **AND** SHALL note that these parameters are configurable via `ArcMemProperties.memory()` but their semantics are undocumented upstream
...
The document SHALL clearly distinguish which behaviors are implemented in context units vs. provided by upstream DICE components.

#### Scenario: Local vs. upstream behavior is unambiguous

- **WHEN** a developer reads the responsibility boundaries section
- **THEN** it SHALL state what context units implements: context unit lifecycle (rank, authority, promotion, eviction), trust pipeline (multi-gate evaluation), conflict detection and resolution, context assembly and prompt injection, budget enforcement
...
- **AND** SHALL identify extension points: `PropositionRepository` SPI (implemented by `ContextUnitRepository`), `ChunkHistoryStore` SPI (wrapped by `ArcMemChunkHistoryStore`), extraction templates (customizable Jinja)
