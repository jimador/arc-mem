...
- [ ] 2.1 Document chat extraction path: `ConversationAnalysisRequestEvent` -> `ConversationPropositionExtraction.onConversationExchange()` (line 83) -> `PropositionIncrementalAnalyzer.analyze()` (line 103) -> `ChunkPropositionResult` -> `PropositionResults.persist()` with fallback (lines 140-160) -> `ContextUnitRepository.assignContextIds()` (line 165) -> `UnitPromoter.evaluateAndPromote()` (line 170)
- [ ] 2.2 Document simulation extraction path: `SimulationExtractionService.extract()` (line 66) -> `PropositionPipeline.process()` (line 79) -> `PropositionResults.persist()` with fallback (lines 93-105) -> `ContextUnitRepository.assignContextIds()` (line 113) -> `ContextUnitRepository.tagSourceIds()` (line 120) -> `UnitPromoter.batchEvaluateAndPromoteWithOutcome()` (line 123)
...
- [ ] 5.1 Document `ArcMemChunkHistoryStore` interface at `ArcMemChunkHistoryStore.java:19-42`; three methods: `delegate()`, `clearByContext()`, `clearAll()`
- [ ] 5.2 Document `InMemoryArcMemChunkHistoryStore` implementation at `InMemoryArcMemChunkHistoryStore.java:22-51`
...
- [ ] 6.1 Document `WindowConfig` construction at `ConversationPropositionExtraction.java:68-72` with parameters from `ArcMemProperties.memory()`
...
**Verification**: WindowConfig parameters match `ArcMemProperties.memory()` fields.
...
- [ ] 8.1 Document what context units implements: context unit lifecycle (rank, authority, promotion, eviction), trust pipeline (multi-gate evaluation), conflict detection and resolution (composite detector), context assembly and prompt injection (ArcMemLlmReference), budget enforcement, decay and reinforcement policies
...
- [ ] 8.3 Document extension points: `PropositionRepository` SPI (implemented by `ContextUnitRepository`), `ChunkHistoryStore` SPI (wrapped by `ArcMemChunkHistoryStore`), extraction templates (customizable Jinja at `dice/extract_dnd_propositions`)
