...
The system SHALL represent pending CANON authority transitions as `CanonizationRequest` records containing: a unique request ID, context unit ID, context ID, snapshot of context unit text, current authority, requested authority, reason string, requester identifier, creation timestamp, and status (PENDING, APPROVED, REJECTED).
...
- `reject(requestId, rejectionReason)` — marks the request REJECTED without modifying the context unit
...
- **GIVEN** a PENDING canonization request for context unit "A1" requesting CANON
- **WHEN** a human approves the request
- **THEN** the context unit's authority is set to CANON, the request status becomes APPROVED, and an `AuthorityChanged` lifecycle event is published with direction PROMOTED

#### Scenario: Reject canonization request

- **GIVEN** a PENDING canonization request for context unit "A1" requesting CANON
- **WHEN** a human rejects the request with reason "Not established enough"
- **THEN** the context unit's authority remains unchanged, the request status becomes REJECTED, and the rejection reason is recorded

#### Scenario: Approve decanonization request

- **GIVEN** a PENDING decanonization request for context unit "A1" requesting demotion from CANON to RELIABLE
- **WHEN** a human approves the request
- **THEN** the context unit's authority is set to RELIABLE, the request status becomes APPROVED, and an `AuthorityChanged` lifecycle event is published with direction DEMOTED
...
When `approve()` is called, the `CanonizationGate` SHALL verify that the context unit's current authority still matches the `currentAuthority` recorded in the `CanonizationRequest`. If the context unit's authority has changed since the request was created (e.g., demoted by conflict resolution while the request was pending), the request SHALL be rejected as stale.

This prevents a pending canonization from promoting an context unit that has been demoted since the request was created (e.g., RELIABLE → UNRELIABLE via conflict, then approved canonization would jump UNRELIABLE → CANON, bypassing the gate's intent).

#### Scenario: Approve stale request rejected

- **GIVEN** a PENDING canonization request for context unit "A1" with currentAuthority RELIABLE
- **AND** context unit "A1" has since been demoted to UNRELIABLE (via conflict resolution)
- **WHEN** a human approves the request
- **THEN** the approval is rejected with a descriptive error indicating the context unit's authority has changed
...
- **GIVEN** a PENDING canonization request for context unit "A1" with currentAuthority RELIABLE
- **AND** context unit "A1" is still at RELIABLE authority
- **WHEN** a human approves the request
- **THEN** the authority change executes normally

### Requirement: Canonization request idempotency

The `CanonizationGate` SHALL prevent duplicate pending requests for the same context unit. If a pending request already exists for an context unit, `requestCanonization()` SHALL return the existing request ID rather than creating a new request.

Approved and rejected requests do not block new requests for the same context unit.

#### Scenario: Duplicate pending request returns existing ID

- **GIVEN** a PENDING canonization request exists for context unit "A1"
- **WHEN** another canonization request is created for context unit "A1"
- **THEN** the existing request ID is returned and no new request is created

#### Scenario: New request after rejection allowed

- **GIVEN** a REJECTED canonization request exists for context unit "A1"
- **WHEN** a new canonization request is created for context unit "A1"
...
- When `promote()` is called with CANON authority: the context unit SHALL be promoted at RELIABLE (the highest auto-assignable level) and a pending canonization request SHALL be created.
- When `demote()` targets a CANON context unit: the context unit SHALL remain at CANON and a pending decanonization request SHALL be created.
- Seed context units defined in scenario YAML files with `authority: CANON` SHALL bypass the gate (the human authored the scenario, so approval is implicit).

#### Scenario: Promote to CANON intercepted by gate

- **GIVEN** the canonization gate is enabled
- **WHEN** a caller requests promotion of context unit "A1" to CANON authority
- **THEN** the context unit is promoted at RELIABLE authority AND a PENDING canonization request is created for the CANON transition

#### Scenario: Demote from CANON intercepted by gate

- **GIVEN** the canonization gate is enabled and context unit "A1" is at CANON authority
- **WHEN** conflict resolution returns DEMOTE_EXISTING for context unit "A1"
- **THEN** the context unit remains at CANON AND a PENDING decanonization request is created

#### Scenario: Seed context units bypass gate

- **GIVEN** a simulation scenario YAML defines a seed context unit with `authority: CANON`
- **WHEN** the simulation engine initializes seed context units
- **THEN** the context unit is created at CANON authority without a pending request

#### Scenario: Gate disabled — promotion to CANON executes immediately

- **GIVEN** `context units.context unit.canonization-gate-enabled` is set to `false`
...
- **GIVEN** `context units.context unit.canonization-gate-enabled` is set to `false` and context unit "A1" is at CANON authority
- **WHEN** a demotion from CANON is requested (via conflict resolution, manual tool call, or trust degradation)
- **THEN** the context unit is demoted to RELIABLE immediately without creating a pending request
...
In simulation mode, the canonization gate SHALL auto-approve pending requests by default. This is configurable via `context units.context unit.auto-approve-in-simulation` (default: `true`).
...
- **GIVEN** `context units.context unit.auto-approve-in-simulation` is set to `false`
...
- `context units.context unit.canonization-gate-enabled` (boolean, default: `true`) — enables or disables the HITL gate for CANON transitions
- `context units.context unit.auto-approve-in-simulation` (boolean, default: `true`) — auto-approves CANON transitions in simulation contexts
