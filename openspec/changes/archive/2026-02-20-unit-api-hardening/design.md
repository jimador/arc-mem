...
context units is a working demo of adversarial drift resistance using Context Units -- enriched DICE Propositions with rank, authority, and budget management. The codebase has ~50 source files across 6 packages (`context unit/`, `assembly/`, `extract/`, `persistence/`, `chat/`, `sim/`) with 27 passing tests.

The current implementation works: context units resist adversarial prompt drift in both chat and simulation modes. But the APIs grew organically around a proof-of-concept. Javadoc coverage is inconsistent (0% on `Authority.java`, 80% on `ArcMemEngine`), SPI interfaces lack formal contracts, computed values go unused (trust authority ceiling), failures are silently swallowed, and the DICE integration surface is narrow (only extraction, not revision classifications or importance scores).
...
```
context unit/                    # Core model + engine + SPIs
  Context Unit.java              # Record: id, text, rank, authority, pinned, confidence, reinforcementCount, trustScore
  ArcMemEngine.java        # Facade: inject, promote, reinforce, applyDecay, archive, conflict ops
  ...
  ArcMemConfiguration.java # Spring beans: detector, resolver, policy defaults
  event/                   # Sealed lifecycle event hierarchy

assembly/                  # Prompt injection + budget management
  ArcMemLlmReference.java       # Cached context unit provider for system prompt
  ...
  ArcMemContextLock.java         # Mutual exclusion during prompt assembly
...
extract/                   # Proposition → context unit promotion pipeline
  UnitPromoter.java      # Multi-gate pipeline: confidence → dedup → conflict → trust → promote
...
persistence/               # Neo4j via Drivine
  ContextUnitRepository.java    # PropositionRepository impl (DICE contract) + context unit ops
  PropositionNode.java     # Neo4j entity: DICE fields + context unit fields
  PropositionView.java     # Drivine GraphView: proposition + mentions

chat/                      # Embabel agent integration
  ChatActions.java         # @EmbabelComponent: respond to UserMessage, inject context units, reinforce
  ContextTools.java         # @MatryoshkaTools: queryFacts, listUnits, pinFact, unpinFact
...
- **G2**: Authority lifecycle is bidirectional -- context units can be promoted AND demoted based on evidence. Trust ceiling is enforced on both promotion and re-evaluation. The old "upgrade-only" invariant is replaced with clear rules for when each direction is valid.
...
- **G5**: DICE importance and decay signals inform context unit priority decisions.
...
- **G10**: `ArcMemContextLock` concurrency is cleaned up (eliminate redundant volatile).
...
- **NG1**: Decomposing `ArcMemEngine` into multiple services. The engine is ~280 lines with clear internal sections. Splitting it would scatter related logic and force consumers to know which service to call.
- **NG2**: Introducing `UnitBlock` (named memory regions) or `UnitMemoryProvider` SPI. These are documented as future directions in the proposal; implementing them now would add structural complexity without clear benefit at current scale.
- **NG3**: Positional-aware prompt assembly (placing context units at cognitively optimal positions). Requires deeper control over Embabel's prompt rendering pipeline than we currently have.
- **NG4**: Memory pressure monitoring. The current budget enforcement (count-based + optional token budget) is sufficient.
- **NG5**: Expanding LLM-callable tools for self-directed context unit management (LLM-initiated promotions, archival suggestions). Current tools (query, list, pin, unpin) are sufficient for the demo.
...
### D1: ArcMemEngine Stays as One Class -- Clean Internal Organization

**Decision**: Keep `ArcMemEngine` as a single facade. Add section-level Javadoc comments organizing methods into logical groups: Injection, Lifecycle (promote/reinforce/archive), Budget, Conflict Operations, and Internal Helpers.
...
**Alternative considered**: Split into ContextUnitLifecycleService + ContextUnitBudgetManager + ContextUnitQueryService. Rejected because it introduces 3 new files, 3 new constructor injection sites, and cross-service calls (promote needs budget check) without reducing complexity.

> **Future direction**: If ArcMemEngine grows beyond ~400 lines or gains new responsibility categories (e.g., batch operations, scheduled maintenance), decompose along the section boundaries documented in class Javadoc.
...
- `ArcMemEngine.nextAuthority()` -- only returns the next level UP, caps at RELIABLE
- `ContextUnitRepository.upgradeAuthority()` -- Cypher `WHERE newLevel > currentLevel` silently ignores downgrades
...
| Invariant | Rule |
|-----------|------|
| **A3a** | CANON is never assigned by automatic promotion. Only explicit action (seed context units, manual tool call) can set CANON. |
| **A3b** | CANON demotion requires explicit action (conflict resolution with REPLACE, or manual tool call). CANON context units are immune to decay-based demotion. |
| **A3c** | Automatic demotion (via decay or trust re-evaluation) can demote RELIABLE → UNRELIABLE → PROVISIONAL. |
| **A3d** | Pinned context units are immune to automatic demotion (same as eviction immunity). Explicit demotion still works. |
...
| Manual / seed | Any level, including CANON | `ContextTools` or scenario seed context units |

**Demotion triggers** (downward transitions):

| Trigger | Scope | Mechanism |
|---------|-------|-----------|
| **Conflict resolution** | Any level including CANON | When `ConflictResolver` returns `DEMOTE_EXISTING` for an incoming proposition that contradicts an existing context unit, the existing context unit's authority is demoted one level and the incoming proposition is promoted. If already PROVISIONAL, the existing context unit is archived. For CANON context units, demotion is routed through the `CanonizationGate` (D3). |
| **Trust re-evaluation** | RELIABLE→UNRELIABLE, UNRELIABLE→PROVISIONAL | Event-driven re-evaluation via `TrustPipeline` (triggered by conflict resolution, reinforcement milestones, or explicit request — see unit-trust spec). If an context unit's trust score drops below the threshold for its current authority level, demote. |
| **Decay-based demotion** | RELIABLE→UNRELIABLE, UNRELIABLE→PROVISIONAL (not CANON) | When rank decays below authority-specific thresholds. A RELIABLE context unit whose rank drops below 400 demotes to UNRELIABLE. An UNRELIABLE context unit whose rank drops below 200 demotes to PROVISIONAL. |
| **Manual demotion** | Any level including CANON | `ContextTools.demoteUnit()` (new LLM tool) or explicit API call |
...
2. **`ArcMemEngine`** -- Add `demote(String unitId, DemotionReason reason)` method:
   - Looks up current authority
   - Computes `previousLevel()`
   - If already PROVISIONAL, archives instead of demoting
   - Calls new `repository.setAuthority(unitId, newAuthority)` (replaces the upgrade-only `upgradeAuthority`)
   - Publishes `AuthorityChanged` event (replaces the upgrade-only `AuthorityUpgraded`)

3. **`ContextUnitRepository`** -- Replace `upgradeAuthority()` with `setAuthority()`:
...
4. **`ConflictResolver`** -- Add `DEMOTE` resolution option:
   ...
   ```
   The `byAuthority()` factory updates: when incoming evidence conflicts with an context unit but the context unit has higher authority, instead of unconditionally keeping the existing, consider demotion if the incoming evidence is strong (high confidence + corroboration).

5. **`DecayPolicy`** -- Add authority demotion thresholds:
   - The decay policy already calculates new rank. Add a method `shouldDemoteAuthority(Context Unit context unit, int newRank)` that returns true when rank drops below authority-specific thresholds.
...
6. **Lifecycle events** -- Rename `AuthorityUpgraded` to `AuthorityChanged`:
   ```java
   record AuthorityChanged(String unitId, Authority previousAuthority,
...
**Trust ceiling enforcement** (originally D2 -- now part of this decision): `TrustScore.authorityCeiling` is enforced in `UnitPromoter` when determining initial authority. The ceiling also applies during trust re-evaluation: if an context unit's trust is re-evaluated and the new ceiling is below current authority, the context unit is demoted to match.

**Where trust ceiling is stored**: The `Context Unit` record's `trustScore` field (currently always null) MUST be populated on promotion and updated on re-evaluation.

**Alternative considered**: Keep upgrade-only, add a separate "revoke" mechanism that archives and re-creates at lower authority. Rejected because it loses the context unit's identity (ID, reinforcement history, pin status) and creates a confusing lifecycle where "the same fact" appears as two different context units.

**Alternative considered**: Allow demotion without any guard on CANON. Rejected because CANON context units serve as the security layer (MUST-comply). Automatic demotion of security constraints would undermine the system's core purpose. CANON demotion is deliberately explicit-only (invariant A3b).

### D3: Human-in-the-Loop Gate for Canonization / Decanonization

**Decision**: All transitions to or from CANON authority pass through a **human approval gate**. The system creates a pending `CanonizationRequest` that a human must approve or reject before the authority change takes effect. This applies to both directions: promoting an context unit to CANON ("canonization") and demoting a CANON context unit ("decanonization").

**Rationale**: CANON is the security tier -- context units at this level carry MUST-comply weight in the prompt. Granting or revoking that status should never happen without a human in the loop, regardless of how the request originated (LLM tool call, conflict resolution, manual API). This is the "you can't have real freedom without boundaries" principle applied to the system itself: even the system's own judgment about what deserves CANON status is bounded by human oversight.

**New types**:

```java
/**
 * A pending request to change an context unit's authority to or from CANON.
...
record CanonizationRequest(
    ...
    String unitId,        // target context unit
    String contextId,       // context for filtering pending requests
    String unitText,      // snapshot of context unit text (for display)
...
public class CanonizationGate {
    ...
    public String requestCanonization(String unitId, String contextId, Authority current, Authority requested, String reason, String requestedBy);

    /** Approve a pending request. Executes the authority change via ArcMemEngine. */
...
1. **`ArcMemEngine.promote()`**: If the caller requests CANON authority (e.g., via `ContextTools.canonizeUnit()`), the engine delegates to `CanonizationGate` instead of setting authority directly. The context unit is promoted at RELIABLE (the highest auto-assignable level) and a pending canonization request is created.

2. **`ArcMemEngine.demote()`**: If the target context unit is CANON, the engine delegates to `CanonizationGate`. The context unit stays at CANON until the human approves the demotion. This means conflict resolution against a CANON context unit creates a pending decanonization rather than immediately demoting.

3. **Seed context units**: Scenario YAML files can set `authority: CANON` on seed context units. These bypass the gate (the human authored the scenario file, so approval is implicit). Document this exception clearly.
...
**Stale request validation**: When `approve()` is called, the gate SHALL verify that the context unit's current authority still matches `request.currentAuthority`. If the context unit's authority has changed since the request was created (e.g., demoted by conflict resolution while pending), the request is rejected as stale with a descriptive error. This prevents a pending canonization from jumping an context unit from UNRELIABLE to CANON if it was demoted from RELIABLE while the request was pending.
...
**Alternative considered**: Just make CANON changes require a special API key or admin role. Rejected because authorization is not the same as approval. The question isn't "are you allowed to?" but "should this specific context unit really be CANON?"

### D4: Eviction Events Added to Sealed Lifecycle Hierarchy

**Decision**: Add `ContextUnitLifecycleEvent.Evicted(String unitId, String contextId, int previousRank)` to the sealed event hierarchy. Publish from `ArcMemEngine` when budget eviction removes an context unit.

**Implementation**: The current eviction path is in `ArcMemEngine.promote()` which calls `repository.evictLowestRanked(contextId, budget)`. This repository method returns the count of evicted context units but not their IDs. Two options:

1. **Query before evict**: Before calling `evictLowestRanked()`, query for the context units that would be evicted (lowest-ranked, non-pinned, exceeding budget), publish events for each, then execute the eviction.
2. **Change repository to return evicted IDs**: Modify `evictLowestRanked()` to return `List<String>` (evicted context unit IDs) instead of `int`.
...
sequenceDiagram
    participant Caller
    participant ArcMemEngine
    participant ContextUnitRepository
    participant EventPublisher

    Caller->>ArcMemEngine: promote(propositionId, rank)
    ArcMemEngine->>ContextUnitRepository: promoteToUnit(...)
    ArcMemEngine->>EventPublisher: publish(Promoted event)
    ArcMemEngine->>ContextUnitRepository: evictLowestRanked(contextId, budget)
    ContextUnitRepository-->>ArcMemEngine: List<EvictedUnitInfo> (id, rank)
    loop each evicted
        ArcMemEngine->>EventPublisher: publish(Evicted event)
    end
```
...
**Integration in ArcMemConfiguration**: Add `"dice"` as a new strategy option alongside `"lexical"`, `"hybrid"`, and `"llm"`. The `"dice"` strategy uses the DICE revision API directly.
...
> **Implementation note — BLOCKED on Q1**: This design assumes DICE's `LlmPropositionReviser` can be called outside the extraction pipeline to compare incoming text against existing context units. If Q1 investigation reveals this is not supported, the `DiceRevisionConflictDetector` SHALL NOT be implemented. Instead: (1) keep the current `LlmConflictDetector` with improved documentation and logging, (2) document the DICE revision API limitation as a future integration point, and (3) remove the `"dice"` strategy option. The lexical `NegationConflictDetector` and existing `LlmConflictDetector` remain unaffected regardless of Q1's outcome.

### D6: DICE Importance and Decay Alignment

**Decision**: Integrate DICE's `decay` field into activation score decay calculations. Currently `ExponentialDecayPolicy` uses a fixed 24-hour half-life independent of the proposition's DICE decay signal.
...
- **Default value**: `1.0` (not `0.0`), preserving current decay behavior for all existing context units that lack DICE decay data

This requires passing the DICE decay value to the decay policy. Currently `DecayPolicy.applyDecay(Context Unit, long)` only receives the `Context Unit` record and hours since reinforcement. The `Context Unit` record does not carry the DICE decay field. Two approaches:

1. **Add decay field to Context Unit record**: Natural place, but adds a field to every Context Unit for a value only used during decay calculation.
...
**Choice**: Option 1 (add `diceDecay` field to Context Unit record). The field is cheap (one double), makes the Context Unit self-contained, and avoids coupling the policy to persistence. The `ArcMemEngine.toUnit()` method already reads all `PropositionNode` fields -- adding one more is trivial.
...
3. Propagate it to the `Context Unit` record
...
**Decision**: `ArcMemLlmReference` and `PropositionsLlmReference` listen for `ContextUnitLifecycleEvent` instances and invalidate their cached content automatically.

**Current problem**: Callers must manually call `refresh()` to invalidate the cache. If they forget (or if an context unit is modified by a background process like decay), the prompt assembly uses stale data.

**Implementation**: Both reference classes become Spring `@EventListener` targets. Any `ContextUnitLifecycleEvent` (Promoted, Reinforced, Archived, Evicted, AuthorityUpgraded, ConflictResolved) triggers cache invalidation for the matching contextId.

**Complication**: `ArcMemLlmReference` is currently created as a local variable in `ChatActions.respond()`, not as a Spring bean. It can't receive events via `@EventListener` without being registered in the application context.
...
2. **Delegate invalidation to a separate Spring bean**: Create `ContextUnitCacheInvalidator` as a singleton Spring bean that listens for events and maintains a set of "dirty" context IDs. `ArcMemLlmReference` checks with the invalidator before using cached data.
3. **Keep manual refresh, improve documentation**: Document when refresh is needed and why.

**Choice**: Option 2 (`ContextUnitCacheInvalidator` bean). Clean separation: the invalidator tracks dirty state per contextId, and the reference classes check it before using cached data. No lifecycle changes to existing code, just an additional check in `ensureUnitsLoaded()`.

```mermaid
flowchart TD
    A[ContextUnitLifecycleEvent] -->|@EventListener| B[ContextUnitCacheInvalidator]
    B -->|marks dirty| C[dirtyContextIds Set]
    D[ArcMemLlmReference.ensureUnitsLoaded] -->|checks| C
    C -->|is dirty| E[Reload context units]
...
**Decision**: Change `ContextUnitRepository.findPropositionNodeById()` from returning `@Nullable PropositionNode` to returning `Optional<PropositionNode>`. Callers update from null-checks to `Optional` handling.
...
**Callers affected**:
- `ArcMemEngine.promote()` -- currently `if (proposition == null) { log.warn(...); return; }`
- `ArcMemEngine.reinforce()` -- same pattern
- `ArcMemEngine.applyDecay()` -- same pattern
- `ArcMemEngine.archive()` -- same pattern
- `ContextTools.pinFact()` / `unpinFact()` -- same pattern
...
**Decision**: Add a `@PostConstruct` validation method in `ArcMemConfiguration` that checks:
- `context unit.budget > 0`
- `context unit.autoActivateThreshold` in [0.0, 1.0]
- `context unit.minRank < context unit.maxRank`
- `context unit.initialRank` in [minRank, maxRank]
...
**Impact**: `ArcMemConfiguration.reinforcementPolicy()` calls `ReinforcementPolicy.threshHold()`. One call site. Simple rename.

### D12: ArcMemContextLock Cleanup
...
**[R1] DICE revision API may not support standalone comparison** → If `LlmPropositionReviser` can only be used during the extraction pipeline (not for comparing arbitrary text against existing context units), the `DiceRevisionConflictDetector` (D5) cannot be implemented as designed.
...
**[R2] Bidirectional authority changes the security model** → The current "upgrade-only" rule was a simplicity safeguard. Allowing demotion means an adversary could theoretically trigger a conflict that demotes a protective context unit.
→ **Mitigation**: CANON context units are immune to automatic demotion (invariant A3b). Only explicit action can demote CANON. Pinned context units are immune to automatic demotion (invariant A3d). The demotion path requires conflict resolution to return DEMOTE_EXISTING, which the `byAuthority()` resolver only does for context units below RELIABLE authority when incoming evidence has high confidence. RELIABLE+ context units defend by default.

**[R3] Adding `diceDecay` field to Context Unit record (D6) changes the record constructor** → All call sites that construct `Context Unit` records need updating.
→ **Mitigation**: The only construction site is `ArcMemEngine.toUnit()` (package-private). No external callers construct `Context Unit` directly. Low blast radius.

**[R4] Event-driven cache invalidation (D7) adds a new Spring bean** → `ContextUnitCacheInvalidator` becomes a singleton that all `ArcMemLlmReference` instances depend on.
...
→ **Mitigation**: Limited to ~5 call sites, all in `ArcMemEngine` and `ContextTools`. Mechanical refactor.
...
### Context Unit Provenance / Chain of Custody

User trust in any agentic system depends on the user being able to understand _why the system believes what it believes_. Context Units are the system's working memory — the beliefs it uses to constrain LLM behavior. If the user cannot trace where an context unit came from, how it earned its authority, and what reinforced or challenged it, the system is a black box.

**The vision**: Every context unit should carry a traceable provenance chain — a "chain of custody" that answers:
...
- **Promotion path**: When was it promoted to context unit status? What was its initial confidence? What trust score justified the promotion?
...
- **Reinforcement log**: What interactions reinforced this context unit? How many times? With what confidence each time?
- **Conflict record**: Was this context unit ever challenged? By what incoming proposition? How was the conflict resolved and why?
- **Canonization audit**: If CANON, who approved it? When? If decanonized, who approved and what was the reason?

**Why this matters now**: The hardening work in this change (lifecycle events, `AuthorityChanged` with direction, `DemotionReason` enum, `CanonizationRequest` records) creates the event infrastructure needed to build provenance. Every significant state change now publishes a typed event with structured context. A future `UnitProvenanceService` could subscribe to these events and build an append-only audit trail per context unit.
...
├──────────────────────────────────────────────────────────┤
│                                                          │
│  Context Unit "Water boils at 100°C"                          │
...
**Surfacing to users**: The provenance chain should be accessible from both the chat UI and the simulation UI. In chat, the LLM could reference it when explaining its reasoning ("I believe X because it was reinforced 5 times with high confidence"). In simulation, the inspector panel could show the full audit trail for any context unit. The existing `EntityMentionNetworkView` graph visualization is a natural home for this — see "Graph Display" below.
...
1. The graph is the natural UI for context unit provenance visualization (see above)
2. The hardening work improves the data model the graph would consume (typed events, authority history, conflict records)
3. When fixed, the graph should show context unit relationships — not just entity mentions — with visual encoding for authority level (color/size), provenance chains (directed edges), and conflict history (red edges)
...
**Resolution required before implementation.** The unit-conflict spec's `DiceRevisionConflictDetector` requirement is conditional on Q1. If Q1 is "no," the requirement and its scenarios are dropped.
...
**Q4**: For the canonization gate UI (D3), should pending requests block the chat turn (synchronous -- human must approve before LLM responds) or be asynchronous (context unit promoted at RELIABLE immediately, CANON upgrade applied later when approved)? Leaning toward asynchronous: the context unit is useful at RELIABLE while waiting, and blocking the chat on human approval would degrade the conversational experience.
