# Anchor Maintenance Strategy -- Delta Spec (wire-simulation-pipeline)
...
- `activeAnchors` from the post-reinforcement anchor state
...
- **GIVEN** a simulation turn that reinforces 3 anchors
- **WHEN** `buildResult()` completes the reinforcement loop
- **THEN** `MaintenanceStrategy.onTurnComplete()` MUST be called exactly once
- **AND** the `MaintenanceContext` MUST contain the post-reinforcement anchor list
...
All invariants from the base `anchor-maintenance-strategy` spec remain in effect. Additionally:
