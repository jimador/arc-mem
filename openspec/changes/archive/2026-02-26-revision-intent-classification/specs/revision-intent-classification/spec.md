...
- `REVISION` — evaluate authority eligibility; if eligible, return `REPLACE` (routed to `AnchorEngine.supersede()` with `SupersessionReason.USER_REVISION`); if not eligible, delegate to `AuthorityConflictResolver`
- `CONTRADICTION` or null — delegate to `AuthorityConflictResolver` (existing behavior)
- `WORLD_PROGRESSION` — return `COEXIST` (both facts valid at different points in time)
...
#### Scenario: Revision of PROVISIONAL anchor accepted

- **GIVEN** a conflict with `conflictType = REVISION` against a PROVISIONAL anchor
...
#### Scenario: Revision of RELIABLE anchor accepted when configured

- **GIVEN** `anchor.revision.reliable-revisable = true`
