...
The repository SHALL include a canonical DICE integration architecture document under `docs/dev` that explains how Context Units composes with DICE proposition extraction and lifecycle behavior.
...
The DICE integration documentation SHALL explicitly describe how Context Units relates to DICE Agent Memory. It SHALL state that Context Units augments DICE memory retrieval with a governed working set for prompt assembly, rather than replacing DICE memory primitives.

#### Scenario: Memory layering is explicit
- **GIVEN** the DICE integration documentation
- **WHEN** memory behavior is reviewed
- **THEN** the docs SHALL describe DICE retrieval surfaces and Context Units working-set injection behavior as separate layers
