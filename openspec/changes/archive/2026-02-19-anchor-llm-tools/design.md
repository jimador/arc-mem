...
Embabel Agent supports `@MatryoshkaTools` records with `@LlmTool` methods that the LLM can call during conversation. dice-anchors doesn't use this feature yet — `ChatActions` only uses `@Action` + `rendering()`. Adding anchor tools lets Bigby (the DM) actively query and manage knowledge, which is a deep integration point.

The codebase already has:
- `AnchorEngine` with `inject()`, `reinforce()`, `promote()`, `detectConflicts()`
- `AnchorRepository` with `findActiveAnchors()`, `semanticSearch()`, `findById()`
- `Anchor` record with `text()`, `rank()`, `authority()`, `pinned()`
...
**Goals:**
- Expose anchor query and management as LLM-callable tools
...
```java
@MatryoshkaTools
public record AnchorTools(AnchorEngine engine, AnchorRepository repository) {

    @LlmTool("Query established facts about a subject. Returns matching anchors with rank and authority.")
    public List<AnchorSummary> queryFacts(String subject) { ... }

    @LlmTool("Get all currently active anchors with their rank, authority, and confidence.")
...
| Tool | Purpose | Guards |
|------|---------|--------|
| `queryFacts(subject)` | Semantic search for anchors matching subject | Read-only. Uses `repository.semanticSearch()` |
| `listAnchors()` | List all active anchors | Read-only. Uses `engine.inject(contextId)` |
| `pinFact(anchorId)` | Pin anchor (immune to eviction) | MUST verify anchor exists. MUST NOT pin archived. |
| `unpinFact(anchorId)` | Unpin anchor | MUST verify anchor exists and is pinned. |
...
```java
record AnchorSummary(String id, String text, int rank, String authority, boolean pinned, double confidence) {}
...
```java
var tools = new AnchorTools(engine, repository);
context.ai()
    .withDefaultLlm()
    .withTools(tools)
...
- `pinFact`/`unpinFact`: Verify anchor exists, is active, and is not CANON (CANON is always treated as pinned)
...
| **LLM pins everything** | Pin is reversible. Budget enforcement still works (pinned anchors are immune to eviction but don't multiply). |
...
1. Create `AnchorSummary` and `PinResult` records
2. Create `AnchorTools` @MatryoshkaTools record
