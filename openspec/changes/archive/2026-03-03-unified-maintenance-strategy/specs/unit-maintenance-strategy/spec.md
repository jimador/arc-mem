# Context Unit Maintenance Strategy Specification

## ADDED Requirements

### REQ-INTERFACE: MaintenanceStrategy sealed interface

`MaintenanceStrategy` MUST be a sealed interface in the `context unit/` package. The interface MUST permit exactly three implementations: `ReactiveMaintenanceStrategy`, `ProactiveMaintenanceStrategy`, and `HybridMaintenanceStrategy`.
...
- **WHEN** the `context unit/` package is inspected
...
- **WHEN** `onTurnComplete` is called with a `MaintenanceContext` containing a contextId, activeUnits list, and turnNumber
...
The `onTurnComplete` method MUST perform lightweight per-turn bookkeeping (logging). Actual decay and reinforcement remain invoked by `ArcMemEngine` directly -- this strategy is a coordination layer, not a replacement for the engine's existing policy invocation.
...
### REQ-CONFIG: ArcMemProperties includes MaintenanceConfig

`ArcMemProperties` MUST include a `MaintenanceConfig` nested record with a `MaintenanceMode mode` field. The default mode MUST be `REACTIVE`.

#### Scenario: Default mode is REACTIVE
- **WHEN** the application starts without specifying `context units.maintenance.mode`
- **THEN** the effective maintenance mode MUST be `REACTIVE`

#### Scenario: Mode is configurable via properties
- **WHEN** `context units.maintenance.mode` is set to `HYBRID`
- **THEN** the effective maintenance mode MUST be `HYBRID`

### REQ-BEAN: ArcMemConfiguration produces MaintenanceStrategy bean

`ArcMemConfiguration` MUST define a `@Bean` method that produces a `MaintenanceStrategy` instance based on the configured `MaintenanceMode`:
...
- **I6**: All context unit invariants (A1-A4 per Article V of the constitution) MUST be preserved. The maintenance strategy coordinates scheduling; it MUST NOT modify rank clamping, budget enforcement, promotion rules, or authority transitions.
