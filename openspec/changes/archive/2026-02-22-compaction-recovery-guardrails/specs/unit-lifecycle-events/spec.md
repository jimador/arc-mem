...
Event publishing SHALL be gated by `context units.compaction.events-enabled` (default `true`). When disabled, no `CompactionCompleted` events SHALL be published.
...
- **GIVEN** `context units.compaction.events-enabled` is `false`
