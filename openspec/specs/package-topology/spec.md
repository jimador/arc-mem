...
`arcmem-core` SHALL organize code by ARC-Mem responsibilities rather than by a single legacy bucket or simulator-oriented names. The canonical bounded areas SHALL include configuration, memory, assembly, extraction, persistence, prompt infrastructure, and explicit SPI or boundary packages where needed.
