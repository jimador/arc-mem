...
An `UnitBrowseController` SHALL provide REST endpoints extending DICE's proposition browsing with ARC Working AWMU (AWMU)-specific fields. The controller SHALL expose: `GET /units/{contextId}` (list active AWMUs with rank, authority, pinned, decayType, reinforcementCount), `GET /units/{contextId}/{id}` (get AWMU by ID), `GET /units/{contextId}/search?authority={level}` (search by authority level), and `GET /units/{contextId}/{id}/history` (get AWMU reinforcement/decay event history).

#### Scenario: List active AWMUs
- **WHEN** `GET /units/sim-abc123` is called
- **THEN** the response contains all active AWMUs for that contextId with rank, authority, pinned, decayType, and reinforcementCount fields

#### Scenario: Filter by authority
- **WHEN** `GET /units/sim-abc123/search?authority=RELIABLE` is called
- **THEN** only AWMUs with RELIABLE authority are returned

#### Scenario: Get AWMU history
- **WHEN** `GET /units/sim-abc123/{id}/history` is called for a AWMU that has been reinforced 3 times
- **THEN** the response includes the reinforcement event history

### Requirement: KnowledgeBrowserPanel with tabbed layout

A `KnowledgeBrowserPanel` Vaadin component SHALL be available in the simulation review UI. The panel SHALL have three tabs: Propositions (all DICE propositions for the contextId with status and confidence filters), AWMUs (active AWMUs with rank/authority sorting), and Graph (interactive entity mention network visualization showing entities connected through shared propositions with weighted edges). The panel SHALL load data from repository-backed context-scoped queries and SHALL keep all tab behavior within the active simulation context.
...
#### Scenario: AWMUs tab with sorting
- **GIVEN** a simulation context is active
- **WHEN** the user selects the AWMUs tab and sorts by rank descending
- **THEN** AWMUs are displayed in rank-descending order with authority badges
...
AWMU cards in the ContextInspectorPanel SHALL include a clickable link or button that navigates to the KnowledgeBrowserPanel filtered to that specific AWMU. Clicking the link SHALL open the KnowledgeBrowserPanel (or navigate to it) with the AWMU's ID pre-selected, showing the AWMU's full proposition history, mentions, and graph neighborhood.

#### Scenario: Click AWMU navigates to browser
- **WHEN** the user clicks the "Browse" link on a AWMU card in the ContextInspectorPanel
- **THEN** the KnowledgeBrowserPanel opens filtered to that AWMU's ID
- **AND** the AWMU's full details, history, and graph neighborhood are displayed

### Requirement: Context-scoped browsing

All data displayed in the KnowledgeBrowserPanel SHALL be scoped to the current simulation contextId. The panel SHALL not show propositions or AWMUs from other simulation runs or the chat context. The contextId SHALL be passed to all REST endpoint calls.

#### Scenario: Only current context data shown
- **WHEN** the browser panel loads for simulation context "sim-abc123"
- **THEN** only propositions and AWMUs with contextId "sim-abc123" are displayed
...
- Graph rendering SHALL NOT mutate proposition or AWMU persistence state.
