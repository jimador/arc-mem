...
- **WHEN** `detect()` is called for incoming text against existing anchors
...
### Requirement: Intra-batch deduplication against existing anchors
...
2. Active anchors in the current context (cross-reference dedup, new)

Currently, batch dedup only filters within the incoming batch by text identity. The cross-reference against existing anchors prevents promoting a proposition that duplicates an anchor already in the context.

#### Scenario: Candidate duplicates existing anchor filtered

- **GIVEN** an active anchor with text "Water boils at 100°C" exists in context "ctx-1"
- **WHEN** a proposition with text "Water boils at 100 degrees Celsius" enters the promotion pipeline
- **THEN** the duplicate detector identifies it as a duplicate of the existing anchor and the proposition is filtered out

#### Scenario: Candidate unique across batch and existing anchors promoted

- **GIVEN** active anchors exist but none match the incoming proposition
...
**Modifies**: `AnchorPromoter` class-level Javadoc.

`AnchorPromoter` SHALL have comprehensive class-level Javadoc documenting:
...
- The relationship between the promoter and `AnchorEngine` (promoter decides what to promote, engine executes the promotion)

### Requirement: AnchorTools documentation and demote tool

**Modifies**: `AnchorTools` (@MatryoshkaTools).

Each `@LlmTool` method in `AnchorTools` SHALL have a comprehensive `@LlmTool(description=...)` that clearly documents:
- What the tool does
- What parameters it accepts
- What guardrails apply (e.g., "cannot modify authority")
- When the LLM should and should not use the tool

The system SHALL add a `demoteAnchor(String anchorId, String reason)` tool that allows the LLM to request demotion of an anchor's authority by one level. The tool SHALL:
- Call `AnchorEngine.demote()` with `DemotionReason.MANUAL`
- Return a result indicating the new authority level
- For CANON anchors: create a pending decanonization request via `CanonizationGate` instead of immediate demotion

#### Scenario: LLM demotes RELIABLE anchor

- **GIVEN** the LLM identifies anchor "A1" at RELIABLE authority as potentially outdated
- **WHEN** the LLM calls `demoteAnchor("A1", "Evidence suggests this fact is outdated")`
- **THEN** the anchor's authority is demoted to UNRELIABLE

#### Scenario: LLM attempts to demote CANON anchor

- **GIVEN** anchor "A1" is at CANON authority
- **WHEN** the LLM calls `demoteAnchor("A1", "Conflicting information found")`
...
#### Scenario: LLM demotes non-existent anchor

- **GIVEN** no anchor exists with ID "missing"
- **WHEN** the LLM calls `demoteAnchor("missing", "reason")`
- **THEN** the tool returns an error message indicating the anchor was not found
