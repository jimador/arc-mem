...
The system SHALL compute a novelty score in [0.0, 1.0] for each proposition evaluated by the trust pipeline. Novelty measures information gain relative to existing active ARC Working Memory Units (AWMUs) in the same context.
...
2. For each active AWMU in the context, tokenize its text into the same form.
3. Compute the Jaccard similarity coefficient between the proposition token set and each AWMU token set: `|intersection| / |union|`.
4. Take the maximum Jaccard similarity across all AWMUs as `maxSimilarity`.
5. Novelty score = `1.0 - maxSimilarity`.

A proposition identical to an existing AWMU scores 0.0 (no novelty). A proposition sharing no tokens with any AWMU scores 1.0 (maximum novelty).

When no active AWMUs exist in the context, novelty SHALL be 1.0 (all propositions are novel in an empty context).

#### Scenario: Proposition identical to existing AWMU

- **GIVEN** an active AWMU with text "The dragon guards the eastern gate"
...
- **GIVEN** active AWMUs about weather and geography
- **WHEN** a proposition "The artifact requires a blood sacrifice" is scored for novelty
- **THEN** the novelty score SHALL be 1.0 (no token overlap with existing AWMUs)

#### Scenario: Partial overlap

- **GIVEN** an active AWMU with text "The dragon breathes fire"
...
- **GIVEN** no active AWMUs exist in the context
...
3. Tokenize the active AWMU texts (representing accumulated conversation context) into a combined token set.
...
When no active AWMUs exist in the context, importance SHALL be 0.5 (neutral -- insufficient context to judge relevance).

#### Scenario: Proposition highly relevant to conversation

- **GIVEN** active AWMUs mentioning "dragon", "gate", "guardian", "eastern"
...
- **GIVEN** active AWMUs about a dungeon and its traps
...
- **GIVEN** a AWMU "The dragon is in the cave"
...
- **QS3**: Quality scoring MUST NOT modify proposition or AWMU state. Scoring is read-only.
