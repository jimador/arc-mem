...
The system SHALL compute a novelty score in [0.0, 1.0] for each proposition evaluated by the trust pipeline. Novelty measures information gain relative to existing active anchors in the same context.
...
2. For each active anchor in the context, tokenize its text into the same form.
3. Compute the Jaccard similarity coefficient between the proposition token set and each anchor token set: `|intersection| / |union|`.
4. Take the maximum Jaccard similarity across all anchors as `maxSimilarity`.
5. Novelty score = `1.0 - maxSimilarity`.

A proposition identical to an existing anchor scores 0.0 (no novelty). A proposition sharing no tokens with any anchor scores 1.0 (maximum novelty).

When no active anchors exist in the context, novelty SHALL be 1.0 (all propositions are novel in an empty context).

#### Scenario: Proposition identical to existing anchor

- **GIVEN** an active anchor with text "The dragon guards the eastern gate"
...
- **GIVEN** active anchors about weather and geography
- **WHEN** a proposition "The artifact requires a blood sacrifice" is scored for novelty
- **THEN** the novelty score SHALL be 1.0 (no token overlap with existing anchors)

#### Scenario: Partial overlap

- **GIVEN** an active anchor with text "The dragon breathes fire"
...
- **GIVEN** no active anchors exist in the context
...
3. Tokenize the active anchor texts (representing accumulated conversation context) into a combined token set.
...
When no active anchors exist in the context, importance SHALL be 0.5 (neutral -- insufficient context to judge relevance).

#### Scenario: Proposition highly relevant to conversation

- **GIVEN** active anchors mentioning "dragon", "gate", "guardian", "eastern"
...
- **GIVEN** active anchors about a dungeon and its traps
...
- **GIVEN** an anchor "The dragon is in the cave"
...
- Property: `dice-anchors.anchor.quality-scoring.enabled`
- Default: `false`
...
- **GIVEN** `dice-anchors.anchor.quality-scoring.enabled=true`
- **WHEN** a proposition is evaluated through the trust pipeline
...
- **QS3**: Quality scoring MUST NOT modify proposition or anchor state. Scoring is read-only.
