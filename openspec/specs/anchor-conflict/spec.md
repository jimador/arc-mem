...
The `ConflictResolver.Resolution` enum SHALL include a `DEMOTE_EXISTING` value alongside the existing `KEEP_EXISTING`, `REPLACE`, and `COEXIST` values. When `DEMOTE_EXISTING` is returned, the existing memory unit's authority is demoted one level (via `ArcMemEngine.demote()`) and the incoming proposition is promoted.
...
- The existing memory unit is below CANON authority
- The conflict evidence is strong enough to warrant demotion but not full replacement

For CANON memory units, the resolver SHALL never return `DEMOTE_EXISTING` directly. Instead, the engine delegates to the `CanonizationGate` for decanonization approval.
...
The effective thresholds SHALL incorporate tier-aware modifiers from `ConflictConfig.tier()`. For each resolution comparison, the effective threshold is computed as: `effectiveThreshold = baseThreshold + tierModifier`, where `tierModifier` is determined by the existing memory unit's current tier (HOT, WARM, or COLD). The modifier adjusts the threshold, not the incoming confidence score.

The confidence thresholds (default 0.8 for REPLACE, default 0.6 for DEMOTE_EXISTING) SHALL be sourced from `ConflictConfig.replaceThreshold()` and `ConflictConfig.demoteThreshold()` respectively, rather than from hardcoded values or `ArcMemProperties.UnitConfig.autoActivateThreshold`.

#### Scenario: Conflict with RELIABLE memory unit returns DEMOTE_EXISTING

- **GIVEN** an existing memory unit at RELIABLE authority
- **WHEN** a high-confidence incoming proposition contradicts the memory unit
- **THEN** the conflict resolver returns `DEMOTE_EXISTING`

#### Scenario: Conflict with CANON memory unit keeps existing

- **GIVEN** an existing memory unit at CANON authority
- **WHEN** an incoming proposition contradicts the memory unit
...
- **GIVEN** conflict resolution returns `DEMOTE_EXISTING` for memory unit "A1" at UNRELIABLE authority
- **WHEN** the engine processes the resolution
- **THEN** memory unit "A1" is demoted to PROVISIONAL, the incoming proposition is promoted, and both `AuthorityChanged` (DEMOTED) and `Promoted` lifecycle events are published
...
- **WHEN** an incoming proposition with confidence `0.85` contradicts the memory unit
...
#### Scenario: COLD memory unit resolved with permissive thresholds

- **GIVEN** an existing COLD memory unit at UNRELIABLE authority
- **AND** `demote-threshold` is `0.6` and `cold-defense-modifier` is `-0.1`
- **WHEN** an incoming proposition with confidence `0.55` contradicts the memory unit
...
When `AuthorityConflictResolver` produces a REPLACE outcome and the engine processes the resolution, the resolution flow SHALL, in addition to archiving the existing memory unit and promoting the incoming proposition:

1. Create a `SUPERSEDES` relationship from the incoming memory unit to the replaced memory unit with `reason = "CONFLICT_REPLACEMENT"` and `occurredAt = Instant.now()`
2. Set `validTo = Instant.now()` and `transactionEnd = Instant.now()` on the replaced memory unit
3. Set `supersededBy` on the replaced memory unit to the incoming memory unit's ID
4. Set `supersedes` on the incoming memory unit to the replaced memory unit's ID
...
- **GIVEN** an existing memory unit "A1" at CANON authority
...
- **GIVEN** an existing memory unit "A1" at RELIABLE authority
...
- **AND** "A1" temporal fields SHALL remain unchanged (the memory unit is still active, just at lower authority)
...
- **GIVEN** an incoming text that DICE classifies as CONTRADICTORY to existing memory unit "A1"
- **WHEN** the DICE revision conflict detector evaluates the pair
- **THEN** a `Conflict` is returned with the existing memory unit and high confidence

#### Scenario: SIMILAR classification produces low-confidence conflict

- **GIVEN** an incoming text that DICE classifies as SIMILAR to existing memory unit "A1"
- **WHEN** the DICE revision conflict detector evaluates the pair
...
- **GIVEN** an incoming text that DICE classifies as UNRELATED to existing memory unit "A1"
- **WHEN** the DICE revision conflict detector evaluates the pair
- **THEN** no conflict is returned

#### Scenario: IDENTICAL classification defers to dedup

- **GIVEN** an incoming text that DICE classifies as IDENTICAL to existing memory unit "A1"
- **WHEN** the DICE revision conflict detector evaluates the pair
...
**Modifies**: `unit.conflict-detection-strategy` configuration property.

The system SHALL add `"dice"` as a valid strategy value for the `unit.conflict-detection-strategy` property, alongside existing values `"lexical"`, `"hybrid"`, and `"llm"`. When `"dice"` is selected, the `DiceRevisionConflictDetector` is used.
...
- **GIVEN** `arc-mem.unit.conflict-detection-strategy` is set to `"dice"`
...
- **Invariants**: The detector SHALL NOT modify any memory unit state. The resolver SHALL NOT directly modify memory unit state; it returns a decision that the engine executes.
