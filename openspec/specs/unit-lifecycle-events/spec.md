...
The system SHALL add an `InvariantViolation` event type to the `MemoryUnitLifecycleEvent` sealed hierarchy. The event SHALL be published when an invariant is evaluated and found to be violated, regardless of whether the violation blocked the action or only produced a warning.
...
| `contextId` | String | Inherited from `MemoryUnitLifecycleEvent` |
| `occurredAt` | Instant | Inherited from `MemoryUnitLifecycleEvent` |

The `MemoryUnitLifecycleEvent` sealed interface `permits` clause SHALL be updated to include `InvariantViolation`.

A static factory method `MemoryUnitLifecycleEvent.invariantViolation(...)` SHALL be provided, consistent with the existing factory methods (`promoted(...)`, `reinforced(...)`, etc.).
...
- **WHEN** working-memory capacity enforcement attempts to evict memory unit "A1"
...
The existing `MemoryUnitLifecycleListener` SHALL handle `InvariantViolation` events with an `@EventListener` method. The listener SHALL:
...
- **WHEN** the `MemoryUnitLifecycleListener` handles the event
