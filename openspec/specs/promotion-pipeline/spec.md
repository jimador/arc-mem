...
The `AnchorPromoter` pipeline SHALL include a conflict pre-check gate positioned after the confidence gate and before the dedup gate. The pre-check gate SHALL use `ConflictIndex.getConflicts()` for O(1) lookup to determine whether an incoming proposition conflicts with any existing anchor at RELIABLE or CANON authority.

Propositions that conflict with at least one RELIABLE+ anchor in the index SHALL be filtered from the pipeline before the dedup gate. The pre-check MUST NOT invoke any LLM calls.

#### Scenario: Pre-check filters proposition conflicting with RELIABLE anchor

- **GIVEN** an active anchor "A" with authority RELIABLE in the `ConflictIndex`
- **AND** an incoming proposition "P" that has an indexed conflict with anchor "A"
...
#### Scenario: Pre-check filters proposition conflicting with CANON anchor

- **GIVEN** an active anchor "A" with authority CANON in the `ConflictIndex`
- **AND** an incoming proposition "P" that has an indexed conflict with anchor "A"
- **WHEN** "P" enters the promotion pipeline
- **THEN** the pre-check gate SHALL filter "P" before the dedup gate

#### Scenario: Pre-check passes proposition conflicting with PROVISIONAL anchor

- **GIVEN** an active anchor "A" with authority PROVISIONAL in the `ConflictIndex`
- **AND** an incoming proposition "P" that has an indexed conflict with anchor "A"
- **WHEN** "P" enters the promotion pipeline
- **THEN** the pre-check gate SHALL NOT filter "P"
- **AND** "P" SHALL proceed to the dedup gate

#### Scenario: Pre-check passes proposition conflicting with UNRELIABLE anchor

- **GIVEN** an active anchor "A" with authority UNRELIABLE in the `ConflictIndex`
- **AND** an incoming proposition "P" that has an indexed conflict with anchor "A"
- **WHEN** "P" enters the promotion pipeline
- **THEN** the pre-check gate SHALL NOT filter "P"
- **AND** "P" SHALL proceed to the dedup gate
...
- **GIVEN** `AnchorPromoter` constructed without a `ConflictIndex` (Optional.empty)
...
The conflict pre-check gate SHALL NOT change the set of propositions that ultimately get promoted for candidates that do not conflict with RELIABLE+ anchors. The optimization MUST be transparent to callers for non-conflicting inputs.

Specifically: for any proposition "P" that would have been promoted by the pre-F06 pipeline, if "P" does not conflict with any RELIABLE+ anchor in the index, "P" SHALL still be promoted by the post-F06 pipeline.

#### Scenario: Non-conflicting proposition promoted identically

- **GIVEN** a proposition "P" with no conflicts against any active anchor
...
#### Scenario: Proposition conflicting with PROVISIONAL-only anchors processed identically

- **GIVEN** a proposition "P" that conflicts only with PROVISIONAL anchors (no RELIABLE+ conflicts)
- **WHEN** "P" is processed through the post-F06 pipeline
...
2. The conflicting anchor ID from the index entry
3. The conflicting anchor's authority level
...
#### Scenario: Rejection logged with anchor details

- **GIVEN** a proposition "P" filtered by the pre-check gate due to a conflict with anchor "A" (RELIABLE, confidence 0.9)
- **WHEN** the pre-check filters "P"
- **THEN** an INFO log entry SHALL be emitted containing: proposition ID, anchor "A" ID, authority RELIABLE, confidence 0.9
...
The pre-check gate SHALL only filter propositions that conflict with anchors at RELIABLE or CANON authority. Conflicts with PROVISIONAL or UNRELIABLE anchors SHALL NOT trigger pre-check filtering.

The threshold MUST be hardcoded to RELIABLE (not configurable). PROVISIONAL and UNRELIABLE anchors are not stable enough to gate new propositions; filtering against low-authority anchors would be overly conservative and could starve the anchor pool.

#### Scenario: RELIABLE anchor triggers pre-check filter
...
#### Scenario: CANON anchor triggers pre-check filter

- **GIVEN** a `ConflictEntry` with authority CANON
- **WHEN** the pre-check evaluates the entry
- **THEN** the entry SHALL trigger filtering of the conflicting proposition

#### Scenario: UNRELIABLE anchor does not trigger pre-check filter

- **GIVEN** a `ConflictEntry` with authority UNRELIABLE
- **WHEN** the pre-check evaluates the entry
- **THEN** the entry SHALL NOT trigger filtering

#### Scenario: PROVISIONAL anchor does not trigger pre-check filter
...
- **WHEN** the proposition conflicts with a RELIABLE+ anchor in the index
...
- **WHEN** a proposition in the batch conflicts with a RELIABLE+ anchor in the index
...
`AnchorPromoter` SHALL accept `ConflictIndex` as an `Optional<ConflictIndex>` constructor parameter. This ensures backward compatibility: when no `ConflictIndex` bean is available (F05 not implemented or strategy is not INDEXED), the promoter operates without the pre-check gate.
...
- **WHEN** `AnchorPromoter` is constructed
...
- **PP2**: The pre-check gate MUST NOT modify anchor state (rank, authority, pinned status). It is read-only.
...
- **PP6**: All anchor invariants (A1-A4 per Article V of the constitution) MUST be preserved. The pre-check filters candidates; it does not modify ranks, authorities, promotion rules, or budget enforcement.
