...
The repository SHALL include a canonical DICE integration architecture document under `docs/dev` that explains how ARC-Mem composes with DICE proposition extraction and lifecycle behavior.
...
The DICE integration documentation SHALL explicitly describe how ARC-Mem relates to DICE Agent Memory. It SHALL state that ARC-Mem augments DICE memory retrieval with a governed working set for prompt assembly, rather than replacing DICE memory primitives.

#### Scenario: Memory layering is explicit
- **GIVEN** the DICE integration documentation
- **WHEN** memory behavior is reviewed
- **THEN** the docs SHALL describe DICE retrieval surfaces and ARC-Mem working-set injection behavior as separate layers
