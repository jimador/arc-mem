...
- **GIVEN** 5 candidate propositions where 2 match existing memory units via fast-path
...
- **GIVEN** all candidate propositions match existing memory units via normalized string comparison
- **WHEN** `batchIsDuplicate()` is called
...
The system SHALL provide a `batchDetectConflicts(List<String> candidateTexts, List<ContextUnit> existingUnits)` method on the conflict detector that evaluates multiple candidates against existing memory units in a single LLM call. The method SHALL return a `Map<String, List<ContextUnit>>` mapping each candidate to its list of conflicting memory units (empty list if no conflicts).

#### Scenario: Batch conflict evaluation
- **GIVEN** 4 candidate propositions and 10 existing memory units
- **WHEN** `batchDetectConflicts()` is called
- **THEN** all candidates SHALL be evaluated against all memory units in a single LLM call
- **AND** the result SHALL map each candidate to its conflicting memory units

#### Scenario: No conflicts found
- **GIVEN** candidate propositions that do not conflict with any existing memory units
- **WHEN** `batchDetectConflicts()` is called
...
- **AND** surviving candidates SHALL be promoted individually via `ArcMemEngine.promote()`
...
- I2: Batch size MUST NOT exceed configured maximum (`arc-mem.promotion.max-batch-size`, default 20)
- I3: If batch LLM call fails, the system SHALL fall back to individual per-proposition calls
- I4: Promotion itself (ArcMemEngine.promote) SHALL remain individual per proposition -- only evaluation gates are batched
