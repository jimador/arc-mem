# Memory Unit LLM Tools Specification

## ADDED Requirements

### Requirement: Query facts tool

The LLM SHALL have access to a `queryFacts(subject)` tool that performs semantic search against active memory units and returns matching results with activation score, authority, and confidence.
...
- **AND** results include id, text, activation score, authority, pinned status, and confidence
...
The LLM SHALL have access to a `listUnits()` tool that returns all active memory units for the current context, ordered by activation score descending.
...
- **AND** results are ordered by activation score descending
- **AND** each result includes id, text, activation score, authority, pinned, confidence

### Requirement: Pin fact tool

The LLM SHALL have access to a `pinFact(unitId)` tool that pins a memory unit, making it immune to capacity eviction. The tool MUST NOT pin archived memory units or modify CANON authority.
...
Tools MUST NOT allow operations that violate memory unit invariants. No tool SHALL modify authority, assign CANON, change activation score directly, or delete memory units.
...
- **I3**: Pin/unpin MUST NOT affect activation score, authority, or reinforcement count
...
The `ContextTools` class SHALL NOT expose a `reviseFact` LLM-callable tool. Memory unit text mutation SHALL NOT be available to the LLM. The `RevisionResult` record SHALL be removed.
