...
The project SHALL maintain a share-readiness checklist in `docs/README.md`. The checklist SHALL enumerate required implementation safety checks, evidence/provenance checks, and documentation completeness checks. Before community sharing, a maintainer SHALL run through the checklist and record pass/fail for each item.

#### Scenario: Checklist covers required check categories
- **GIVEN** the share-readiness checklist in `docs/README.md`
...
Before sharing, all legacy `docs/` files SHALL be classified in `docs/migration-tracker.md`. Unclassified files under `docs/` SHALL block share-ready signoff.
...
- **THEN** every file under `docs/` (excluding index files) SHALL appear in migration-tracker.md with a classification

### Requirement: Gate report contract

Completion of the share-readiness checklist SHALL be recorded as a brief summary note (inline in `docs/README.md` or as a git-tagged commit message) including: date, what was checked, and outcome for each check.
