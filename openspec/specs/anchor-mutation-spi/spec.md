...
The system SHALL provide an `UnitMutationStrategy` interface that gates all memory unit text mutation requests. Every memory unit mutation — regardless of origin (UI, LLM tool, conflict resolver) — MUST route through this SPI before execution. The interface is unsealed to allow test doubles; the `HitlOnlyMutationStrategy` implementation is `final`.
...
- **WHEN** no `unit.mutation.strategy` property is set
- **THEN** `HitlOnlyMutationStrategy` SHALL be the active bean

#### Scenario: Explicit hitl-only configuration

- **WHEN** `unit.mutation.strategy = hitl-only` is configured
- **THEN** `HitlOnlyMutationStrategy` SHALL be the active bean
...
The `UnitManipulationPanel` SHALL include an inline text field and "Revise" button for each memory unit in the edit card. Revision requests SHALL route through `UnitMutationStrategy` with `source = UI`.
