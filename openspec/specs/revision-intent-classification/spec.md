...
- `REVISION` — the incoming statement represents a user-intended correction or update to an existing memory unit
- `CONTRADICTION` — the incoming statement is inconsistent with an existing memory unit without revision intent
...
5. The existing memory unit's `authority` as a template variable
...
- **GIVEN** an existing PROVISIONAL memory unit "Anakin is a wizard"
...
- **GIVEN** an existing memory unit "The capital of France is Paris"
...
- **GIVEN** an existing memory unit "The king is alive"
...
`LlmConflictDetector.batchDetect()` SHALL extend the `batch-conflict-detection.jinja` prompt to return `conflictType` and `reasoning` per memory unit match within each candidate's results.
...
- **GIVEN** 3 candidates evaluated against 5 memory units
- **WHEN** candidate 1 revises memory unit A and contradicts memory unit B
- **THEN** the batch result SHALL report candidate 1 with two conflicts: one with `conflictType = REVISION` for memory unit A and one with `conflictType = CONTRADICTION` for memory unit B
...
- `WORLD_PROGRESSION` — return `COEXIST` (both facts are valid at different points in time; KEEP_EXISTING would reject the incoming proposition, causing memory unit loss when combined with REPLACE outcomes in the same conflict batch)
...
#### Scenario: Revision of PROVISIONAL memory unit accepted (permissive strategy)

- **GIVEN** the active mutation strategy allows `CONFLICT_RESOLVER` source
- **AND** a conflict with `conflictType = REVISION` against a PROVISIONAL memory unit
- **WHEN** `RevisionAwareConflictResolver.resolve()` is called
- **THEN** the resolution SHALL be `REPLACE`

#### Scenario: Revision of CANON memory unit rejected

- **GIVEN** a conflict with `conflictType = REVISION` against a CANON memory unit
- **WHEN** `RevisionAwareConflictResolver.resolve()` is called
- **THEN** the resolution SHALL be `KEEP_EXISTING`

#### Scenario: Revision of RELIABLE memory unit rejected by default

- **GIVEN** `unit.revision.reliable-revisable = false` (default)
- **AND** a conflict with `conflictType = REVISION` against a RELIABLE memory unit
- **WHEN** `RevisionAwareConflictResolver.resolve()` is called
- **THEN** the resolution SHALL delegate to `AuthorityConflictResolver`

#### Scenario: Revision of RELIABLE memory unit accepted when configured (permissive strategy)

- **GIVEN** the active mutation strategy allows `CONFLICT_RESOLVER` source
- **AND** `unit.revision.reliable-revisable = true`
- **AND** a conflict with `conflictType = REVISION` against a RELIABLE memory unit with confidence >= `replaceThreshold`
- **WHEN** `RevisionAwareConflictResolver.resolve()` is called
- **THEN** the resolution SHALL be `REPLACE`

#### Scenario: Revision below confidence threshold delegates to contradiction path

- **GIVEN** a conflict with `conflictType = REVISION` against an UNRELIABLE memory unit
...
#### Scenario: CANON memory unit is never revision-eligible

- **GIVEN** a CANON memory unit
...
#### Scenario: UNRELIABLE memory unit requires confidence threshold

- **GIVEN** an UNRELIABLE memory unit
...
| `reliable-revisable` | boolean | `false` | Whether RELIABLE memory units accept revisions |
