...
- **GIVEN** memory unit "A1" is evicted during budget enforcement when memory unit "A2" is promoted
...
- `DORMANCY_DECAY` -- memory unit was archived due to rank decay below minimum threshold
- `USER_ACTION` -- memory unit was explicitly archived or replaced by user action
- `EVICTION` -- memory unit was evicted during budget enforcement
