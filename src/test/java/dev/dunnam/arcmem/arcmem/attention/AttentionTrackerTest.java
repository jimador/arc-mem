...
class AttentionTrackerTest {
    ...
    private static final DiceAnchorsProperties.AttentionConfig ENABLED =
            new DiceAnchorsProperties.AttentionConfig(true, Duration.ofMinutes(5), 0.5, 3, 0.7, 0.2, 3, 20);

    private static final DiceAnchorsProperties.AttentionConfig DISABLED =
    ...
    class EventProcessing {
        ...
        void createsWindowOnFirstEvent() {
            ...
            tracker.onLifecycleEvent(AnchorLifecycleEvent.promoted(this, "ctx-1", "p1", "a1", 500));
        ...
        @Test
        @DisplayName("ConflictDetected updates windows for all conflicting anchors")
        void conflictDetectedUpdatesAll() {
            var tracker = tracker(ENABLED);
            tracker.onLifecycleEvent(AnchorLifecycleEvent.conflictDetected(
        ...
        void reinforcedIncrementsCount() {
            var tracker = tracker(ENABLED);
            tracker.onLifecycleEvent(AnchorLifecycleEvent.promoted(this, "ctx", "p1", "a1", 500));
            tracker.onLifecycleEvent(AnchorLifecycleEvent.reinforced(this, "ctx", "a1", 500, 520, 1));
        ...
        void evictedRemovesWindow() {
            ...
            tracker.onLifecycleEvent(AnchorLifecycleEvent.evicted(this, "ctx", "a1", 500));
    ...
    class ThresholdSignals {
        ...
        void pressureSpikeHysteresis() {
            ...
            tracker.onLifecycleEvent(AnchorLifecycleEvent.conflictDetected(this, "ctx", "t1", 1, List.of("a1")));
            tracker.onLifecycleEvent(AnchorLifecycleEvent.conflictDetected(this, "ctx", "t2", 1, List.of("a1")));
            tracker.onLifecycleEvent(AnchorLifecycleEvent.conflictDetected(this, "ctx", "t3", 1, List.of("a1")));
            ...
            reset(publisher);
            tracker.onLifecycleEvent(AnchorLifecycleEvent.conflictDetected(this, "ctx", "t4", 1, List.of("a1")));
        ...
        void hysteresisResetsAfterDrop() {
            ...
            for (int i = 0; i < 5; i++) {
                tracker.onLifecycleEvent(AnchorLifecycleEvent.reinforced(this, "ctx", "a1", 500, 510, i));
            }

            reset(publisher);
            // Re-trigger with 3 more conflicts (6 conflicts / 11 events = 0.545 >= 0.5, conflictCount 6 >= 3)
            tracker.onLifecycleEvent(AnchorLifecycleEvent.conflictDetected(this, "ctx", "t5", 1, List.of("a1")));
            tracker.onLifecycleEvent(AnchorLifecycleEvent.conflictDetected(this, "ctx", "t6", 1, List.of("a1")));
            tracker.onLifecycleEvent(AnchorLifecycleEvent.conflictDetected(this, "ctx", "t7", 1, List.of("a1")));
    ...
    class WindowRemoval {
        ...
        void archivedRemovesWindow() {
            ...
            tracker.onLifecycleEvent(AnchorLifecycleEvent.archived(this, "ctx", "a1", ArchiveReason.DORMANCY_DECAY));
            assertThat(tracker.getWindow("a1", "ctx")).isEmpty();
        }
    }
    ...
    class DisabledConfig {
        ...
        void disabledReturnsEmptyCollections() {
            ...
            assertThat(tracker.getHottestAnchors("ctx", 5)).isEmpty();
    ...
    class QueryApi {
        ...
        @Test
        @DisplayName("getHottestAnchors returns correct order and limit")
        void hottestAnchorsOrderAndLimit() {
            var tracker = tracker(ENABLED);
            // a1 gets 1 event
            tracker.onLifecycleEvent(AnchorLifecycleEvent.promoted(this, "ctx", "p1", "a1", 500));
            // a2 gets 3 events (hottest)
            tracker.onLifecycleEvent(AnchorLifecycleEvent.promoted(this, "ctx", "p2", "a2", 500));
            tracker.onLifecycleEvent(AnchorLifecycleEvent.reinforced(this, "ctx", "a2", 500, 510, 1));
            tracker.onLifecycleEvent(AnchorLifecycleEvent.reinforced(this, "ctx", "a2", 510, 520, 2));
            // a3 gets 2 events
            tracker.onLifecycleEvent(AnchorLifecycleEvent.promoted(this, "ctx", "p3", "a3", 500));
            tracker.onLifecycleEvent(AnchorLifecycleEvent.reinforced(this, "ctx", "a3", 500, 510, 1));

            var hottest = tracker.getHottestAnchors("ctx", 2);

            assertThat(hottest).hasSize(2);
            assertThat(hottest.getFirst().anchorId()).isEqualTo("a2");
        }
        ...
        void emptyContextReturnsEmpty() {
            ...
            assertThat(tracker.getHottestAnchors("unknown", 5)).isEmpty();
        ...
        void getAllWindowsIsolatesContext() {
            var tracker = tracker(ENABLED);
            tracker.onLifecycleEvent(AnchorLifecycleEvent.promoted(this, "ctx-A", "p1", "a1", 500));
            tracker.onLifecycleEvent(AnchorLifecycleEvent.promoted(this, "ctx-B", "p2", "a2", 500));

            assertThat(tracker.getAllWindows("ctx-A")).hasSize(1);
            assertThat(tracker.getAllWindows("ctx-A").getFirst().anchorId()).isEqualTo("a1");
            assertThat(tracker.getAllWindows("ctx-B")).hasSize(1);
        }

        @Test
        @DisplayName("snapshot keys match anchorIds of all tracked windows")
        void snapshotKeysMatchAnchorIds() {
            var tracker = tracker(ENABLED);
            tracker.onLifecycleEvent(AnchorLifecycleEvent.promoted(this, "ctx", "p1", "a1", 500));
            tracker.onLifecycleEvent(AnchorLifecycleEvent.promoted(this, "ctx", "p2", "a2", 500));
