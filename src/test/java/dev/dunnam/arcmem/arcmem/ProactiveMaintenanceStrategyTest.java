...
class ProactiveMaintenanceStrategyTest {
    ...
    private static final DiceAnchorsProperties.PressureConfig PRESSURE_CONFIG =
            new DiceAnchorsProperties.PressureConfig(true, 0.4, 0.3, 0.2, 0.1, 0.4, 0.8, 1.5, 5);

    private static final DiceAnchorsProperties.ProactiveConfig PROACTIVE_CONFIG =
            new DiceAnchorsProperties.ProactiveConfig(10, 0.1, 0.3, 0.6, 10, 0.8, 5, 50, 50, false, false);

    private static final DiceAnchorsProperties.AnchorConfig ANCHOR_CONFIG =
            new DiceAnchorsProperties.AnchorConfig(
                    ...
                    new DiceAnchorsProperties.RevisionConfig(true, false, 0.75),
                    null, null, null);

    @BeforeEach
    void setUp() {
        var maintenance = new DiceAnchorsProperties.MaintenanceConfig(MaintenanceMode.PROACTIVE, PROACTIVE_CONFIG);
        var properties = new DiceAnchorsProperties(
                ANCHOR_CONFIG, null, null, null, null, null, null, null, null, null, null,
                maintenance, PRESSURE_CONFIG, null, null);
        strategy = new ProactiveMaintenanceStrategy(pressureGauge, anchorEngine, repository,
    ...
    class AuditStep {
        ...
        @Test
        @DisplayName("high-rank HOT anchor with recent reinforcement scores high")
        void highRankHotAnchorScoresHigh() {
            var anchor = anchor("a1", 800, Authority.RELIABLE, false, 20, MemoryTier.HOT);
            var context = ctx("c1", List.of(anchor), 22);
            when(pressureGauge.computePressure(anyString(), anyInt(), anyInt()))
                    .thenReturn(new PressureScore(0.9, 0.9, 0.0, 0.0, 0.0, java.time.Instant.now()));

            var result = strategy.executeSweep(context);
            assertThat(result.anchorsAudited()).isEqualTo(1);
        }

        @Test
        @DisplayName("low-rank COLD anchor scores low and is reflected in prune candidate")
        void lowRankColdAnchorScoresLow() {
            var anchor = anchor("a2", 110, Authority.PROVISIONAL, false, 0, MemoryTier.COLD);
            var context = ctx("c1", List.of(anchor), 50);
            when(pressureGauge.computePressure(anyString(), anyInt(), anyInt()))
                    .thenReturn(new PressureScore(0.9, 0.9, 0.0, 0.0, 0.0, java.time.Instant.now()));
            ...
            verify(anchorEngine).archive("a2", ArchiveReason.PROACTIVE_MAINTENANCE);
        }
    }
    ...
    class RefreshStep {
        ...
        @Test
        @DisplayName("high-score anchor receives rank boost")
        void highScoreReceivesBoost() {
            // HOT + high rank + recent reinforcement -> score >= 0.7
            var anchor = anchor("a1", 800, Authority.RELIABLE, false, 22, MemoryTier.HOT);
            var context = ctx("c1", List.of(anchor), 22);
            when(pressureGauge.computePressure(anyString(), anyInt(), anyInt()))
                    .thenReturn(new PressureScore(0.5, 0.5, 0.0, 0.0, 0.0, java.time.Instant.now()));
        ...
        @Test
        @DisplayName("CANON anchor is immune to rank penalty")
        void canonAnchorImmuneToRankPenalty() {
            // COLD + low rank + no reinforcement -> would score low, but CANON
            var anchor = anchor("a1", 110, Authority.CANON, false, 0, MemoryTier.COLD);
            var context = ctx("c1", List.of(anchor), 50);
            when(pressureGauge.computePressure(anyString(), anyInt(), anyInt()))
                    .thenReturn(new PressureScore(0.5, 0.5, 0.0, 0.0, 0.0, java.time.Instant.now()));

            strategy.executeSweep(context);
            // CANON anchors skip rank penalty — no updateRank call at all
            verify(repository, never()).updateRank(anyString(), anyInt());
        }

        @Test
        @DisplayName("pinned anchor is immune to rank penalty")
        void pinnedAnchorImmuneToRankPenalty() {
            var anchor = anchor("a1", 110, Authority.RELIABLE, true, 0, MemoryTier.COLD);
            var context = ctx("c1", List.of(anchor), 50);
            when(pressureGauge.computePressure(anyString(), anyInt(), anyInt()))
                    .thenReturn(new PressureScore(0.5, 0.5, 0.0, 0.0, 0.0, java.time.Instant.now()));

            strategy.executeSweep(context);
            // Pinned anchors skip rank penalty — no updateRank call at all
            verify(repository, never()).updateRank(anyString(), anyInt());
        }
        ...
        void rankBoostClampedAtMax() {
            var anchor = anchor("a1", 870, Authority.RELIABLE, false, 22, MemoryTier.HOT);
            var context = ctx("c1", List.of(anchor), 22);
            when(pressureGauge.computePressure(anyString(), anyInt(), anyInt()))
                    .thenReturn(new PressureScore(0.5, 0.5, 0.0, 0.0, 0.0, java.time.Instant.now()));
        ...
        void rankPenaltyClampedAtMin() {
            var anchor = anchor("a1", 120, Authority.PROVISIONAL, false, 0, MemoryTier.COLD);
            var context = ctx("c1", List.of(anchor), 50);
            when(pressureGauge.computePressure(anyString(), anyInt(), anyInt()))
                    .thenReturn(new PressureScore(0.5, 0.5, 0.0, 0.0, 0.0, java.time.Instant.now()));
            when(invariantEvaluator.evaluate(anyString(), any(), any(), any()))
    ...
    class ConsolidateStep {
        ...
        @Test
        @DisplayName("RELIABLE anchor meeting all criteria is routed to CanonizationGate")
        void reliableAnchorMeetingCriteriaIsRouted() {
        ...
        @Test
        @DisplayName("RELIABLE anchor missing reinforcement count is not routed")
        void reliableAnchorMissingReinforcements() {
            var anchor = anchor("a1", 800, Authority.RELIABLE, false, 5, MemoryTier.HOT);
            var context = ctx("c1", List.of(anchor), 7);
            when(pressureGauge.computePressure(anyString(), anyInt(), anyInt()))
                    .thenReturn(new PressureScore(0.5, 0.5, 0.0, 0.0, 0.0, java.time.Instant.now()));
        ...
        @Test
        @DisplayName("PROVISIONAL anchor is skipped")
        void provisionalAnchorSkipped() {
            var anchor = anchor("a1", 800, Authority.PROVISIONAL, false, 20, MemoryTier.HOT);
            var context = ctx("c1", List.of(anchor), 22);
            when(pressureGauge.computePressure(anyString(), anyInt(), anyInt()))
                    .thenReturn(new PressureScore(0.5, 0.5, 0.0, 0.0, 0.0, java.time.Instant.now()));
        ...
        @Test
        @DisplayName("UNRELIABLE anchor is skipped")
        void unreliableAnchorSkipped() {
            var anchor = anchor("a1", 800, Authority.UNRELIABLE, false, 20, MemoryTier.HOT);
            var context = ctx("c1", List.of(anchor), 22);
            when(pressureGauge.computePressure(anyString(), anyInt(), anyInt()))
                    .thenReturn(new PressureScore(0.5, 0.5, 0.0, 0.0, 0.0, java.time.Instant.now()));
    ...
    class PruneStep {
        ...
        @Test
        @DisplayName("anchor below hard floor is always pruned")
        void belowHardFloorAlwaysPruned() {
            ...
            var anchor = anchor("a1", 100, Authority.PROVISIONAL, false, 0, MemoryTier.COLD);
            when(pressureGauge.computePressure(anyString(), anyInt(), anyInt()))
                    .thenReturn(new PressureScore(0.5, 0.5, 0.0, 0.0, 0.0, java.time.Instant.now()));
            when(invariantEvaluator.evaluate(anyString(), any(), any(), any()))
                    .thenReturn(new InvariantEvaluation(List.of(), 0));

            strategy.executeSweep(ctx("c1", List.of(anchor), 100));
            verify(anchorEngine).archive("a1", ArchiveReason.PROACTIVE_MAINTENANCE);
        }

        @Test
        @DisplayName("anchor below soft floor pruned when pressure >= threshold")
        void belowSoftFloorPrunedUnderPressure() {
            ...
            var anchor = anchor("a1", 200, Authority.PROVISIONAL, false, 2, MemoryTier.WARM);
            var context = ctx("c1", List.of(anchor), 10);
            when(pressureGauge.computePressure(anyString(), anyInt(), anyInt()))
        ...
        @Test
        @DisplayName("anchor above soft floor is never pruned")
        void aboveSoftFloorNotPruned() {
            ...
            strategy.executeSweep(context);
            verify(anchorEngine, never()).archive(anyString(), any(ArchiveReason.class));
        }

        @Test
        @DisplayName("CANON anchor immune to pruning regardless of score")
        void canonAnchorImmuneTopruning() {
            var anchor = anchor("a1", 100, Authority.CANON, false, 0, MemoryTier.COLD);
            var context = ctx("c1", List.of(anchor), 100);
            when(pressureGauge.computePressure(anyString(), anyInt(), anyInt()))
                    .thenReturn(new PressureScore(0.9, 0.9, 0.0, 0.0, 0.0, java.time.Instant.now()));
        ...
        @Test
        @DisplayName("pinned anchor immune to pruning regardless of score")
        void pinnedAnchorImmuneToPruning() {
            var anchor = anchor("a1", 100, Authority.PROVISIONAL, true, 0, MemoryTier.COLD);
            var context = ctx("c1", List.of(anchor), 100);
            when(pressureGauge.computePressure(anyString(), anyInt(), anyInt()))
                    .thenReturn(new PressureScore(0.9, 0.9, 0.0, 0.0, 0.0, java.time.Instant.now()));
    ...
    class FullSweep {
        ...
        void sweepReturnsCorrectMetrics() {
            var highAnchor = anchor("h1", 800, Authority.RELIABLE, false, 22, MemoryTier.HOT);
            var lowAnchor = anchor("l1", 100, Authority.PROVISIONAL, false, 0, MemoryTier.COLD);
            var context = ctx("c1", List.of(highAnchor, lowAnchor), 22);
            when(pressureGauge.computePressure(anyString(), anyInt(), anyInt()))
                    .thenReturn(new PressureScore(0.9, 0.9, 0.0, 0.0, 0.0, java.time.Instant.now()));
            when(invariantEvaluator.evaluate(anyString(), any(), any(), any()))
                    .thenReturn(new InvariantEvaluation(List.of(), 0));
            ...
            assertThat(result.anchorsAudited()).isEqualTo(2);
            assertThat(result.anchorsPruned()).isGreaterThanOrEqualTo(0);
        ...
        void lightSweepExecutesAllSteps() {
            var anchor = anchor("a1", 500, Authority.RELIABLE, false, 10, MemoryTier.WARM);
            var context = ctx("c1", List.of(anchor), 10);
            when(pressureGauge.computePressure(anyString(), anyInt(), anyInt()))
                    .thenReturn(new PressureScore(0.5, 0.5, 0.0, 0.0, 0.0, java.time.Instant.now()));
    ...
    private static MaintenanceContext ctx(String contextId, List<Anchor> anchors, int turn) {
        return new MaintenanceContext(contextId, anchors, turn, null);
    }

    private static Anchor anchor(String id, int rank, Authority authority, boolean pinned,
                                  int reinforcementCount, MemoryTier tier) {
        return new Anchor(id, "text-" + id, rank, authority, pinned, 0.9,
