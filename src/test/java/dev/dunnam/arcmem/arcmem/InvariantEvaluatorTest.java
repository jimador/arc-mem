...
class InvariantEvaluatorTest {
    ...
    class AuthorityFloorEvaluation {
        ...
        @Test
        @DisplayName("blocks demotion when anchor authority is at or above floor")
        void blocksDemotionAboveFloor() {
            ...
            var anchor = Anchor.withoutTrust("a1", "safety protocol", 500, Authority.RELIABLE, false, 0.9, 3);
            var eval = evaluator.evaluate(CONTEXT_ID, ProposedAction.DEMOTE, List.of(anchor), anchor);
        ...
        @Test
        @DisplayName("does not block when anchor text does not match pattern")
        void noBlockWhenTextDoesNotMatch() {
            var rule = new InvariantRule.AuthorityFloor(
                    "af-2", InvariantStrength.MUST, null, "safety", Authority.RELIABLE);
            when(ruleProvider.rulesForContext(CONTEXT_ID)).thenReturn(List.of(rule));

            var anchor = Anchor.withoutTrust("a1", "weather data", 500, Authority.RELIABLE, false, 0.9, 3);
            var eval = evaluator.evaluate(CONTEXT_ID, ProposedAction.DEMOTE, List.of(anchor), anchor);
        ...
        void noBlockForArchiveAction() {
            ...
            var anchor = Anchor.withoutTrust("a1", "safety protocol", 500, Authority.RELIABLE, false, 0.9, 3);
            var eval = evaluator.evaluate(CONTEXT_ID, ProposedAction.ARCHIVE, List.of(anchor), anchor);
        ...
        void triggersOnAuthorityChangeAction() {
            ...
            var anchor = Anchor.withoutTrust("a1", "safety protocol", 500, Authority.RELIABLE, false, 0.9, 3);
            var eval = evaluator.evaluate(CONTEXT_ID, ProposedAction.AUTHORITY_CHANGE, List.of(anchor), anchor);
    ...
    class EvictionImmunityEvaluation {
        ...
        @Test
        @DisplayName("blocks eviction of matching anchor")
        void blocksEvictionOfMatchingAnchor() {
            var rule = new InvariantRule.EvictionImmunity(
                    "ei-1", InvariantStrength.MUST, null, "core rule");
            when(ruleProvider.rulesForContext(CONTEXT_ID)).thenReturn(List.of(rule));

            var anchor = Anchor.withoutTrust("a1", "core rule of engagement", 200, Authority.PROVISIONAL, false, 0.8, 1);
            var eval = evaluator.evaluate(CONTEXT_ID, ProposedAction.EVICT, List.of(anchor), anchor);
    ...
    class MinAuthorityCountEvaluation {
        ...
        void blocksWhenCountDropsBelowMinimum() {
            ...
            var a1 = Anchor.withoutTrust("a1", "fact one", 600, Authority.RELIABLE, false, 0.9, 5);
            var a2 = Anchor.withoutTrust("a2", "fact two", 500, Authority.RELIABLE, false, 0.8, 3);
            var anchors = List.of(a1, a2);

            var eval = evaluator.evaluate(CONTEXT_ID, ProposedAction.ARCHIVE, anchors, a2);
        ...
        void noFireWhenTargetBelowMinimumAuthority() {
            ...
            var a1 = Anchor.withoutTrust("a1", "fact one", 600, Authority.RELIABLE, false, 0.9, 5);
            var a2 = Anchor.withoutTrust("a2", "fact two", 500, Authority.RELIABLE, false, 0.8, 3);
            var a3 = Anchor.withoutTrust("a3", "provisional", 300, Authority.PROVISIONAL, false, 0.7, 1);
            var anchors = List.of(a1, a2, a3);

            // Archiving a3 (PROVISIONAL) should not trigger the RELIABLE count rule
            var eval = evaluator.evaluate(CONTEXT_ID, ProposedAction.ARCHIVE, anchors, a3);
    ...
    class ArchiveProhibitionEvaluation {
        ...
        @Test
        @DisplayName("blocks archive of matching anchor")
        void blocksArchiveOfMatchingAnchor() {
            var rule = new InvariantRule.ArchiveProhibition(
                    "ap-1", InvariantStrength.MUST, null, "permanent");
            when(ruleProvider.rulesForContext(CONTEXT_ID)).thenReturn(List.of(rule));

            var anchor = Anchor.withoutTrust("a1", "permanent record", 500, Authority.RELIABLE, false, 0.9, 3);
            var eval = evaluator.evaluate(CONTEXT_ID, ProposedAction.ARCHIVE, List.of(anchor), anchor);
    ...
    class ContextScoping {
        ...
        void globalRulesApplyToAllContexts() {
            ...
            var anchor = Anchor.withoutTrust("a1", "core principle", 200, Authority.PROVISIONAL, false, 0.8, 1);
            var eval = evaluator.evaluate("any-context", ProposedAction.EVICT, List.of(anchor), anchor);
        ...
        void contextSpecificRulesOnlyApplyToTheirContext() {
            ...
            var anchor = Anchor.withoutTrust("a1", "core principle", 200, Authority.PROVISIONAL, false, 0.8, 1);
            var eval = evaluator.evaluate("other-context", ProposedAction.EVICT, List.of(anchor), anchor);
    ...
    class MustVsShouldSeparation {
        ...
        void mustViolationIsBlocking() {
            ...
            var anchor = Anchor.withoutTrust("a1", "critical data", 500, Authority.RELIABLE, false, 0.9, 3);
            var eval = evaluator.evaluate(CONTEXT_ID, ProposedAction.ARCHIVE, List.of(anchor), anchor);
        ...
        void shouldViolationIsWarningOnly() {
            ...
            var anchor = Anchor.withoutTrust("a1", "preferred data", 500, Authority.RELIABLE, false, 0.9, 3);
            var eval = evaluator.evaluate(CONTEXT_ID, ProposedAction.ARCHIVE, List.of(anchor), anchor);
    ...
    class NoViolations {
        ...
        void noViolationsWhenActionDoesNotMatch() {
            ...
            var anchor = Anchor.withoutTrust("a1", "data record", 500, Authority.RELIABLE, false, 0.9, 3);
        ...
        void emptyRulesReturnsCleanEvaluation() {
            ...
            var anchor = Anchor.withoutTrust("a1", "some data", 500, Authority.RELIABLE, false, 0.9, 3);
            var eval = evaluator.evaluate(CONTEXT_ID, ProposedAction.ARCHIVE, List.of(anchor), anchor);
    ...
    @Nested
    @DisplayName("null target anchor")
    class NullTargetAnchor {

        @Test
        @DisplayName("all rules gracefully handle null target anchor")
