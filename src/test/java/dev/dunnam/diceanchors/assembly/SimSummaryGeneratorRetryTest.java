...
@DisplayName("SimSummaryGenerator — Retry and Fallback (Sections 8.3, 8.4)")
@ExtendWith(MockitoExtension.class)
class SimSummaryGeneratorRetryTest {
    ...
    private SimSummaryGenerator generator;

    @BeforeEach
    void setUp() {
        generator = new SimSummaryGenerator(chatModel);
    ...
    class RetryBehavior {
        ...
        void allAttemptsFailUsesFallback() {
            ...
            var protectedContent = List.of(
                    new ProtectedContent("a-1", "Dragon sleeps", 100, "anchor"),
                    new ProtectedContent("a-2", "Sword is cursed", 50, "anchor")
            );
        ...
        void zeroRetriesWithFailureImmediateFallback() {
            ...
            var protectedContent = List.of(
                    new ProtectedContent("a-1", "Important fact", 100, "anchor")
            );
    ...
    class ExtractiveFallback {
        ...
        void fallbackSortsByPriorityDescending() {
            ...
            var protectedContent = List.of(
                    new ProtectedContent("a-1", "High priority text", 100, "anchor"),
                    new ProtectedContent("a-2", "Low priority text", 50, "anchor"),
                    new ProtectedContent("a-3", "Medium priority text", 75, "anchor")
            );
