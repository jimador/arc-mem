...
class CompactedContextProviderAtomicityTest {
    ...
    @BeforeEach
    void setUp() {
        provider = new CompactedContextProvider(List.of(), summaryGenerator, anchorEngine, eventPublisher);
    }
    ...
    class Atomicity {
        ...
        void validationFailureLeavesMessagesIntact() {
            // Summary won't contain the anchor's significant words
            ...
            when(anchorEngine.inject(CONTEXT_ID)).thenReturn(List.of(
                    Anchor.withoutTrust("a-1", "The cursed blade corrupts its wielder",
        ...
        void rollbackOnFallbackValidationFailure() {
            // Fallback summary won't match the anchors checked by validator
            when(summaryGenerator.generateSummary(any(), anyString(), any(), anyInt(), any()))
                    .thenReturn(new SummaryResult("Some fallback text.", 2, true));
            when(anchorEngine.inject(CONTEXT_ID)).thenReturn(List.of(
                    Anchor.withoutTrust("a-1", "Elminster lives in Shadowdale tower",
        ...
        void successfulCompactionClearsMessagesAndStoresSummary() {
            ...
            when(anchorEngine.inject(CONTEXT_ID)).thenReturn(List.of(
                    Anchor.withoutTrust("a-1", "The dragon sleeps in the mountain cave",
    ...
    class ResultFields {
        ...
        void compactionAppliedFalseOnValidationFailure() {
            ...
            when(anchorEngine.inject(CONTEXT_ID)).thenReturn(List.of(
                    Anchor.withoutTrust("a-1", "The cursed blade corrupts wielders",
                            700, Authority.RELIABLE, false, 0.9, 3)
            ));
    ...
    class EventPublishing {
        ...
        void eventPublishedOnFallbackSuccess() {
            // Fallback summary that passes validation (no anchors to check against)
            when(summaryGenerator.generateSummary(any(), anyString(), any(), anyInt(), any()))
        ...
        void noEventOnRollback() {
            ...
            when(anchorEngine.inject(CONTEXT_ID)).thenReturn(List.of(
                    Anchor.withoutTrust("a-1", "The dragon guards golden treasure",
                            700, Authority.RELIABLE, false, 0.9, 3)
            ));
    ...
    class OtelAttributes {
        ...
        void noOtelAttributesWhenNoCompaction() {
            // Simply add messages but never call compact().
            // Span.current() returns a noop span — no attributes should be set.
            provider.addMessage(CONTEXT_ID, "msg1");
            provider.addMessage(CONTEXT_ID, "msg2");

            // If compact() is never called, no span attributes are written.
            // Verify by ensuring summaryGenerator was never invoked (proxy for no compaction path).
        ...
        void noNpeWithNoopSpan() {
            // Span.current() returns a noop span when no OTEL is configured.
            // Verify that compact() does not throw.
            when(summaryGenerator.generateSummary(any(), anyString(), any(), anyInt(), any()))
                    .thenReturn(new SummaryResult("Summary.", 0, false));
            when(anchorEngine.inject(CONTEXT_ID)).thenReturn(List.of());
