...
class KnowledgeBrowserGraphTest {
    ...
    void appliesGraphControlFiltersOnRefresh() {
        when(anchorEngine.inject(any())).thenReturn(List.of());
        when(anchorRepository.findByContextIdValue(any())).thenReturn(List.of());
        when(anchorRepository.findEntityMentionGraph(eq("sim-ctx"), any(EntityMentionGraphFilter.class)))
                .thenReturn(EntityMentionGraph.empty());

        var panel = new KnowledgeBrowserPanel(anchorEngine, anchorRepository);
        ...
        verify(anchorRepository, atLeast(2)).findEntityMentionGraph(eq("sim-ctx"), captor.capture());
