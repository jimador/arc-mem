...
class SimulationExtractionServiceTest {
    ...
    @Mock private AnchorRepository anchorRepository;
    @Mock private AnchorPromoter promoter;
    ...
    void persistsWithoutNamedEntityRepository() {
        ...
        when(promoter.evaluateAndPromote(eq("ctx-1"), eq(List.of(proposition)))).thenReturn(1);
        ...
        verify(promoter).evaluateAndPromote("ctx-1", List.of(proposition));
    ...
    void usesFullPersistPathWhenNamedEntityRepositoryAvailable() {
        ...
        when(proposition.getId()).thenReturn("p1");
        when(proposition.getText()).thenReturn("Fact text");
        when(promoter.evaluateAndPromote(eq("ctx-1"), eq(List.of(proposition)))).thenReturn(0);
    ...
    @Test
    @DisplayName("falls back to proposition-only persistence when full persist throws")
    void fallsBackWhenFullPersistThrows() {
        when(pipeline.process(anyList(), any())).thenReturn(propositionResults);
        when(propositionResults.propositionsToPersist()).thenReturn(List.of(proposition));
        when(proposition.getId()).thenReturn("p1");
        when(proposition.getText()).thenReturn("Fact text");
