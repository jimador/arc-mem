 ...
 * <dl>
 *   <dt>{@code id}</dt>
 *   <dd>The Neo4j node ID of the underlying {@code PropositionNode}. Anchors share the
 *       same node as the proposition; {@code rank > 0} signals anchor status.</dd>
 *
 *   <dt>{@code text}</dt>
 ...
 *
 *   <dt>{@code rank}</dt>
 *   <dd>Priority within the anchor budget. Higher rank = higher priority = less likely
 ...
 *       the LLM must treat this anchor as ground truth. See {@link Authority} for the
 *       compliance mapping and invariants A3a–A3e.</dd>
 *
 *   <dt>{@code pinned}</dt>
 *   <dd>When {@code true}, the anchor is excluded from automatic eviction and automatic
 *       demotion (invariant A3d). Pinned anchors can still be explicitly archived or
 *       demoted.</dd>
 *
 ...
 *   <dd>Number of times this anchor has been reinforced. Drives authority promotion
 *       thresholds (3× → UNRELIABLE, 7× → RELIABLE) and trust re-evaluation triggers.</dd>
 *
 *   <dt>{@code trustScore}</dt>
 *   <dd>Trust pipeline result, or {@code null} for anchors created before trust scoring
 ...
 *       anchors (> 0.7) receive a rank boost during budget eviction priority calculations.
 *       Low-importance anchors (&lt; 0.3) may be evicted earlier. Default 0.0 has no
 ...
 *
 * <h2>Lifecycle</h2>
 * An anchor starts at PROVISIONAL authority after promotion. It may be reinforced
 ...
 *   <li><strong>A2</strong>: authority is never null for an active anchor.</li>
 *   <li><strong>A3a–A3e</strong>: see {@link Authority} for the bidirectional authority
 *       lifecycle invariants.</li>
 *   <li><strong>A4</strong>: pinned anchors are immune to rank-based eviction and
 *       automatic authority demotion.</li>
 *   <li><strong>A5</strong>: trustScore may be null for anchors created before trust
